...
export function RemoteMcpConnectionSetup({ provider, state: s, actions: a, agents, connectionId, fixedGrantKind, lockedAgentId, host = "page", authorizationUrl }: {
  ...
  const boundary = <InlineBanner compact>
    {t("remotemcpconnectionsetup.general.paperclipcontrolsaccesstothetoolslisted")} {external("manage", provider.name)}.
  </InlineBanner>;
  ...
  return <div className={host === "dialog" ? "min-w-0 text-foreground" : "mx-auto max-w-6xl p-4 text-foreground sm:p-8"} data-remote-mcp-provider={provider.id}>
    ...
    <main className="space-y-6">
        ...
        {s.step === "permissions" && <>
          <div className="flex items-center justify-between gap-3"><h2 className="text-sm font-semibold">{t("remotemcpconnectionsetup.general.permissions")}</h2><Button variant="outline" onClick={a.finish}>{t("remotemcpconnectionsetup.general.connectionsettings")}</Button></div>
        ...
        {s.step === "connect" && <>
          ...
          {s.connectStatus === "sign_in" && provider.supportsBrowserAuth ? <>
            <div role="status"><InlineBanner title={`Finish signing in to ${provider.name}`}>
              {t("remotemcpconnectionsetup.general.completesigninintheproviderwindow")}</InlineBanner></div>
            <p className="text-sm text-muted-foreground">{t("remotemcpconnectionsetup.general.ifawindowdidnotopen")} {authorizationUrl ? <a className="text-current underline" href={authorizationUrl} onClick={() => a.openProvider("sign_in")} target="_blank" rel="noopener noreferrer">{t("remotemcpconnectionsetup.general.opensigninagain")}</a> : external("sign_in", "open sign-in again")}.</p>
            {footer(<><Button variant="outline" onClick={a.cancelConnect}>{t("remotemcpconnectionsetup.general.cancelsignin")}</Button><Button disabled>{t("remotemcpconnectionsetup.general.waitingforsignin")}</Button></>)}
            ...
            {s.connectStatus === "cancelled" && <p role="status" className="text-sm text-muted-foreground">{t("remotemcpconnectionsetup.general.connectioncancelledyoursetupdetailsarepreserved")}</p>}
            <fieldset disabled={busy} className="min-w-0 space-y-5">
              <div className="space-y-2">
                <div className="flex items-center gap-2"><Label htmlFor={`${uid}-url`}>{t("remotemcpconnectionsetup.general.mcpserverurl")}</Label><FieldHelp label={t("remotemcpconnectionsetup.general.mcpserverurl1")}>{provider.urlHelp}</FieldHelp></div>
              ...
              <details open={s.advanced} onToggle={(event) => { if (event.currentTarget.open !== s.advanced) change({ advanced: event.currentTarget.open }); }}>
                <summary className="cursor-pointer text-sm font-medium">{t("remotemcpconnectionsetup.general.advancedauthentication")}</summary>
                <div className="space-y-4 pt-4">
                  <p className="text-sm text-muted-foreground">{provider.authHelp}</p>
                  <div className="space-y-2"><Label htmlFor={`${uid}-auth`}>{t("remotemcpconnectionsetup.general.authentication")}</Label><select id={`${uid}-auth`} className={selectClass} value={s.auth} onChange={(event) => change({ auth: event.target.value as RemoteMcpSetupState["auth"] })}>
                    {provider.supportsBrowserAuth && <option value="auto">{t("remotemcpconnectionsetup.general.automaticsigninifrequired")}</option>}<option value="bearer">{t("remotemcpconnectionsetup.general.bearertoken")}</option><option value="headers">{t("remotemcpconnectionsetup.general.customheaders")}</option><option value="none">{t("remotemcpconnectionsetup.general.noadditionalauthentication")}</option>
                  </select></div>
                  {s.auth === "bearer" && <div className="space-y-2"><div className="flex items-center gap-2"><Label htmlFor={`${uid}-token`}>{t("remotemcpconnectionsetup.general.bearertoken2")}</Label><FieldHelp label={t("remotemcpconnectionsetup.general.bearertoken3")}>{t("remotemcpconnectionsetup.general.pastethetokenonlywithouttheword")}</FieldHelp></div><Input id={`${uid}-token`} type="password" autoComplete="off" value={s.token} onChange={(event) => change({ token: event.target.value })} /></div>}
                  {(s.auth === "bearer" || s.auth === "headers") && <div className="space-y-3">
                    <p className="text-sm font-medium">{s.auth === "bearer" ? t("remotemcpconnectionsetup.general.additionalheaders") : t("remotemcpconnectionsetup.general.headers")}</p>
                    {s.headers.map((header, index) => <div key={header.id} className="flex flex-wrap items-end gap-2">
                      <div className="min-w-0 flex-1 space-y-2"><Label htmlFor={`${uid}-${header.id}-name`}>{t("remotemcpconnectionsetup.general.header")} {index + 1} {t("remotemcpconnectionsetup.general.name")}</Label><Input id={`${uid}-${header.id}-name`} value={header.name} placeholder={provider.id === "arcade" ? "Arcade-User-ID" : "Header name"} onChange={(event) => change({ headers: s.headers.map((h) => h.id === header.id ? { ...h, name: event.target.value } : h) })} /></div>
                      <div className="min-w-0 flex-1 space-y-2"><Label htmlFor={`${uid}-${header.id}-value`}>{t("remotemcpconnectionsetup.general.header4")} {index + 1} {t("remotemcpconnectionsetup.general.value")}</Label><Input id={`${uid}-${header.id}-value`} type="password" autoComplete="off" value={header.value} onChange={(event) => change({ headers: s.headers.map((h) => h.id === header.id ? { ...h, value: event.target.value } : h) })} /></div>
                    ...
                    <Button type="button" variant="outline" size="sm" onClick={() => change({ headers: [...s.headers, { id: crypto.randomUUID(), name: "", value: "" }] })}><Plus className="size-4" />{t("remotemcpconnectionsetup.general.addheader")}</Button>
                  </div>}
                </div>
              </details>
            </fieldset>
            {busy && <p role="status" className="flex items-center gap-2 text-sm"><Loader2 className="size-4 animate-spin motion-reduce:animate-none" />{t("remotemcpconnectionsetup.general.connectinganddiscoveringtools")}</p>}
            {footer(<><Button type="button" variant="outline" disabled={busy} onClick={() => s.setupComplete ? a.finish() : a.navigate("access")}>{t("remotemcpconnectionsetup.general.back")}</Button><Button type="submit" disabled={busy || !s.url.trim()}>{busy ? "Connecting…" : error || s.connectStatus === "cancelled" ? "Try again" : "Connect"}</Button></>)}
          </form>}
        </>}

        {s.step === "management" && <>
          {!s.connected ? <InlineBanner tone="warning" title={t("remotemcpconnectionsetup.general.disconnected")}>{t("remotemcpconnectionsetup.general.agentscannotusethisconnectionreconnectto")}</InlineBanner> : <div className="space-y-2"><p className="flex items-center gap-2 text-sm"><CheckCircle2 className="size-4" />{t("remotemcpconnectionsetup.general.connected")}{s.identity ? ` as ${s.identity}` : ""}</p><p className="text-sm text-muted-foreground">{s.grantKind === "user" ? t("remotemcpconnectionsetup.general.justme") : t("remotemcpconnectionsetup.general.anyhumanintheorganization")} · {s.tools.length} {t("remotemcpconnectionsetup.general.tools")} {s.allAgents ? t("remotemcpconnectionsetup.general.anyagent") : `${s.agentIds.length} agents with access`}</p></div>}
          <div className="flex flex-wrap gap-2"><Button onClick={() => a.navigate("access")}>{t("remotemcpconnectionsetup.general.whocanusethisconnection")}</Button><Button variant="outline" disabled={!s.connected} onClick={() => a.navigate("permissions")}>{t("remotemcpconnectionsetup.general.permissions5")}</Button></div>
          <p className="text-sm text-muted-foreground">{t("remotemcpconnectionsetup.general.refreshthecatalogafterchangingtoolsin")} {provider.name}{t("remotemcpconnectionsetup.general.existingoffandaskfirstchoicesare")}</p>
          <Button variant="outline" disabled={!s.connected || s.refreshing} onClick={a.refresh}>{s.refreshing ? t("remotemcpconnectionsetup.general.refreshing") : t("remotemcpconnectionsetup.general.refreshtools")}</Button>
        ...
        {s.step === "draft" && <><p className="text-sm">{t("remotemcpconnectionsetup.general.youraccesschoicesandsetupprogressare")}</p><div className="flex justify-end"><Button onClick={a.resumeDraft}>{t("remotemcpconnectionsetup.general.resumesetup")}</Button></div></>}
        </div>
    </main>
  </div>;
}
