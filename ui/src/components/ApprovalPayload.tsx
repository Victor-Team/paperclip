...
function SkillList({ values }: { values: unknown }) {
  ...
  return (
    <div className="flex items-start gap-2">
      <span className="text-muted-foreground w-20 sm:w-24 shrink-0 text-xs pt-0.5">{t("approvalpayload.general.skills")}</span>
...
export function HireAgentPayload({ payload }: { payload: Record<string, unknown> }) {
  ...
  return (
    <div className="mt-3 space-y-1.5 text-sm">
      <div className="flex items-center gap-2">
        <span className="text-muted-foreground w-20 sm:w-24 shrink-0 text-xs">{t("approvalpayload.general.name")}</span>
        <span className="font-medium">{String(payload.name ?? "—")}</span>
      </div>
      <PayloadField label={t("approvalpayload.general.role")} value={payload.role} />
      <PayloadField label={t("approvalpayload.general.title")} value={payload.title} />
      <PayloadField label={t("approvalpayload.general.icon")} value={payload.icon} />
      {!!payload.capabilities && (
        <div className="flex items-start gap-2">
          <span className="text-muted-foreground w-20 sm:w-24 shrink-0 text-xs pt-0.5">{t("approvalpayload.general.capabilities")}</span>
          <span className="text-muted-foreground">{String(payload.capabilities)}</span>
        </div>
      )}
      {!!payload.adapterType && (
        <div className="flex items-center gap-2">
          <span className="text-muted-foreground w-20 sm:w-24 shrink-0 text-xs">{t("approvalpayload.general.adapter")}</span>
...
export function CeoStrategyPayload({ payload }: { payload: Record<string, unknown> }) {
  ...
  return (
    <div className="mt-3 space-y-1.5 text-sm">
      <PayloadField label={t("approvalpayload.general.title1")} value={payload.title} />
      {!!plan && (
...
export function BudgetOverridePayload({ payload }: { payload: Record<string, unknown> }) {
  ...
  return (
    <div className="mt-3 space-y-1.5 text-sm">
      <PayloadField label={t("approvalpayload.general.scope")} value={payload.scopeName ?? payload.scopeType} />
      <PayloadField label={t("approvalpayload.general.window")} value={payload.windowKind} />
    ...
    <div className="mt-4 space-y-3.5 text-sm">
      {title && (
        <div className="space-y-1">
          <p className="text-(length:--text-micro) font-medium uppercase tracking-(--tracking-label) text-muted-foreground">{t("approvalpayload.general.title2")}</p>
          <p className="font-medium leading-6 text-foreground">{title}</p>
        </div>
      )}
      {summary && (
        <div className="space-y-1">
          <p className="text-(length:--text-micro) font-medium uppercase tracking-(--tracking-label) text-muted-foreground">{t("approvalpayload.general.summary")}</p>
      ...
      {recommendedAction && (
        <div className="rounded-lg border border-amber-500/20 bg-amber-500/10 px-3.5 py-3">
          <p className="text-(length:--text-micro) font-medium uppercase tracking-(--tracking-label) text-amber-700 dark:text-amber-300">
            {t("approvalpayload.general.recommendedaction")}</p>
          <MarkdownBody className="mt-1 leading-6 text-foreground">{recommendedAction}</MarkdownBody>
        </div>
      )}
      {nextActionOnApproval && (
        <div className="rounded-lg border border-border/60 bg-background/60 px-3.5 py-3">
          <p className="text-(length:--text-micro) font-medium uppercase tracking-(--tracking-label) text-muted-foreground">{t("approvalpayload.general.onapproval")}</p>
          <MarkdownBody className="mt-1 leading-6 text-foreground">{nextActionOnApproval}</MarkdownBody>
        </div>
      )}
      {risks.length > 0 && (
        <div className="space-y-1.5">
          <p className="text-(length:--text-micro) font-medium uppercase tracking-(--tracking-label) text-muted-foreground">{t("approvalpayload.general.risks")}</p>
      ...
      {proposedComment && (
        <div className="space-y-1.5">
          <p className="text-(length:--text-micro) font-medium uppercase tracking-(--tracking-label) text-muted-foreground">
            {t("approvalpayload.general.proposedcomment")}</p>
