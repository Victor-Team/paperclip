  ...
  override render() {
    ...
    return (
      <div className="mx-auto max-w-2xl space-y-4 px-4 py-10">
        <div>
          <h1 className="text-lg font-semibold">{t("routeerrorboundary.general.thispagehitanerror")}</h1>
          <p className="mt-1 text-sm text-muted-foreground">
            {t("routeerrorboundary.general.somethingwentwrongwhilerendering")}
          </p>
        </div>
        ...
        <div className="flex items-center gap-2">
          <Button variant="outline" size="sm" onClick={this.props.onReset}>
            {t("routeerrorboundary.general.goback")}
          </Button>
          <Button size="sm" onClick={() => window.location.reload()}>
            {t("routeerrorboundary.general.reloadpage")}
          </Button>
        </div>
      </div>
    );
