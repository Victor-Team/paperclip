...
export function ProjectRepositories({ project }: { project: Project }) {
  ...
  return <section aria-label={t("projectrepositories.general.repositories")} className="flex min-w-0 flex-col gap-4 py-4">
    ...
    <div className="flex flex-wrap items-center justify-end gap-2 border-t border-border pt-4">
      ...
      {save.isSuccess && <span role="status" className="mr-auto text-sm text-muted-foreground">{t("projectrepositories.general.changessaved")}</span>}
      {draft && <Button type="button" variant="ghost" disabled={save.isPending} onClick={() => { setDraft(null); save.reset(); }}>{t("projectrepositories.general.discardchanges")}</Button>}
      <Button type="button" disabled={!draft || save.isPending} onClick={() => save.mutate()}>{save.isPending ? t("projectrepositories.general.saving") : t("projectrepositories.general.savechanges")}</Button>
    </div>
    <Dialog open={connecting} onOpenChange={setConnecting}><DialogContent showCloseButton={false} aria-describedby={undefined} className="max-h-(--sz-calc-18) overflow-y-auto sm:max-w-2xl">
      <DialogTitle className="sr-only">{t("projectrepositories.general.connectgithub")}</DialogTitle>
