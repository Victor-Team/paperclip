      ...
      <DialogContent className="max-h-(--sz-85vh) overflow-y-auto sm:max-w-2xl">
        <DialogHeader>
          ...
          <DialogDescription className="break-words">
            {t("executionworkspaceclosedialog.general.archive")}<span className="font-medium text-foreground">{workspaceName}</span> {t("executionworkspaceclosedialog.general.andcleanupanyownedworkspaceartifacts")}</DialogDescription>
        </DialogHeader>
        ...
        {readinessQuery.isLoading ? (
          <div className="flex items-center gap-2 rounded-xl border border-border bg-background px-4 py-3 text-sm text-muted-foreground">
            ...
            {t("executionworkspaceclosedialog.general.checkingwhetherthisworkspaceissafeto")}</div>
        ) : readinessQuery.error ? (
          <div className="rounded-xl border border-destructive/30 bg-destructive/5 px-4 py-3 text-sm text-destructive">
            {readinessQuery.error instanceof Error ? readinessQuery.error.message : t("executionworkspaceclosedialog.general.failedtoinspectworkspaceclosereadiness")}
          </div>
        ) : readiness ? (
          <div className="space-y-4">
            <div className={`rounded-xl border px-4 py-3 text-sm ${readinessTone(readiness.state)}`}>
              <div className="font-medium">
                {readiness.state === "blocked"
                  ? t("executionworkspaceclosedialog.general.closeisblocked")
                  : readiness.state === "ready_with_warnings"
                    ? t("executionworkspaceclosedialog.general.closeisallowedwithwarnings")
                    : t("executionworkspaceclosedialog.general.closeisready")}
              </div>
              <div className="mt-1 text-xs opacity-80">
                {readiness.isSharedWorkspace
                  ? t("executionworkspaceclosedialog.general.thisisasharedworkspacesessionarchiving")
                  : readiness.git?.workspacePath && readiness.git.repoRoot && readiness.git.workspacePath !== readiness.git.repoRoot
                    ? t("executionworkspaceclosedialog.general.thisexecutionworkspacehasitsowncheckout")
                    : readiness.isProjectPrimaryWorkspace
                      ? t("executionworkspaceclosedialog.general.thisexecutionworkspacecurrentlypointsatthe")
                      : t("executionworkspaceclosedialog.general.thisworkspaceisdisposableandcanbe")}
              </div>
            </div>

            {blockingIssues.length > 0 ? (
              <section className="space-y-2">
                <h3 className="text-sm font-medium">{t("executionworkspaceclosedialog.general.blockingtasks")}</h3>
            ...
            {readiness.blockingReasons.length > 0 ? (
              <section className="space-y-2">
                <h3 className="text-sm font-medium">{t("executionworkspaceclosedialog.general.blockingreasons")}</h3>
            ...
            {readiness.warnings.length > 0 ? (
              <section className="space-y-2">
                <h3 className="text-sm font-medium">{t("executionworkspaceclosedialog.general.warnings")}</h3>
                <ul className="space-y-2 text-sm text-muted-foreground">
            ...
            {readiness.git ? (
              <section className="space-y-2">
                <h3 className="text-sm font-medium">{t("executionworkspaceclosedialog.general.gitstatus")}</h3>
                <div className="rounded-xl border border-border bg-background px-4 py-3 text-sm">
                  <div className="grid gap-2 sm:grid-cols-2">
                    <div>
                      <div className="text-xs uppercase tracking-(--tracking-eyebrow) text-muted-foreground">{t("executionworkspaceclosedialog.general.branch")}</div>
                      <div className="font-mono text-xs">{readiness.git.branchName ?? t("executionworkspaceclosedialog.general.unknown")}</div>
                    </div>
                    <div>
                      <div className="text-xs uppercase tracking-(--tracking-eyebrow) text-muted-foreground">{t("executionworkspaceclosedialog.general.baseref")}</div>
                      <div className="font-mono text-xs">{readiness.git.baseRef ?? t("executionworkspaceclosedialog.general.notset")}</div>
                    </div>
                    <div>
                      <div className="text-xs uppercase tracking-(--tracking-eyebrow) text-muted-foreground">{t("executionworkspaceclosedialog.general.mergedintobase")}</div>
                      <div>{readiness.git.isMergedIntoBase == null ? t("executionworkspaceclosedialog.general.unknown1") : readiness.git.isMergedIntoBase ? t("executionworkspaceclosedialog.general.yes") : t("executionworkspaceclosedialog.general.no")}</div>
                    </div>
                    <div>
                      <div className="text-xs uppercase tracking-(--tracking-eyebrow) text-muted-foreground">{t("executionworkspaceclosedialog.general.aheadbehind")}</div>
                      <div>
                        {(readiness.git.aheadCount ?? 0).toString()} / {(readiness.git.behindCount ?? 0).toString()}
                      </div>
                    </div>
                    <div>
                      <div className="text-xs uppercase tracking-(--tracking-eyebrow) text-muted-foreground">{t("executionworkspaceclosedialog.general.dirtytrackedfiles")}</div>
                      <div>{readiness.git.dirtyEntryCount}</div>
                    </div>
                    <div>
                      <div className="text-xs uppercase tracking-(--tracking-eyebrow) text-muted-foreground">{t("executionworkspaceclosedialog.general.untrackedfiles")}</div>
            ...
            {otherLinkedIssues.length > 0 ? (
              <section className="space-y-2">
                <h3 className="text-sm font-medium">{t("executionworkspaceclosedialog.general.otherlinkedtasks")}</h3>
                <div className="space-y-2">
            ...
            {readiness.runtimeServices.length > 0 ? (
              <section className="space-y-2">
                <h3 className="text-sm font-medium">{t("executionworkspaceclosedialog.general.attachedruntimeservices")}</h3>
                <div className="space-y-2">
            ...
            <section className="space-y-2">
              <h3 className="text-sm font-medium">{t("executionworkspaceclosedialog.general.cleanupactions")}</h3>
              <div className="space-y-2">
            ...
            {currentStatus === "cleanup_failed" ? (
              <div className="rounded-xl border border-amber-500/20 bg-amber-500/5 px-4 py-3 text-sm text-muted-foreground">
                {t("executionworkspaceclosedialog.general.cleanuppreviouslyfailedonthisworkspaceretrying")}</div>
            ) : null}

            {currentStatus === "archived" ? (
              <div className="rounded-xl border border-border bg-background px-4 py-3 text-sm text-muted-foreground">
                {t("executionworkspaceclosedialog.general.thisworkspaceisalreadyarchived")}</div>
            ) : null}

            {readiness.git?.repoRoot ? (
              <div className="break-words text-xs text-muted-foreground">
                {t("executionworkspaceclosedialog.general.reporoot")}<span className="font-mono break-all">{readiness.git.repoRoot}</span>
                {readiness.git.workspacePath ? (
                  <>
                    {" · "}{t("executionworkspaceclosedialog.general.workspacepath")}<span className="font-mono break-all">{readiness.git.workspacePath}</span>
                  </>
                ) : null}
              </div>
            ) : null}
            ...
            <div className="text-xs text-muted-foreground">
              {t("executionworkspaceclosedialog.general.lastchecked")} {formatDateTime(new Date(readinessQuery.dataUpdatedAt))}
            </div>
          </div>
        ) : null}
        ...
        <DialogFooter>
          ...
          >
            {t("executionworkspaceclosedialog.general.cancel")}</Button>
          <Button
