...
export function ProjectWorkspacesContent({
  ...
  if (summaries.length === 0) {
    return <p className="text-sm text-muted-foreground">{t("projectworkspacescontent.general.nonondefaultworkspaceactivityyet")}</p>;
  }
        ...
        {cleanupFailedSummaries.length > 0 ? (
          <div className="space-y-2">
            <div className="text-xs font-medium uppercase tracking-(--tracking-caps) text-muted-foreground">
              {t("projectworkspacescontent.general.cleanupattentionneeded")}</div>
            <div className="space-y-3">
