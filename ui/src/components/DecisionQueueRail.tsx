...
export function DecisionQueueRail({ companyId, activeQueueKey = null }: DecisionQueueRailProps) {
  ...
  return (
    <nav className="flex flex-wrap items-center gap-1.5" aria-label={t("decisionqueuerail.general.decisionqueues")} data-decision-queue-rail>
      <Chip href={decisionsHref(null)} active={activeQueueKey == null} label={t("decisionqueuerail.general.all")} />
      {queues.map((queue) => {
...
function Chip({
  ...
  return (
    ...
    >
      {recent && (
        <span className="h-1.5 w-1.5 shrink-0 rounded-full bg-primary" aria-label={t("decisionqueuerail.general.recentactivity")} />
      )}
