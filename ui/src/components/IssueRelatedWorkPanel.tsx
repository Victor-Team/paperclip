...
function ExternalObjectsSection({
  ...
  return (
    <section className="space-y-3 rounded-lg border border-border p-3">
      <div className="space-y-1">
        <h3 className="text-sm font-semibold">{t("issuerelatedworkpanel.general.externalobjects")}</h3>
        <p className="text-xs text-muted-foreground">
          {t("issuerelatedworkpanel.general.remoteworkreferencedfromthisissuepull")}</p>
      </div>

      {isError ? (
        <p className="text-xs text-muted-foreground">
          {t("issuerelatedworkpanel.general.couldntloadexternalobjects")}{" "}
          {onRetry ? (
            <button
              type="button"
              onClick={onRetry}
            ...
            >
              {t("issuerelatedworkpanel.general.retry")}</button>
          ) : null}
        </p>
      ) : isLoading ? (
        <p className="text-xs text-muted-foreground">{t("issuerelatedworkpanel.general.loadingexternalobjects")}</p>
      ) : sorted.length === 0 ? (
        <p className="text-xs text-muted-foreground">
          {t("issuerelatedworkpanel.general.thisissuedoesnotreferenceanyexternal")}</p>
      ) : (
        <ul className="-mx-1 flex flex-col">
...
export function IssueRelatedWorkPanel({
  ...
  return (
    <div className="space-y-3">
      <Section
        title={t("issuerelatedworkpanel.general.references")}
      ...
      <Section
        title={t("issuerelatedworkpanel.general.referencedby")}
