  ...
  return (
    <DropdownMenu open={open} onOpenChange={setOpen}>
        ...
        >
          <div className="flex items-center gap-2 min-w-0">
            ...
            <span className="text-sm font-medium truncate">
              {selectedCompany?.name ?? t("companyswitcher.general.selectOrganization")}
            </span>
          </div>
      ...
      <DropdownMenuContent align="start" className="w-(--sz-220px)">
        <DropdownMenuLabel>{t("companyswitcher.general.organizations")}</DropdownMenuLabel>
        <DropdownMenuSeparator />
        ...
        {sidebarCompanies.length === 0 && (
          ...
          companyListUnavailable ? (
            <>
              <DropdownMenuItem disabled>{t("companyswitcher.general.couldnTLoadOrganizations")}</DropdownMenuItem>
              <DropdownMenuItem
                onSelect={(event) => {
              ...
              >
                ...
                {t("companyswitcher.general.tryAgain")}
              </DropdownMenuItem>
            </>
          ) : (
            <DropdownMenuItem disabled>{t("companyswitcher.general.noOrganizations")}</DropdownMenuItem>
          )
        )}
        <DropdownMenuSeparator />
        <DropdownMenuItem asChild>
          <Link to="/company/settings" className="no-underline text-inherit">
            ...
            {t("companyswitcher.general.settings")}
          </Link>
        </DropdownMenuItem>
        <DropdownMenuItem asChild>
          <Link to="/companies" className="no-underline text-inherit">
            <Plus className="h-4 w-4 mr-2" />
            {t("companyswitcher.general.manageOrganizations")}
          </Link>
        </DropdownMenuItem>
      </DropdownMenuContent>
    </DropdownMenu>
