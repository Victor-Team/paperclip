...
export function InboxAgentPolicyControl({ companyId }: { companyId: string | null | undefined }) {
  ...
  const modeOptions = useMemo<RadioCardOption[]>(
    () => [
      {
        value: "open",
        title: t("inboxagentpolicycontrol.general.anyOfMyAgents"),
        description: t("inboxagentpolicycontrol.general.letAnyAgentYou"),
      },
      {
        value: "allowlist",
        title: t("inboxagentpolicycontrol.general.onlyChosenAgents"),
        description: t("inboxagentpolicycontrol.general.restrictInboxTidyingTo"),
      },
      {
        value: "disabled",
        title: t("inboxagentpolicycontrol.general.off"),
        description: t("inboxagentpolicycontrol.general.agentsCanNeverArchive"),
      },
    ],
    [t],
  );
  ...
  if (policyQuery.error) {
    return (
      <div className="text-sm text-destructive">
        {policyQuery.error instanceof Error ? policyQuery.error.message : t("inboxagentpolicycontrol.general.failedToLoadInbox")}
      </div>
    );
  }

  if (policyQuery.isLoading || !draft) {
    return <div className="text-sm text-muted-foreground">{t("inboxagentpolicycontrol.general.loadingInboxAgentPolicy")}</div>;
  }

  return (
    <section className="space-y-4" aria-label={t("inboxagentpolicycontrol.general.letAgentsTidyMy")}>
      <div className="space-y-1">
        <div className="flex items-center gap-2">
          <Inbox className="h-5 w-5 text-muted-foreground" />
          <h2 className="text-base font-semibold">{t("inboxagentpolicycontrol.general.letAgentsTidyMy")}</h2>
        </div>
        <p className="max-w-2xl text-sm text-muted-foreground">
          {t("inboxagentpolicycontrol.general.chooseWhetherTheAgents")}
        </p>
      </div>

      <RadioCardGroup
        ariaLabel={t("inboxagentpolicycontrol.general.inboxAgentArchivingPolicy")}
        value={draft.mode}
      ...
      {draft.mode === "allowlist" ? (
        <div className="max-w-2xl space-y-2">
          <div className="text-sm font-medium">{t("inboxagentpolicycontrol.general.agentsAllowedToTidy")}</div>
          <AgentMultiSelect
            agents={agentOptions}
            ...
            triggerLabel={
              selectedAgentIds.size === 0
                ? t("inboxagentpolicycontrol.general.selectAgents")
                : selectedAgentIds.size === 1
                  ? t("inboxagentpolicycontrol.general.agentSelected", { count: selectedAgentIds.size })
                  : t("inboxagentpolicycontrol.general.agentsSelected", { count: selectedAgentIds.size })
            }
            triggerFullWidth={false}
            showSelectionPreview={false}
            emptyMessage={t("inboxagentpolicycontrol.general.youDonTManageAny")}
          />
        </div>
      ) : null}
      ...
      {updateMutation.error ? (
        <div className="max-w-2xl rounded-md border border-destructive/40 bg-destructive/5 px-3 py-2 text-sm text-destructive">
          {updateMutation.error instanceof Error ? updateMutation.error.message : t("inboxagentpolicycontrol.general.failedToSaveInbox")}
        </div>
      ) : null}

      <div className="flex max-w-2xl items-center justify-end gap-3">
        {updateMutation.isSuccess && !isDirty ? (
          <span className="text-xs text-muted-foreground" role="status">{t("inboxagentpolicycontrol.general.saved")}</span>
        ) : null}
        <Button
          type="button"
        ...
        >
          ...
          {updateMutation.isPending ? t("inboxagentpolicycontrol.general.saving") : t("common.actions.save")}
        </Button>
      </div>
    </section>
  );
