...
export function CompanySettingsSidebar() {
  ...
  return (
    <ContextualSidebarFrame
      surface="settings"
      title={t("companysettingssidebar.general.settings")}
      showHeader={false}
    ...
    >
      ...
      >
        <div data-slot="settings-back-group" className={`${primarySidebarStyles.group} w-full`}>
          <SidebarNavItem to="/dashboard" label={t("companysettingssidebar.general.backToApp")} icon={ArrowLeft} />
        </div>
      </div>
      <nav
        aria-label={t("companysettingssidebar.general.settings")}
        data-slot="contextual-sidebar-nav"
        className={primarySidebarStyles.nav}
      >
        <div data-slot="contextual-sidebar-group" className={primarySidebarStyles.group}>
          <SidebarNavItem to="/company/settings" label={t("companysettingssidebar.general.general")} icon={SlidersHorizontal} end />
          {showPage("instance.profile") && (
            <SidebarNavItem
              to={`${INSTANCE_SETTINGS_PATH_PREFIX}/profile`}
              label={t("companysettingssidebar.general.profile")}
              icon={UserRoundPen}
              end
            />
          )}
          {showPage("company.members") && (
            <SidebarNavItem
              ...
              label={t("companysettingssidebar.general.members")}
              icon={Users}
          ...
          {showPage("company.secrets") && (
            <SidebarNavItem to="/company/settings/secrets" label={t("companysettingssidebar.general.secrets")} icon={KeyRound} end />
          )}
          {showPage("instance.environments") && (
            <SidebarNavItem
              to={`${INSTANCE_SETTINGS_PATH_PREFIX}/environments`}
              label={t("companysettingssidebar.general.environments")}
              icon={MonitorCog}
              end
            />
          )}
          {showPage("instance.access") && (
            <SidebarNavItem
              ...
              label={t("companysettingssidebar.general.access")}
              icon={Shield}
              end
            />
          )}
          {showPage("company.export") && (
            <SidebarNavItem to="/company/export" label={t("companysettingssidebar.general.export")} icon={Download} />
          )}
          {!isCloud && showPage("company.import") && (
            <SidebarNavItem to="/company/import" label={t("companysettingssidebar.general.import")} icon={Upload} end />
          )}
          {showPage("instance.experimental") && (
            <SidebarNavItem
              to={`${INSTANCE_SETTINGS_PATH_PREFIX}/experimental`}
              label={t("companysettingssidebar.general.experimental")}
              icon={FlaskConical}
            />
          )}
          {showPlugins && (
            <SidebarNavItem
              ...
              label={t("companysettingssidebar.general.plugins")}
              icon={Puzzle}
            />
          )}
          ...
          {showPage("instance.adapters") && (
            <SidebarNavItem
              ...
              label={t("companysettingssidebar.general.adapters")}
              icon={Cpu}
            />
          )}
        </div>
