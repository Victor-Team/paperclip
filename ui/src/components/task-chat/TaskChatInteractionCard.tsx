      ...
      <div id={`interaction-${interaction.id}`} data-testid="task-chat-tool-review" className="w-full space-y-2">
        {interaction.status === "pending" ? (
          <div className="flex flex-wrap items-center gap-2 text-sm">
            ...
            <Button className="ml-auto" size="sm" variant="outline" disabled={!onReviewRequest} onClick={() => onReviewRequest?.(interaction.id)}>{t("taskchatinteractioncard.general.reviewrequest")}</Button>
          </div>
        ) : (
