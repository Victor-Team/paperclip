      ...
      >
        <div className="flex max-w-(--pct-85) items-center gap-1.5">
          ...
          {onTryAgain && !open ? (
            ...
            >
              {tryAgainPending ? t("taskchatmarker.general.tryingagain") : t("taskchatmarker.general.tryagain")}
            </Button>
          ) : null}
        </div>
        {open ? (
              ...
              <div className="flex items-center justify-end gap-2 border-t border-border/70 bg-background/50 px-3 py-2 dark:bg-background/30">
                {item.runHref ? (
                  <Button asChild variant="ghost" size="xs">
                    <Link to={item.runHref}>{t("taskchatmarker.general.viewrun")}</Link>
                  </Button>
                ) : null}
                {onTryAgain ? (
                  <Button
                    ...
                    data-testid="task-chat-run-failed-try-again"
                  >
                    {tryAgainPending ? t("taskchatmarker.general.tryingagain1") : t("taskchatmarker.general.tryagain2")}
                  </Button>
                ) : null}
              </div>
            ) : null}
      ...
      >
        ...
        {onTryAgain ? (
          <Button
            ...
            data-testid="task-chat-run-failed-try-again"
          >
            {tryAgainPending ? t("taskchatmarker.general.tryingagain3") : t("taskchatmarker.general.tryagain4")}
          </Button>
        ) : null}
      </span>
      <span
