...
export function SidebarNavItem({
  ...
  const railStatusText = hasLive
    ? t("sidebarnavitem.general.live", { liveCount })
    : hasBadge
      ? `${badge}${badgeLabel ? ` ${badgeLabel}` : ""}`
      : alert
        ? t("sidebarnavitem.general.attentionNeeded")
        : undefined;
        ...
        <span className="ml-auto flex items-center gap-1.5">
          ...
          {hasLive && (
            <>
              ...
              <span className="text-(length:--text-micro) font-medium text-blue-600 dark:text-blue-400">{t("sidebarnavitem.general.live", { liveCount })}</span>
            </>
          )}
        </span>
      )}
