  ...
  return (
    <div className="relative">
      ...
      <Input
        ...
        placeholder={t("issuefilterspopover.general.searchPlaceholder", { label: normalizedLabel })}
        aria-label={t("issuefilterspopover.general.searchAriaLabel", { label: normalizedLabel })}
...
export function IssueFiltersPopover({
  ...
  const inboxCategoryOptions: ReadonlyArray<[InboxCategoryFilter, string]> = [
    ["everything", t("issuefilterspopover.inboxCategory.allCategories")],
    ["issues_i_touched", t("issuefilterspopover.inboxCategory.myRecentTasks")],
    ["join_requests", t("issuefilterspopover.inboxCategory.joinRequests")],
    ["approvals", t("issuefilterspopover.inboxCategory.approvals")],
    ["failed_runs", t("issuefilterspopover.inboxCategory.failedRuns")],
    ["alerts", t("issuefilterspopover.inboxCategory.alerts")],
  ];
  const inboxApprovalStatusOptions: ReadonlyArray<[InboxApprovalFilter, string]> = [
    ["all", t("issuefilterspopover.inboxApproval.allApprovalStatuses")],
    ["actionable", t("issuefilterspopover.inboxApproval.needsAction")],
    ["resolved", t("issuefilterspopover.inboxApproval.resolved")],
  ];
  const statusLabels: Record<string, string> = {
    todo: t("issuefilterspopover.status.todo"),
    in_progress: t("issuefilterspopover.status.inProgress"),
    in_review: t("issuefilterspopover.status.inReview"),
    blocked: t("issuefilterspopover.status.blocked"),
    done: t("issuefilterspopover.status.done"),
    cancelled: t("issuefilterspopover.status.cancelled"),
    backlog: t("issuefilterspopover.status.backlog"),
  };
  const priorityLabels: Record<string, string> = {
    critical: t("issuefilterspopover.priority.critical"),
    high: t("issuefilterspopover.priority.high"),
    medium: t("issuefilterspopover.priority.medium"),
    low: t("issuefilterspopover.priority.low"),
  };
  const externalObjectLabels: Record<string, string> = {
    failed: t("issuefilterspopover.externalObject.anyFailed"),
    waiting: t("issuefilterspopover.externalObject.anyWaiting"),
    running: t("issuefilterspopover.externalObject.anyRunning"),
    auth_required: t("issuefilterspopover.externalObject.authRequired"),
    unreachable: t("issuefilterspopover.externalObject.unreachable"),
    stale: t("issuefilterspopover.externalObject.stale"),
    none: t("issuefilterspopover.externalObject.noExternalObjects"),
  };
  const quickFilterLabels: Record<string, string> = {
    All: t("issuefilterspopover.quickFilters.all"),
    Active: t("issuefilterspopover.quickFilters.active"),
    Backlog: t("issuefilterspopover.quickFilters.backlog"),
    Done: t("issuefilterspopover.quickFilters.done"),
  };
  ...
  return (
    <Popover>
      <PopoverTrigger asChild>
        <Button variant={buttonVariant} size={iconOnly ? "icon" : "sm"} className={`text-xs ${iconOnly ? "relative h-8 w-8 shrink-0" : ""} ${activeFilterCount > 0 ? "text-blue-600 dark:text-blue-400" : ""}`} title={iconOnly ? (activeFilterCount > 0 ? t("issuefilterspopover.general.filterButtonLabelWithCount", { count: activeFilterCount }) : t("issuefilterspopover.general.filterButtonLabel")) : undefined}>
          <Filter className={iconOnly ? "h-3.5 w-3.5" : "h-3.5 w-3.5 sm:h-3 sm:w-3 sm:mr-1"} />
          {!iconOnly && <span className="hidden sm:inline">{activeFilterCount > 0 ? t("issuefilterspopover.general.filterButtonLabelWithCount", { count: activeFilterCount }) : t("issuefilterspopover.general.filterButtonLabel")}</span>}
      ...
      >
        <div className="space-y-3 p-3">
          <div className="flex items-center justify-between">
            <span className="text-sm font-medium">{t("issuefilterspopover.general.filters")}</span>
            {activeFilterCount > 0 ? (
              <button
                type="button"
              ...
              >
                {t("issuefilterspopover.general.clear")}
              </button>
            ) : null}
          </div>
            ...
            <>
              <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
                <div className="space-y-1" data-filter-options="inbox-category">
                  <span className="text-xs text-muted-foreground">{t("issuefilterspopover.general.category")}</span>
                  <div className="space-y-0.5">
                    {inboxCategoryOptions.map(([value, label]) => {
                ...
                {inboxScopeFilters.showApprovalStatus ? (
                  <div className="space-y-1" data-filter-options="inbox-approval-status">
                    <span className="text-xs text-muted-foreground">{t("issuefilterspopover.general.approvalStatus")}</span>
                    <div className="space-y-0.5">
                      {inboxApprovalStatusOptions.map(([value, label]) => {
          ...
          <div className="space-y-1.5">
            <span className="text-xs text-muted-foreground">{t("issuefilterspopover.general.quickFilters")}</span>
            <div className="flex flex-wrap gap-1.5">
              {issueQuickFilterPresets.map((preset) => {
                ...
                return (
                  ...
                  >
                    {quickFilterLabels[preset.label] ?? preset.label}
                  </button>
                );
              })}
            </div>
          ...
          <div className="grid grid-cols-1 gap-4 md:grid-cols-3">
            <div className="min-w-0 space-y-3">
              <div className="space-y-1">
                <span className="text-xs text-muted-foreground">{t("issuefilterspopover.general.status")}</span>
                <div className="space-y-0.5">
              ...
              <div className="space-y-1">
                <span className="text-xs text-muted-foreground">{t("issuefilterspopover.general.priority")}</span>
                <div className="space-y-0.5">
                  {issuePriorityOrder.map((priority) => (
                    <label key={priority} className="flex cursor-pointer items-center gap-2 rounded-sm px-2 py-1 hover:bg-accent/50">
                      ...
                      <span className="text-sm">{priorityLabels[priority] ?? priority}</span>
                    </label>
                  ))}
                </div>
              </div>
            ...
            <div className="min-w-0 space-y-3">
              <div className="space-y-1">
                <span className="text-xs text-muted-foreground">{t("issuefilterspopover.general.responsible")}</span>
                {streamlined && (agents?.length ?? 0) + (currentUserId ? 2 : 1) > SEARCHABLE_FILTER_THRESHOLD ? (
                  <FilterOptionSearch value={assigneeSearch} onChange={setAssigneeSearch} label={t("issuefilterspopover.general.labelResponsible")} />
                ) : null}
                <div data-filter-options="responsible" className={streamlined ? "space-y-0.5" : "max-h-32 space-y-0.5 overflow-y-auto"}>
                  <label className="flex cursor-pointer items-center gap-2 rounded-sm px-2 py-1 hover:bg-accent/50">
                    ...
                    />
                    <span className="text-sm">{t("issuefilterspopover.general.noResponsible")}</span>
                  </label>
                  {currentUserId ? (
                    <label className="flex cursor-pointer items-center gap-2 rounded-sm px-2 py-1 hover:bg-accent/50">
                      <Checkbox
                      ...
                      <span className="text-sm">{t("issuefilterspopover.general.me")}</span>
                    </label>
                  ) : null}
              ...
              {creatorOptions.length > 0 ? (
                <div className="space-y-1">
                  <span className="text-xs text-muted-foreground">{t("issuefilterspopover.general.creator")}</span>
                      ...
                      {selectedCreatorOptions.map((creator) => (
                        <Badge key={creator.id} variant="secondary" className="gap-1 pr-1">
                          ...
                          <button
                            ...
                            aria-label={t("issuefilterspopover.general.removeCreatorAriaLabel", { label: creator.label })}
                          >
                            <X className="h-3 w-3" />
                          </button>
                        </Badge>
                  ...
                  <div className="relative">
                    ...
                    <Input
                      ...
                      placeholder={t("issuefilterspopover.general.placeholderSearchcreators")}
                      className="h-8 pl-7 text-xs"
                    />
                  </div>
                  <div data-filter-options="creators" className={streamlined ? "space-y-0.5" : "max-h-32 space-y-0.5 overflow-y-auto"}>
                    {visibleCreatorOptions.length > 0 ? visibleCreatorOptions.map((creator) => {
                      ...
                      <div className="px-2 py-1 text-xs text-muted-foreground">{t("issuefilterspopover.general.noCreatorsMatch")}</div>
                    )}
                  </div>
                </div>
              ) : null}
              ...
              {projects && projects.length > 0 ? (
                <div className="space-y-1">
                  <span className="text-xs text-muted-foreground">{t("issuefilterspopover.general.project")}</span>
                  {streamlined && projects.length > SEARCHABLE_FILTER_THRESHOLD ? (
                    <FilterOptionSearch value={projectSearch} onChange={setProjectSearch} label={t("issuefilterspopover.general.labelProjects")} />
                  ) : null}
            ...
            <div className="min-w-0 space-y-3">
              {labels && labels.length > 0 ? (
                <div className="space-y-1">
                  <span className="text-xs text-muted-foreground">{t("issuefilterspopover.general.labels")}</span>
                  {streamlined && labels.length > SEARCHABLE_FILTER_THRESHOLD ? (
                    <FilterOptionSearch value={labelSearch} onChange={setLabelSearch} label={t("issuefilterspopover.general.labelLabels")} />
                  ) : null}
              ...
              {workspaces && workspaces.length > 0 ? (
                <div className="space-y-1">
                  <span className="text-xs text-muted-foreground">{t("issuefilterspopover.general.workspace")}</span>
                  {streamlined && workspaces.length > SEARCHABLE_FILTER_THRESHOLD ? (
                    <FilterOptionSearch value={workspaceSearch} onChange={setWorkspaceSearch} label={t("issuefilterspopover.general.labelWorkspaces")} />
                  ) : null}
              ...
              {enableExternalObjectFilters ? (
                <div className="space-y-1">
                  <span className="text-xs text-muted-foreground">{t("issuefilterspopover.general.externalObjectStatus")}</span>
                  <div className="space-y-0.5">
                    {externalObjectFilterOrder.map((value) => {
                      ...
                      return (
                        ...
                        >
                          ...
                          <span className="text-sm">{externalObjectLabels[value] ?? value}</span>
                        </label>
                      );
                    })}
                  </div>
              ...
              <div className="space-y-1">
                <span className="text-xs text-muted-foreground">{t("issuefilterspopover.general.visibility")}</span>
                <label className="flex cursor-pointer items-center gap-2 rounded-sm px-2 py-1 hover:bg-accent/50">
                  <Checkbox
                    checked={state.liveOnly}
                    onCheckedChange={(checked) => onChange({ liveOnly: checked === true })}
                  />
                  <span className="text-sm">{t("issuefilterspopover.general.liveRunsOnly")}</span>
                </label>
                {enableRoutineVisibilityFilter ? (
                  <label className="flex cursor-pointer items-center gap-2 rounded-sm px-2 py-1 hover:bg-accent/50">
                    ...
                    />
                    <span className="text-sm">{t("issuefilterspopover.general.hideRoutineRuns")}</span>
                  </label>
                ) : null}
              </div>
            </div>
