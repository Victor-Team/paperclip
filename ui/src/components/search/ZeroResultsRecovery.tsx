  ...
  return (
    ...
    >
      ...
      <div className="space-y-1">
        <div className="text-base font-semibold">{t("zeroresultsrecovery.general.noresultswiththesefilters")}</div>
        <p className="text-sm text-muted-foreground">
          {query
            ? t("zeroresultsrecovery.general.matchsummarywithquery", {
              count: unfilteredTotal,
              query,
              filters: activeCount,
            })
            : t("zeroresultsrecovery.general.matchsummary", { count: unfilteredTotal, filters: activeCount })}
        </p>
      </div>
      ...
      {suggestions.length > 0 ? (
        <div className="flex w-full flex-col gap-1.5">
          <div className="text-(length:--text-micro) font-semibold uppercase tracking-wide text-muted-foreground">
            {t("zeroresultsrecovery.general.loosenafilter")}
          </div>
          {suggestions.map((suggestion) => (
            ...
            >
              <span className="min-w-0 truncate">
                {t("zeroresultsrecovery.general.remove")} {" "}
              ...
              <span className="shrink-0 tabular-nums text-emerald-600 dark:text-emerald-400">
                +{t(
                  suggestion.additionalCount === 1
                    ? "zeroresultsrecovery.general.oneresult"
                    : "zeroresultsrecovery.general.resultcount",
                  { count: suggestion.additionalCount },
                )}
              </span>
            </button>
          ))}
      ...
      <Button onClick={onClearAll} variant="default" size="sm">
        ...
        {t("zeroresultsrecovery.general.clearallfilters")}
      </Button>
    </div>
  );
}
