...
export function SearchFilterMenu(props: SearchFilterMenuProps) {
  ...
  const {
    ...
    selected,
    searchable = false,
    searchPlaceholder = t("searchfiltermenu.general.search"),
    emptyMessage = t("searchfiltermenu.general.nooptions"),
    triggerClassName,
    contentClassName,
    align = "start",
  } = props;
    ...
    <Popover open={open} onOpenChange={setOpen}>
      <PopoverTrigger asChild>
        <Button
          ...
          aria-label={t("searchfiltermenu.general.filterby", { filter: label })}
        >
      ...
      <PopoverContent align={align} className={cn("w-64 p-0", contentClassName)}>
        <div className="flex items-center justify-between px-3 py-2">
          ...
          {props.multi && active ? (
            ...
            >
              {t("searchfiltermenu.general.clear")}
            </button>
          ) : null}
        </div>
