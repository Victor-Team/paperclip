...
function runDayTooltip(t: TranslateFn, entry: DashboardRunActivityDay): string {
  const lines = [
    entry.total === 1
      ? t("activitycharts.general.tooltipRunHeaderSingular", { date: entry.date, count: entry.total })
      : t("activitycharts.general.tooltipRunHeaderPlural", { date: entry.date, count: entry.total }),
  ];
  if (entry.succeeded > 0) lines.push(t("activitycharts.general.tooltipSucceededLine", { count: entry.succeeded }));
  if (entry.recovered > 0) lines.push(t("activitycharts.general.tooltipRecoveredLine", { count: entry.recovered }));
  if (entry.failed > 0) {
    lines.push(t("activitycharts.general.tooltipFailedLine", { count: entry.failed }));
    // Error codes are technical identifiers, not natural-language text, so they stay untranslated.
    const codes = Object.entries(entry.failedByErrorCode ?? {}).sort((a, b) => b[1] - a[1]);
    for (const [code, count] of codes) lines.push(`    ${code}: ${count}`);
  }
  if (entry.other > 0) lines.push(t("activitycharts.general.tooltipOtherLine", { count: entry.other }));
  return lines.join("\n");
}
...
export function RunActivityChart(props: RunChartProps) {
  ...
  if (!hasData) return <p className="text-xs text-muted-foreground">{t("activitycharts.general.noRunsYet")}</p>;

  const legendItems = [
    { color: runSegmentColors.succeeded, label: t("activitycharts.general.succeeded") },
    ...(hasRecovered ? [{ color: runSegmentColors.recovered, label: t("activitycharts.general.recovered") }] : []),
    { color: runSegmentColors.failed, label: t("common.status.failed") },
    { color: runSegmentColors.other, label: t("activitycharts.general.other") },
  ];
    ...
    <div>
      <div className="flex items-end gap-(--sz-3px) h-20">
        {days.map(day => {
          ...
          return (
            <div key={day} className="flex-1 h-full flex flex-col justify-end" title={runDayTooltip(t, entry)}>
              {total > 0 ? (
...
export function PriorityChart({ issues }: { issues: { priority: string; createdAt: Date }[] }) {
  ...
  const priorityLabels: Record<string, string> = {
    critical: t("activitycharts.priority.critical"),
    high: t("activitycharts.priority.high"),
    medium: t("activitycharts.priority.medium"),
    low: t("activitycharts.priority.low"),
  };
  const days = getLast14Days();
  ...
  if (!hasData) return <p className="text-xs text-muted-foreground">{t("activitycharts.general.noTasks")}</p>;
    ...
    <div>
      <div className="flex items-end gap-(--sz-3px) h-20">
        {days.map(day => {
          ...
          const heightPct = (total / maxValue) * 100;
          return (
            <div key={day} className="flex-1 h-full flex flex-col justify-end" title={t("activitycharts.general.tooltipIssuesCount", { date: day, count: total })}>
              {total > 0 ? (
                <div className="flex flex-col-reverse gap-px overflow-hidden" style={{ height: `${heightPct}%`, minHeight: 2 }}>
...
export function IssueStatusChart({ issues }: { issues: { status: string; createdAt: Date }[] }) {
  ...
  const statusLabels: Record<string, string> = {
    ...
    in_review: t("activitycharts.status.inReview"),
    ...
    <div>
      <div className="flex items-end gap-(--sz-3px) h-20">
        {days.map(day => {
          ...
          const percent = entry.total > 0 ? Math.round(rate * 100) : 0;
          return (
            <div key={day} className="flex-1 h-full flex flex-col justify-end" title={t("activitycharts.general.tooltipSuccessRate", { date: day, percent, succeeded: effectiveSucceeded, total: entry.total })}>
              {entry.total > 0 ? (
