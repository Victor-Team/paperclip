...
function SidebarAgentItem({
  ...
  const pauseResumeLabel = isPaused
    ? t("sidebaragents.general.resumeAgent")
    : t("sidebaragents.general.pauseAgent");
  ...
  const pauseResumeDisabledLabel = disabled
    ? t("sidebaragents.general.updating")
    : isBudgetPaused
      ? t("sidebaragents.general.budgetPaused")
      : isPaused && hasInvalidOrgChain
        ? t("sidebaragents.general.invalidOrgChain")
      : pauseResumeLabel;
  ...
  const trailingLabel = [
    showBuiltInLifecycle
      ? builtInStatus === "pending_approval"
        ? t("sidebaragents.general.builtinAgentPendingApproval")
        : t("sidebaragents.general.builtinAgentNeedsSetup")
      : null,
    hasInvalidOrgChain ? t("sidebaragents.general.invalidReportingChain") : null,
    ...
    <SidebarNavItem
          ...
          <span className="ml-1 flex shrink-0 items-center gap-1">
            ...
            {hasInvalidOrgChain ? (
              <AlertTriangle
                ...
                aria-label={t("sidebaragents.general.invalidReportingChain")}
              />
            ) : null}
          </span>
        ) : undefined
      ...
      liveAccessory={
        agent.pauseReason === "budget" ? (
          <BudgetSidebarMarker title={t("sidebaragents.general.titleAgentpausedbybudget")} />
        ) : undefined
      }
    />
  );
      ...
      <DropdownMenu>
        <DropdownMenuTrigger asChild>
          <Button
            ...
            aria-label={t("sidebaragents.general.openActionsFor", { name: agent.name })}
          >
        ...
        <DropdownMenuContent align="end" className="w-48">
          {onToggleStar ? (
            <>
              ...
              >
                ...
                <span>
                  {starred
                    ? t("sidebaragents.general.removeFromStarred")
                    : t("sidebaragents.general.starAgent")}
                </span>
              </DropdownMenuItem>
              <DropdownMenuSeparator />
            </>
          ...
          <DropdownMenuItem asChild>
            ...
            >
              ...
              <span>{t("sidebaragents.general.editAgent")}</span>
            </Link>
          </DropdownMenuItem>
          <DropdownMenuSeparator />
          <DropdownMenuItem
            ...
            title={isBudgetPaused ? t("sidebaragents.general.wasPausedByBudgetLimits") : undefined}
          >
          ...
          >
            ...
            <span>
              {leaving ? t("sidebaragents.general.leaving") : t("sidebaragents.general.leaveAgent")}
            </span>
          </DropdownMenuItem>
        </DropdownMenuContent>
      </DropdownMenu>
...
export function SidebarAgents({ streamlined = false }: { streamlined?: boolean } = {}) {
  ...
  const pauseResumeAgent = useMutation({
    ...
    onSuccess: async (_agent, { agent, action }) => {
      ...
      pushToast({
        title: action === "pause"
          ? t("sidebaragents.general.agentPaused")
          : t("sidebaragents.general.agentResumed"),
        body: agent.name,
        tone: "success",
      });
    },
    onError: (error, { agent, action }) => {
      pushToast({
        title: action === "pause"
          ? t("sidebaragents.general.couldNotPauseAgent")
          : t("sidebaragents.general.couldNotResumeAgent"),
  ...
  const agentSortChoices: SidebarSectionRadioChoice[] = [
    { value: "top", label: t("sidebaragents.general.top") },
    { value: "alphabetical", label: t("sidebaragents.general.alphabetical") },
    { value: "recent", label: t("sidebaragents.general.recent") },
  ];

  return (
    <SidebarSection
      label={t("sidebaragents.general.labelAgents")}
      collapsible={{ open, onOpenChange: setOpen }}
      headerAction={{
        ariaLabel: t("sidebaragents.general.newAgent"),
        icon: Plus,
        onClick: openNewAgent,
      }}
      menu={{
        ariaLabel: t("sidebaragents.general.agentsSectionActions"),
        actions: [
          { type: "item", label: t("sidebaragents.general.browseAgents"), icon: Users, href: "/agents/all" },
          { type: "separator" },
        ],
        radioLabel: t("sidebaragents.general.agentSort"),
      ...
      {showSeeAllLink && (() => {
        ...
        const seeAllLink = (
          <Link
            ...
            aria-label={rail ? t("sidebaragents.general.seeAllAgents") : undefined}
            onClick={() => {
              if (isMobile) setSidebarOpen(false);
            }}
          ...
          >
            ...
            <span className={rail ? SIDEBAR_RAIL_HIDDEN_LABEL : undefined}>
              {t("sidebaragents.general.seeAllAgents")}
            </span>
          </Link>
        );
        return rail ? (
          <Tooltip>
            ...
            <TooltipContent side="right">{t("sidebaragents.general.seeAllAgents")}</TooltipContent>
          </Tooltip>
        ) : (
          seeAllLink
        );
