  ...
  return (
    <section className={cn("space-y-4", className)}>
      <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
        <div className="min-w-0 space-y-1">
          <div className="flex flex-wrap items-center gap-2">
            ...
            {isGenerating ? <Badge variant="secondary">{t("summaryslotcard.general.generating")}</Badge> : null}
            {displayingHistoricalRevision ? <Badge variant="outline">{t("summaryslotcard.general.historicalrevision")}</Badge> : null}
            {latestDocument && !displayingHistoricalRevision ? <Badge variant="outline">{t("summaryslotcard.general.latestrevision")}</Badge> : null}
          </div>
        ...
        <div className="flex shrink-0 flex-wrap items-center gap-2">
          {displayingHistoricalRevision ? (
            ...
            >
              {t("summaryslotcard.general.latest")}</Button>
          ) : null}
          {latestDocument && !generationFailed ? (
            ...
            >
              ...
              {t("summaryslotcard.general.refresh")}</Button>
          ) : null}
        </div>
      </div>
            ...
            <div className="flex items-start gap-3">
              ...
              <div className="space-y-1 text-sm">
                <p className="font-medium text-foreground">
                  {needsSetup.status === "pending_approval"
                    ? t("summaryslotcard.general.summarizersetupispendingapproval")
                    : t("summaryslotcard.general.setupthesummarizer")}
                </p>
                <p className="text-muted-foreground">
                  {t("summaryslotcard.general.summariesaregeneratedbypaperclipsbuilt")}</p>
              </div>
            </div>
            {needsSetup.status === "pending_approval" ? null : (
              <Button type="button" size="sm" onClick={() => setConfigureOpen(true)}>
                {t("summaryslotcard.general.setupsummarizer")}</Button>
            )}
          </div>
          {companyId ? (
      ...
      {!needsSetup && summarizerState?.status === "paused" && summarizerState.agent ? (
        <InlineBanner
          tone="warning"
          title={t("summaryslotcard.general.summarizerispaused")}
          actions={
            <Button
              type="button"
              size="sm"
        ...
        >
          {t("summaryslotcard.general.existingsummariesremainreadablebutnewsummaries")}</InlineBanner>
      ) : null}

      {actionError ? (
        <InlineBanner tone="warning" title={t("summaryslotcard.general.summaryrequestfailed")}>
          {actionError}
        </InlineBanner>
      ) : null}
      ...
      {slotQuery.isError ? (
        <InlineBanner
          tone="warning"
          title={t("summaryslotcard.general.summarycouldnotbeloaded")}
          actions={
            <Button type="button" size="sm" variant="outline" onClick={() => void slotQuery.refetch()}>
              {t("summaryslotcard.general.retry")}</Button>
          }
        >
          {slotQuery.error instanceof Error ? slotQuery.error.message : t("summaryslotcard.general.tryloadingthesummaryagain")}
        </InlineBanner>
      ) : null}
      ...
      {!slotQuery.isError && generationFailed ? (
        <InlineBanner
          tone="danger"
          title={t("summaryslotcard.general.summarygenerationfailed")}
          actions={
            <Button
              type="button"
              size="sm"
        ...
        >
          {slotQuery.data?.slot?.failureReason ?? t("summaryslotcard.general.thegenerationtaskendedbeforewritinga")}
        </InlineBanner>
      ) : null}
      ...
      {!slotQuery.isError && isGenerating && generatingIssue ? (
        <div className="flex items-start gap-3 text-sm">
          ...
          <div className="min-w-0 space-y-1">
            <p className="font-medium text-foreground">{t("summaryslotcard.general.generatingsummary")}</p>
            ...
            <p className="text-muted-foreground">
              {t("summaryslotcard.general.summarizerisworkingin")}{" "}
      ...
      {!slotQuery.isError && !latestDocument && !isGenerating && !generationFailed && canGenerateFirstSummary ? (
        <div className="flex flex-col items-start gap-3 rounded-lg border border-border bg-muted/30 p-4 sm:flex-row sm:items-center sm:justify-between">
          <div className="space-y-1 text-sm">
            <p className="font-medium text-foreground">{t("summaryslotcard.general.nosummaryyet")}</p>
            <p className="text-muted-foreground">{t("summaryslotcard.general.generateaconcisestatussnapshotforthis")}</p>
          </div>
          <Button
            type="button"
            size="sm"
          ...
          >
            ...
            {generateMutation.isPending ? t("summaryslotcard.general.generating1") : t("summaryslotcard.general.generatesummary")}
          </Button>
        </div>
      ) : null}
        ...
        <div className="space-y-4">
          ...
          <div className="flex flex-col gap-3 text-xs text-muted-foreground sm:flex-row sm:items-center sm:justify-between">
            <span title={formatDateTime(historicalRevision?.createdAt ?? latestRevision?.createdAt ?? latestDocument.updatedAt)}>
              {t("summaryslotcard.general.updated")} {relativeTime(historicalRevision?.createdAt ?? latestRevision?.createdAt ?? latestDocument.updatedAt)}
            </span>
            ...
            {revisions.length > 1 ? (
              ...
              >
                <SelectTrigger
                  ...
                  aria-label={t("summaryslotcard.general.selectsummaryrevision")}
                  title={historicalRevision ? revisionOptionLabel(historicalRevision) : latestSelectLabel}
                >
                  <SelectValue>
                    <History className="size-3.5" aria-hidden="true" />
                    <span>{revisions.length} {t("summaryslotcard.general.revisions")}</span>
                  </SelectValue>
                </SelectTrigger>
