  ...
  if (revisionsQuery.error) {
    return (
      <div className="rounded-md border border-l-2 border-l-destructive border-border p-4 space-y-3">
        <div>
          <p className="text-sm font-medium">{t("routinehistorytab.general.couldnotloadrevisions")}</p>
          <p className="text-xs text-muted-foreground">
            {revisionsQuery.error instanceof Error
              ? revisionsQuery.error.message
              : t("routinehistorytab.general.unknownerrorloadingrevisions")}
          </p>
        </div>
        <Button size="sm" variant="outline" onClick={() => revisionsQuery.refetch()}>
          {t("routinehistorytab.general.retry")}</Button>
      </div>
    );
  }
        ...
        {!isEditDirty && onlyBootstrapRevision ? (
          <div className="space-y-2">
            ...
            <p className="text-center text-xs text-muted-foreground">
              {t("routinehistorytab.general.revision1istheonlyhistorythis")}</p>
          </div>
        ) : (
          selectedRevision && (
            <>
      ...
      <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
        <div className="space-y-1">
          <p className="text-sm font-medium text-amber-800 dark:text-amber-200">
            {t("routinehistorytab.general.viewingrevision")} {revisionNumber} {t("routinehistorytab.general.readonly")}</p>
          <p className="text-xs text-muted-foreground">
            {t("routinehistorytab.general.restoringthisrevisioncreatesanewrevision")} {nextRevisionNumber} {t("routinehistorytab.general.withthesamecontenthistorystaysappend")}</p>
        </div>
        <div className="flex flex-wrap items-center gap-2">
          <Button variant="outline" size="sm" onClick={onReturn} disabled={pending}>
            {t("routinehistorytab.general.returntocurrent")}</Button>
          <Button size="sm" onClick={onRestore} disabled={pending}>
            <RotateCcw className="mr-1.5 h-3.5 w-3.5" />
            {t("routinehistorytab.general.restoreasnewrevision")}</Button>
        </div>
      </div>
    </div>
  );
...
function ConflictBanner({
  ...
  return (
    <div className="rounded-md border border-amber-500/30 bg-amber-500/5 px-4 py-3">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
        <div className="space-y-1">
          <p className="text-sm font-medium text-amber-800 dark:text-amber-200">{t("routinehistorytab.general.unsavedroutineedits")}</p>
          <p className="text-xs text-muted-foreground">
            {t("routinehistorytab.general.youchanged")} {fieldsText} {t("routinehistorytab.general.buthaventsavedyetsaveor")}</p>
        </div>
        <div className="flex flex-wrap items-center gap-2">
          <Button variant="outline" size="sm" onClick={onDiscard}>
            {t("routinehistorytab.general.discardchanges")}</Button>
          <Button size="sm" onClick={onSave}>
            {t("routinehistorytab.general.saveandcontinue")}</Button>
        </div>
      </div>
    ...
    <aside className="space-y-1">
      <header className="flex items-center justify-between pb-2">
        <p className="text-xs font-medium uppercase tracking-(--tracking-caps) text-muted-foreground">
          {t("routinehistorytab.general.revisions")}</p>
        <span className="text-(length:--text-micro) text-muted-foreground">{totalRevisions} {t("routinehistorytab.general.total")}</span>
      </header>
        ...
        return (
          ...
          >
            <div className="flex items-center gap-2 text-sm font-medium">
              <span>{t("routinehistorytab.general.rev")} {revision.revisionNumber}</span>
              {isCurrent && (
                <Badge variant="outline" className="border-border px-1.5 text-(length:--text-nano) uppercase tracking-(--tracking-eyebrow) text-muted-foreground">
                  {t("routinehistorytab.general.current")}</Badge>
              )}
              {revision.restoredFromRevisionId && (
                <Badge variant="outline" className="border-amber-500/40 bg-amber-500/10 px-1.5 text-(length:--text-nano) uppercase tracking-(--tracking-eyebrow) text-amber-800 dark:text-amber-200">
                  {t("routinehistorytab.general.restored")}</Badge>
              )}
            </div>
      ...
      {totalRevisions > revisions.length && !showOlder && (
        <Button variant="ghost" size="sm" className="w-full" onClick={onShowOlder}>
          {t("routinehistorytab.general.show")} {totalRevisions - revisions.length} {t("routinehistorytab.general.older")}</Button>
      )}
    </aside>
  );
}
    ...
    <div className="space-y-4">
      <header className={`${cardWrapper} p-4 space-y-2`}>
        <div className="flex flex-wrap items-start justify-between gap-3">
          <div className="space-y-1 min-w-0">
            <p className="text-sm font-medium">{t("routinehistorytab.general.rev1")} {revision.revisionNumber}</p>
            <p className="text-xs text-muted-foreground truncate">
              {t("routinehistorytab.general.saved")} {relativeTime(revision.createdAt)} {t("routinehistorytab.general.by")} {getActorLabel(revision)}
              {revision.changeSummary ? ` · ${revision.changeSummary}` : ""}
            </p>
          </div>
          <div className="flex flex-wrap items-center gap-2">
            <Button variant="outline" size="sm" onClick={onCompare}>
              ...
              {t("routinehistorytab.general.comparewithcurrent")}</Button>
            <Button
              size="sm"
              onClick={onRestore}
      ...
      <div className={`${cardWrapper} p-3`}>
        <p className="pb-2 text-xs font-medium uppercase tracking-(--tracking-caps) text-muted-foreground">
          {t("routinehistorytab.general.structuredfields")}</p>
              ...
              <p className="text-sm">
                ...
                {row.differs && (
                  <Badge variant="outline" className="ml-2 border-amber-500/40 bg-amber-500/10 px-1.5 text-(length:--text-nano) uppercase tracking-(--tracking-eyebrow) text-amber-800 dark:text-amber-200">
                    {t("routinehistorytab.general.differsfromcurrent")}</Badge>
                )}
              </p>
            </div>
          ))}
      ...
      <div className={`${cardWrapper} p-3 space-y-2`}>
        <p className="text-xs font-medium uppercase tracking-(--tracking-caps) text-muted-foreground">
          {t("routinehistorytab.general.description")}</p>
        <div className="rounded-md bg-background/40 p-3 text-sm leading-7">
          {snapshot.description ? (
            <MarkdownBody>{snapshot.description}</MarkdownBody>
          ) : (
            <span className="text-muted-foreground">{t("routinehistorytab.general.nodescription")}</span>
          )}
        </div>
      </div>
      ...
      <div className={`${cardWrapper} p-3 space-y-2`}>
        <p className="text-xs font-medium uppercase tracking-(--tracking-caps) text-muted-foreground">
          {t("routinehistorytab.general.triggers")}{triggers.length})
        </p>
        {triggers.length === 0 ? (
          <p className="text-sm text-muted-foreground">{t("routinehistorytab.general.notriggersinthisrevision")}</p>
        ) : (
        ...
        <p className="text-xs text-muted-foreground">
          {t("routinehistorytab.general.webhooksecretsarenotstoredinrevisions")}</p>
      </div>

      {snapshot.variables.length > 0 && (
        <div className={`${cardWrapper} p-3 space-y-2`}>
          <p className="text-xs font-medium uppercase tracking-(--tracking-caps) text-muted-foreground">
            {t("routinehistorytab.general.variables")}{snapshot.variables.length})
          </p>
          <ul className="divide-y divide-border">
            {snapshot.variables.map((variable) => (
              <li key={variable.name} className="py-2 flex items-center justify-between text-sm">
                ...
                <span className="text-xs text-muted-foreground">
                  {t("routinehistorytab.general.default")} {formatVariableDefault(variable)}
                </span>
              </li>
            ))}
          </ul>
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-md">
        <DialogHeader>
          <DialogTitle>{t("routinehistorytab.general.restorerevision")} {target.revisionNumber}?</DialogTitle>
          <DialogDescription>
            {t("routinehistorytab.general.thiscreatesanewrevision")} {newRevisionNumber} {t("routinehistorytab.general.withthesamecontentasrevision")}{" "}
            {target.revisionNumber}{t("routinehistorytab.general.revisions2")} {target.revisionNumber}–{currentRevisionNumber} {t("routinehistorytab.general.stayinhistoryandarenotmodified")}</DialogDescription>
        </DialogHeader>
        <ul className="space-y-2 text-sm">
          <li className="flex items-start gap-2">
            <span className="mt-1 inline-block h-1.5 w-1.5 rounded-full bg-emerald-400" />
            {t("routinehistorytab.general.routinefieldvaluesvariablesandschedulecron")}</li>
          {envDiffCounts.total > 0 && (
            <li className="flex items-start gap-2">
              <span className="mt-1 inline-block h-1.5 w-1.5 rounded-full bg-emerald-400" />
              {t("routinehistorytab.general.routinesecretswillrevert")} {formatEnvDiffCounts(envDiffCounts)}.
            </li>
          )}
          <li className="flex items-start gap-2">
            <span className="mt-1 inline-block h-1.5 w-1.5 rounded-full bg-emerald-400" />
            {t("routinehistorytab.general.previousrunhistoryispreserved")}</li>
          {recreatedWebhookLabels.map((label) => (
            <li key={label} className="flex items-start gap-2 text-amber-800 dark:text-amber-200">
              ...
              {t("routinehistorytab.general.thewebhooktrigger")} {label} {t("routinehistorytab.general.willberecreatedwithanewurl")}</li>
          ))}
        </ul>
        <div className="space-y-1.5">
          <Label htmlFor="restore-change-summary" className="text-xs">
            {t("routinehistorytab.general.changesummaryoptional")}</Label>
          <Input
            id="restore-change-summary"
            value={changeSummary}
            placeholder={t("routinehistorytab.general.whyareyourestoringvisibleinhistory")}
            onChange={(event) => onChangeSummaryChange(event.target.value)}
          />
        </div>
        <DialogFooter>
          <Button variant="outline" onClick={() => onOpenChange(false)} disabled={pending}>
            {t("routinehistorytab.general.cancel")}</Button>
          <Button onClick={onConfirm} disabled={pending}>
            <RotateCcw className="mr-1.5 h-3.5 w-3.5" />
            {pending ? t("routinehistorytab.general.restoring") : `Restore as revision ${newRevisionNumber}`}
          </Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="!max-w-(--pct-90) w-full max-h-(--sz-85vh) overflow-hidden flex flex-col">
        <DialogHeader>
          <DialogTitle>{t("routinehistorytab.general.compareroutinerevisions")}</DialogTitle>
        </DialogHeader>
        <div className="flex flex-wrap items-center gap-3">
          <RevisionPicker
            label={t("routinehistorytab.general.old")}
            value={leftId}
            onChange={setLeftId}
            revisions={revisions}
            tone="red"
          />
          <RevisionPicker
            label={t("routinehistorytab.general.new")}
            value={rightId}
            onChange={setRightId}
            revisions={revisions}
            tone="green"
        ...
        <div className="overflow-auto flex-1 space-y-4">
          <section className="space-y-2">
            <p className="text-xs font-medium uppercase tracking-(--tracking-caps) text-muted-foreground">
              {t("routinehistorytab.general.fieldchanges")}</p>
            {fieldChanges.length === 0 ? (
              <p className="text-sm text-muted-foreground">{t("routinehistorytab.general.nostructuralfieldchanges")}</p>
            ) : (
              <table className="w-full text-sm border border-border rounded-md overflow-hidden">
                <thead>
                  <tr className="text-xs uppercase tracking-wide bg-muted/30 text-muted-foreground">
                    <th className="px-3 py-2 text-left">{t("routinehistorytab.general.field")}</th>
                    <th className="px-3 py-2 text-left">{t("routinehistorytab.general.oldvalue")}</th>
                    <th className="px-3 py-2 text-left">{t("routinehistorytab.general.newvalue")}</th>
                  </tr>
                </thead>
                <tbody>
          ...
          <section className="space-y-2">
            <p className="text-xs font-medium uppercase tracking-(--tracking-caps) text-muted-foreground">
              {t("routinehistorytab.general.descriptiondiff")}</p>
            <DiffTable rows={descriptionDiff} />
          </section>
        </div>
        <DialogFooter className="justify-between sm:justify-between">
          <Button variant="outline" onClick={() => onOpenChange(false)}>
            {t("routinehistorytab.general.close")}</Button>
          {leftIsHistorical && left && (
            <Button onClick={() => onRestore(left)}>
              <RotateCcw className="mr-1.5 h-3.5 w-3.5" />
              {t("routinehistorytab.general.restorerev")} {left.revisionNumber} {t("routinehistorytab.general.asnewrevision")}</Button>
          )}
        </DialogFooter>
      </DialogContent>
    </Dialog>
      ...
      >
        {revisions.map((revision) => (
          <option key={revision.id} value={revision.id}>
            {t("routinehistorytab.general.rev3")} {revision.revisionNumber} — {relativeTime(revision.createdAt)}
            {revision.changeSummary ? ` • ${revision.changeSummary}` : ""}
          </option>
        ))}
      </select>
...
function DiffTable({ rows }: { rows: DiffRow[] }) {
  const { t } = useTranslation();
  if (rows.length === 0) {
    return <p className="text-sm text-muted-foreground">{t("routinehistorytab.general.nodescriptiononeitherrevision")}</p>;
  }
  if (rows.every((row) => row.kind === "context")) {
    return <p className="text-sm text-muted-foreground">{t("routinehistorytab.general.descriptionsareidentical")}</p>;
  }
  ...
  return (
    <div className="rounded-md border border-border text-xs font-mono leading-6 overflow-hidden">
      <div className="grid grid-cols-(--gtc-1) border-b border-border/60 bg-muted/30 px-3 py-2 text-(length:--text-micro) uppercase tracking-wide text-muted-foreground">
        <span>{t("routinehistorytab.general.old4")}</span>
        <span>{t("routinehistorytab.general.new5")}</span>
        <span />
        <span>{t("routinehistorytab.general.content")}</span>
      </div>
      {rows.map((row, index) => (
        <div
