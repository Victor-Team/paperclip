...
export function ThemeToggle({ className, variant = "icon", onAfterToggle }: ThemeToggleProps) {
  ...
  const label = isDark ? t("themetoggle.general.switchToLight") : t("themetoggle.general.switchToDark");
    ...
    return (
      ...
      >
        ...
        <span className="min-w-0 flex-1">
          ...
          <span className="block text-xs text-muted-foreground">{t("themetoggle.general.toggleDescription")}</span>
        </span>
      </button>
    );
  }
