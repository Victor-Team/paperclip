...
function DeliveryBadge({ mode }: { mode: "env" | "api" }) {
  ...
  if (mode === "env") {
    return (
      ...
      >
        <Variable className="size-3" /> {t("agentsecretaccesseditor.general.envvar")}</Badge>
    );
  }
  return (
    <Badge
    ...
    >
      <ServerCog className="size-3" /> {t("agentsecretaccesseditor.general.apiaccess")}</Badge>
  );
}
  ...
  return (
    <div className="space-y-3">
      ...
      {hasBindings ? (
        ...
        <p className="text-sm text-muted-foreground">{t("agentsecretaccesseditor.general.nosecretsareboundtothisagent")}</p>
      )}
      ...
      {bindingProposals.length > 0 && onApproveProposal && onRejectProposal ? (
        <div className="space-y-2">
          <div className="text-(length:--text-micro) font-medium uppercase tracking-wide text-muted-foreground">
            {t("agentsecretaccesseditor.general.proposedaccess")}</div>
              ...
              >
                <div className="min-w-0 flex-1 space-y-1">
                  ...
                  <p className="flex flex-wrap items-center gap-1 text-muted-foreground">
                    <span>{t("agentsecretaccesseditor.general.proposedby")} {proposal.proposedBy.name}</span>
      ...
      <div className="space-y-2">
        <div className="text-(length:--text-micro) font-medium uppercase tracking-wide text-muted-foreground">
          {t("agentsecretaccesseditor.general.apiaccessnoenvvar")}</div>
        {rows.length > 0 ? (
          <div className="space-y-2">
            {rows.map((row) => {
                ...
                <div key={row.id} className="space-y-1">
                  <div className="grid grid-cols-(--gtc-65) items-start gap-1.5">
                    <div>
                      <Input
                        ...
                        placeholder="ALIAS"
                        aria-label={t("agentsecretaccesseditor.general.accessalias")}
                        disabled={disabled}
                        className={cn(
                    ...
                    <div className="flex min-w-0 items-start gap-1.5">
                      ...
                      <select
                        ...
                        aria-label={t("agentsecretaccesseditor.general.version")}
                      >
                        <option value="latest">{t("agentsecretaccesseditor.general.latest")}</option>
                        {selectedSecret
                          ? Array.from({ length: Math.max(0, selectedSecret.latestVersion) }, (_, index) => {
                              ...
                              return (
                                <option key={version} value={version}>
                                  {t("agentsecretaccesseditor.general.v")}{version}
                                </option>
                              );
                            })
                          : null}
                    ...
                    <button
                      ...
                      disabled={disabled}
                      aria-label={t("agentsecretaccesseditor.general.removeapiaccess")}
                  ...
                  {aliasInvalid ? (
                    <p className="pl-0.5 text-(length:--text-micro) text-destructive">
                      {t("agentsecretaccesseditor.general.invalidaliasuselettersdigitsand")}</p>
                  ) : aliasDuplicate ? (
                    <p className="pl-0.5 text-(length:--text-micro) text-destructive">{t("agentsecretaccesseditor.general.duplicatealias")}</p>
                  ) : null}
                </div>
              );
            })}
        ...
        >
          ...
          {t("agentsecretaccesseditor.general.addapiaccess")}</button>
      </div>

      <p className="text-(length:--text-micro) text-muted-foreground/70">
        {deliveryModeDescription("api")} {t("agentsecretaccesseditor.general.theagentreadsthembyaliasthrough")}<code>{t("agentsecretaccesseditor.general.getagentsmesecrets")}</code>.
      </p>
    </div>
  );
}
