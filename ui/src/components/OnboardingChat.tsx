  ...
  if (isLoading) {
    return (
      <div className="flex items-center justify-center py-8 text-muted-foreground text-sm">
        ...
        {t("onboardingchat.general.loadingconversation")}</div>
    );
  }
              ...
              <div className="flex items-center gap-1.5 mb-1">
                ...
                {isPlan && (
                  <span className="inline-flex items-center gap-0.5 text-(length:--text-nano) text-green-600 dark:text-green-400 font-medium">
                    ...
                    {t("onboardingchat.general.hiringplandetected")}</span>
                )}
              </div>
            ...
            <div className="flex items-center gap-2">
              ...
              <div>
                <p className="text-sm font-medium">
                  {agentName} {t("onboardingchat.general.haspreparedahiringplan")}</p>
                <p className="text-(length:--text-micro) text-muted-foreground">
                  {t("onboardingchat.general.reviewitmakeeditsthenapprove")}</p>
              </div>
            </div>
            <Button size="sm" onClick={onReviewPlan}>
              {t("onboardingchat.general.reviewplan")}<ArrowRight className="h-3.5 w-3.5 ml-1" />
            </Button>
          </div>
        </div>
      )}
    ...
    <>
      ...
      {showMessage && (
        <div className="rounded-md px-3 py-2 text-sm bg-muted/50 border border-border mr-8 animate-in fade-in duration-300">
          ...
          <p>
            {t("onboardingchat.general.hithanksforbringingmeonto")}<strong>{companyName}</strong>.
          </p>
          <p className="mt-1">
            {t("onboardingchat.general.ourmissionis")}<em>{companyGoal}</em>
          </p>
          <p className="mt-1">
            {t("onboardingchat.general.imreadytoputtogethera")}</p>
        </div>
      )}
      ...
      {showChips && (
        <div className="flex gap-2 ml-auto justify-end animate-in fade-in duration-500">
          <button
            ...
            onClick={onDiscuss}
          >
            {t("onboardingchat.general.letsdiscussfirst")}</button>
          <button
            ...
            onClick={onStart}
          >
            {t("onboardingchat.general.yesgetstarted")}</button>
        </div>
      )}
