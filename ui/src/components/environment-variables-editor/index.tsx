    ...
    <TooltipProvider>
      ...
      {attentionCount > 1 ? (
        <p className="inline-flex items-center gap-1.5 text-(length:--text-micro) font-medium text-amber-700 dark:text-amber-400">
          ...
          {attentionCount} {t("index.general.bindingsneedattention")}</p>
      ) : null}
      ...
      {hasRows ? (
        <>
          ...
          <div className="hidden gap-x-1.5 @[40rem]/env:grid @[40rem]/env:grid-cols-(--gtc-14)">
            <span className="text-(length:--text-micro) font-medium uppercase tracking-wide text-muted-foreground">{t("index.general.name")}</span>
            <span className="text-(length:--text-micro) font-medium uppercase tracking-wide text-muted-foreground">{t("index.general.value")}</span>
            <span />
          </div>
        ...
        <p className="text-sm text-muted-foreground">{t("index.general.noenvironmentvariables")}</p>
      )}
      ...
      <div className="flex flex-wrap items-center justify-between gap-2 pt-1">
        ...
        >
          ...
          {t("index.general.addvariable")}</button>
        ...
        {quickBind.length > 0 && !disabled ? (
          <div className="flex flex-wrap items-center gap-1.5">
            <span className="inline-flex items-center gap-1 text-(length:--text-micro) text-muted-foreground/70">
              ...
              {t("index.general.recentlyused")}</span>
        ...
        >
          <div className="flex min-w-0 flex-col gap-0.5">
            <div className="flex items-center gap-2 text-sm font-medium">
              ...
              <span>{t("index.general.unsavedchanges")}</span>
            </div>
            {changeSummaryText ? (
      ...
      {rows.some((row) => row.source === "user_secret" && row.userSecretKey) ? (
        <p className="inline-flex items-start gap-1 text-(length:--text-micro) text-muted-foreground/70">
          ...
          <span>
            {t("index.general.usersecretsresolvefromtheuserresponsible")}</span>
        </p>
      ) : null}
      </div>
    </TooltipProvider>
