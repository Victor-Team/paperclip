...
export function RoutineContextualSidebar({
  ...
  return (
    ...
    >
      <nav className="min-h-0 flex-1 overflow-y-auto px-3 py-2" aria-label={t("routinecontextualsidebar.general.routinenavigation")}>
        ...
        <p className="px-4 pb-1 pt-5 text-(length:--text-nano) font-mono font-medium uppercase tracking-widest text-muted-foreground/60">
          {t("routinecontextualsidebar.general.operate")}</p>
        <div className="flex flex-col gap-0.5">
          <SidebarNavItem
            to={routineDetailHref(routineId, "runs")}
            label={t("routinecontextualsidebar.general.runs")}
            icon={Play}
          />
          <SidebarNavItem
            to={routineDetailHref(routineId, "activity")}
            label={t("routinecontextualsidebar.general.activity")}
            icon={Activity}
          />
        </div>
      </nav>
