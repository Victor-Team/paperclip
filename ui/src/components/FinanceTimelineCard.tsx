  ...
  return (
    <Card>
      <CardHeader className="px-4 pt-4 pb-1">
        <CardTitle className="text-base">{t("financetimelinecard.general.recentfinancialevents")}</CardTitle>
        <CardDescription>{t("financetimelinecard.general.topupsfeescreditscommitmentsandother")}</CardDescription>
      </CardHeader>
            ...
            >
              <div className="flex items-start justify-between gap-3">
                <div className="min-w-0 space-y-2">
                  ...
                  {(row.description || row.externalInvoiceId || row.region || row.pricingTier) && (
                    <div className="space-y-1 text-xs text-muted-foreground">
                      ...
                      {row.externalInvoiceId ? <div>{t("financetimelinecard.general.invoice")} {row.externalInvoiceId}</div> : null}
                      {row.region ? <div>{t("financetimelinecard.general.region")} {row.region}</div> : null}
                      {row.pricingTier ? <div>{t("financetimelinecard.general.tier")} {row.pricingTier}</div> : null}
                    </div>
                  )}
                </div>
                <div className="text-right tabular-nums">
                  ...
                  {row.estimated ? <div className="text-(length:--text-micro) uppercase tracking-(--tracking-eyebrow) text-amber-600">{t("financetimelinecard.general.estimated")}</div> : null}
                </div>
              </div>
            </div>
          ))
