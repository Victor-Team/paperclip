  ...
  if (detail.isLoading) {
    return (
      <div className="flex items-center gap-2 py-3 text-xs text-muted-foreground">
        <Loader2 className="h-3.5 w-3.5 animate-spin" /> {t("decisionresolver.general.loadingdecision")}</div>
    );
  }
  ...
  if (detail.error || !decision) {
    return (
      <p className="py-3 text-xs text-muted-foreground">
        {t("decisionresolver.general.thisdecisionisnolongeravailableit")}</p>
    );
  }
