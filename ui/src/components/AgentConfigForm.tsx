      ...
      <div className={cn("relative", cards && "space-y-6")}>
        {isDirty && !props.hideInlineSave && (
          <div className="sticky top-0 z-10 flex items-center justify-end border-b border-primary/20 bg-background/90 px-4 py-2 backdrop-blur-sm">
            <div className="flex items-center gap-3">
              <span className="text-xs text-muted-foreground">{t("agentconfigform.general.unsavedchanges")}</span>
              <Button size="sm" onClick={handleSave} disabled={props.isSaving}>
                {props.isSaving ? t("agentconfigform.general.saving") : t("agentconfigform.general.save")}
              </Button>
            </div>
          </div>
        )}
        ...
        {props.environmentVariablesPlacement === "secrets" && (
          <div data-config-section="environment-variables" className={cn(!cards && "border-b border-border")}>
            {cards
              ? <h3 className="mb-3 text-sm font-medium">{t("agentconfigform.general.environmentvariables")}</h3>
              : <div className="px-4 py-2 text-xs font-medium text-muted-foreground">{t("agentconfigform.general.environmentvariables1")}</div>
            }
        ...
        <div data-config-section="secrets" className={cn(!cards && "border-b border-border")}>
          {cards
            ? <h3 className="mb-3 text-sm font-medium">{t("agentconfigform.general.secretaccess")}</h3>
            : <div className="px-4 py-2 text-xs font-medium text-muted-foreground">{t("agentconfigform.general.secretaccess2")}</div>
          }
    ...
    <ConfigSections order={props.sectionOrder} className={cn("relative", cards && "space-y-6")}>
      ...
      {isDirty && !props.hideInlineSave && (
        <div className="sticky top-0 z-10 flex items-center justify-end px-4 py-2 bg-background/90 backdrop-blur-sm border-b border-primary/20">
          <div className="flex items-center gap-3">
            <span className="text-xs text-muted-foreground">{t("agentconfigform.general.unsavedchanges3")}</span>
            <Button
              size="sm"
              onClick={handleSave}
              disabled={!isCreate && props.isSaving}
            >
              {!isCreate && props.isSaving ? t("agentconfigform.general.saving4") : t("agentconfigform.general.save5")}
            </Button>
          </div>
        </div>
      )}
      ...
      {!isCreate && (
        <div data-config-section="identity" className={cn(!cards && "border-b border-border")}>
          {cards
            ? <h3 className="text-sm font-medium mb-3">{props.sectionTitles?.[t("agentconfigform.general.identity")] ?? t("agentconfigform.general.identity6")}</h3>
            : <div className="px-4 py-2 text-xs font-medium text-muted-foreground">{t("agentconfigform.general.identity7")}</div>
          }
          <div className={cn(cards ? "border border-border rounded-lg p-4 space-y-3" : "px-4 pb-3 space-y-3")}>
            <Field label={t("agentconfigform.general.name")} hint={help.name}>
              <DraftInput
                value={eff("identity", "name", props.agent.name)}
                onCommit={(v) => mark("identity", "name", v)}
                immediate
                className={inputClass}
                placeholder={t("agentconfigform.general.agentname")}
              />
            </Field>
            <Field label={t("agentconfigform.general.title")} hint={help.title}>
              <DraftInput
                value={eff("identity", "title", props.agent.title ?? "")}
                onCommit={(v) => mark("identity", "title", v || null)}
                immediate
                className={inputClass}
                placeholder={t("agentconfigform.general.egvpofengineering")}
              />
            </Field>
            <Field label={t("agentconfigform.general.reportsto")} hint={help.reportsTo}>
              <ReportsToPicker
            ...
            {isLocal && !props.hidePromptTemplate && (
              <>
                <Field label={t("agentconfigform.general.prompttemplate")} hint={help.promptTemplate}>
                  <MarkdownEditor
                    value={eff(
                      "adapterConfig",
                      "promptTemplate",
                    ...
                    placeholder={t("agentconfigform.general.youareagentagentnameyourrole")}
                ...
                <div className="rounded-md border border-amber-500/25 bg-amber-500/10 px-3 py-2 text-xs text-amber-900 dark:text-amber-100">
                  {t("agentconfigform.general.prompttemplateisreplayedoneveryheartbeat")}</div>
              </>
            )}
          </div>
        </div>
      ...
      {forcedKubernetes ? (
        ...
        <div data-config-section="environment" className={cn(!cards && (isCreate ? "border-t border-border" : "border-b border-border"))}>
          {cards
            ? <h3 className="text-sm font-medium mb-3">{t("agentconfigform.general.environment")}</h3>
            : <div className="px-4 py-2 text-xs font-medium text-muted-foreground">{t("agentconfigform.general.environment8")}</div>
          }
          <div className={cn(cards ? "border border-border rounded-lg p-4 space-y-3" : "px-4 pb-3 space-y-3")}>
            <Field
              label={t("agentconfigform.general.defaultenvironment")}
            ...
            >
              {kubernetesEnvironment ? (
                <div className={cn(inputClass, "flex items-center text-muted-foreground")}>
                  {kubernetesEnvironment.name} {t("agentconfigform.general.kubernetessandbox")}</div>
              ) : (
                <div className="rounded-md border border-amber-500/25 bg-amber-500/10 px-3 py-2 text-xs text-amber-800 dark:text-amber-200">
                  {t("agentconfigform.general.thisinstancerequiresthekubernetessandboxbut")}</div>
              )}
            </Field>
          </div>
        </div>
        ...
        <div data-config-section="environment" className={cn(!cards && (isCreate ? "border-t border-border" : "border-b border-border"))}>
          {cards
            ? <h3 className="text-sm font-medium mb-3">{t("agentconfigform.general.environment9")}</h3>
            : <div className="px-4 py-2 text-xs font-medium text-muted-foreground">{t("agentconfigform.general.environment10")}</div>
          }
          <div className={cn(cards ? "border border-border rounded-lg p-4 space-y-3" : "px-4 pb-3 space-y-3")}>
            <Field label={t("agentconfigform.general.environmentoverride")}>
              <div className="space-y-2">
                ...
                >
                  <option value="">{t("agentconfigform.general.default")} {inheritedEnvironmentLabel}</option>
      ...
      <div data-config-section="adapter" className={cn(!cards && (isCreate ? "border-t border-border" : "border-b border-border"))}>
        <div className={cn(cards ? "flex items-center justify-between mb-3" : "px-4 py-2 flex items-center justify-between gap-2")}>
          {cards
            ? <h3 className="text-sm font-medium">{props.sectionTitles?.[t("agentconfigform.general.adapter")] ?? t("agentconfigform.general.adapter11")}</h3>
            : <span className="text-xs font-medium text-muted-foreground">{t("agentconfigform.general.adapter12")}</span>
          }
        ...
        <div className={cn(cards ? "border border-border rounded-lg p-4 space-y-3" : "px-4 pb-3 space-y-3")}>
          {showAdapterTypeField && (
            <Field label={t("agentconfigform.general.adaptertype")} hint={help.adapterType}>
              <AdapterTypeDropdown
                value={adapterType}
          ...
          {showInlineAdapterTestEnvironmentFeedback && !props.compactTestFeedback && (testActionError || testEnvironment.error) && (
            <div className="rounded-md border border-destructive/30 bg-destructive/10 px-3 py-2 text-xs text-destructive">
              {testActionError
                ?? (testEnvironment.error instanceof Error
                  ? testEnvironment.error.message
                  : t("agentconfigform.general.environmenttestfailed"))}
            </div>
          )}
          ...
          {showLegacyWorkingDirectoryField && (
            <Field label={t("agentconfigform.general.workingdirectorydeprecated")} hint={help.cwd}>
                ...
                <p className="text-xs text-destructive">
                  {refreshModelsError
                    ?? (fetchedModelsError instanceof Error
                      ...
                      : t("agentconfigform.general.failedtoloadadaptermodels"))}
                </p>
              )}
              {adapterType === "opencode_local"
                ...
                <p className="text-xs text-muted-foreground">
                  {t("agentconfigform.general.liveopencodemodeldiscoveryonlyrunsfor")} {currentDefaultEnvironment.name}.
                </p>
              )}
                  ...
                  {adapterType === "codex_local" &&
                    ...
                    currentThinkingEffort === "minimal" && (
                      <p className="text-xs text-amber-400">
                        {t("agentconfigform.general.codexmayrejectminimalthinkingwhensearch")}</p>
                    )}
                </>
              )}
          </>)}
      ...
      {(
        <div data-config-section="configuration" className={cn(!cards && "border-b border-border")}>
          {cards
            ? <h3 className="text-sm font-medium mb-3">{t("agentconfigform.general.configuration")}</h3>
            : <div className="px-4 py-2 text-xs font-medium text-muted-foreground">{t("agentconfigform.general.configuration13")}</div>
          }
          <div className={cn(cards ? "border border-border rounded-lg p-4 space-y-3" : "px-4 pb-3 space-y-3")}>
              {!isCreate && typeof config.bootstrapPromptTemplate === "string" && config.bootstrapPromptTemplate && (
                <>
                  <Field label={t("agentconfigform.general.bootstrappromptlegacy")} hint={help.bootstrapPrompt}>
                    <MarkdownEditor
                      value={eff(
                        "adapterConfig",
                      ...
                      placeholder={t("agentconfigform.general.optionalinitialsetuppromptforthefirst")}
                  ...
                  <div className="rounded-md border border-amber-500/25 bg-amber-500/10 px-3 py-2 text-xs text-amber-800 dark:text-amber-200">
                    {t("agentconfigform.general.bootstrappromptislegacyandwillbe")}</div>
                </>
              )}
              ...
              <CollapsibleSection
                title={t("agentconfigform.general.advanced")}
              ...
              */}
              {!hideHostPaths && (
                <Field label={t("agentconfigform.general.command")} hint={help.localCommand}>
                  <DraftInput
                    value={
                      isCreate
                        ? val!.command
              ...
              <Field label={t("agentconfigform.general.extraargscommaseparated")} hint={help.extraArgs}>
                <DraftInput
                  value={
                    isCreate
                      ? val!.extraArgs
                  ...
                  className={inputClass}
                  placeholder={t("agentconfigform.general.egverbosefoobar")}
                />
              </Field>
      ...
      {props.environmentVariablesPlacement !== "secrets" && (isLocal || configSchema?.fields.some((field) => schemaFieldSection(field.key) === "environment")) && (
        <div data-config-section="environment-variables" className={cn(!cards && "border-b border-border")}>
          {cards
            ? <h3 className="text-sm font-medium mb-3">{t("agentconfigform.general.environmentvariables14")}</h3>
            : <div className="px-4 py-2 text-xs font-medium text-muted-foreground">{t("agentconfigform.general.environmentvariables15")}</div>
          }
          <div className={cn(cards ? "border border-border rounded-lg p-4 space-y-3" : "px-4 pb-3 space-y-3")}>
      ...
      {isCreate && showCreateRunPolicySection ? (
        <div data-config-section="run-policy" className={cn(!cards && "border-b border-border")}>
          {cards
            ? <h3 className="text-sm font-medium flex items-center gap-2 mb-3"><Heart className="h-3 w-3" /> {t("agentconfigform.general.runpolicy")}</h3>
            : <div className="px-4 py-2 text-xs font-medium text-muted-foreground flex items-center gap-2"><Heart className="h-3 w-3" /> {t("agentconfigform.general.runpolicy16")}</div>
          }
          <div className={cn(cards ? "border border-border rounded-lg p-4 space-y-3" : "px-4 pb-3 space-y-3")}>
            <ToggleWithNumber
              label={t("agentconfigform.general.heartbeatoninterval")}
            ...
            />
            <CollapsibleSection title={t("agentconfigform.general.advancedrunpolicy")} open={runPolicyAdvancedOpen} onToggle={() => setRunPolicyAdvancedOpen(!runPolicyAdvancedOpen)}>
        ...
        <div data-config-section="run-policy" className={cn(!cards && "border-b border-border")}>
          {cards
            ? <h3 className="text-sm font-medium flex items-center gap-2 mb-3"><Heart className="h-3 w-3" /> {t("agentconfigform.general.runpolicy17")}</h3>
            : <div className="px-4 py-2 text-xs font-medium text-muted-foreground flex items-center gap-2"><Heart className="h-3 w-3" /> {t("agentconfigform.general.runpolicy18")}</div>
          }
          <div className={cn(cards ? "border border-border rounded-lg overflow-hidden" : "")}>
            <div className={cn(cards ? "p-4 space-y-3" : "px-4 pb-3 space-y-3")}>
              <ToggleWithNumber
                label={t("agentconfigform.general.heartbeatoninterval19")}
                hint={help.heartbeatInterval}
            ...
            <CollapsibleSection
              title={t("agentconfigform.general.advancedrunpolicy20")}
              bordered={cards}
            ...
            <div className="space-y-3">
              ...
              {!isCreate && (
                <>
                  ...
                  <Field label={t("agentconfigform.general.timeoutsec")} hint={help.timeoutSec}>
                    <DraftNumberInput
                      value={eff(
                        "adapterConfig",
                        "timeoutSec",
                  ...
                  <Field label={t("agentconfigform.general.interruptgraceperiodsec")} hint={help.graceSec}>
                    <DraftNumberInput
                      value={eff(
                        "adapterConfig",
                        "graceSec",
              ...
              <ToggleField
                label={t("agentconfigform.general.wakeondemand")}
              ...
              />
              <Field label={t("agentconfigform.general.cooldownsec")} hint={help.cooldownSec}>
                <DraftNumberInput
                  value={eff(
                    "heartbeat",
                    "cooldownSec",
              ...
              <Field label={t("agentconfigform.general.maxconcurrentruns")} hint={help.maxConcurrentRuns}>
                <DraftNumberInput
                  value={eff(
                    "heartbeat",
                    "maxConcurrentRuns",
              ...
              <div className="rounded-md border border-border/70 px-3 py-2">
                <ToggleField
                  label={t("agentconfigform.general.continueaftermaxturnstop")}
                ...
                {maxTurnContinuationEnabled ? (
                  <div className="mt-3 grid gap-3 sm:grid-cols-2">
                    <Field label={t("agentconfigform.general.continuationattempts")} hint={help.maxTurnContinuationMaxAttempts}>
                      <DraftNumberInput
                    ...
                    <Field label={t("agentconfigform.general.continuationdelaysec")} hint={help.maxTurnContinuationDelaySec}>
                      <DraftNumberInput
        ...
        <div className={cn(!cards && "border-b border-border")}>
          {cards ? (
            <h3 className="mb-3 flex items-center gap-2 text-sm font-medium">
              <Bug className="h-3 w-3" /> {t("agentconfigform.general.debugging")}</h3>
          ) : (
            <div className="flex items-center gap-2 px-4 py-2 text-xs font-medium text-muted-foreground">
              <Bug className="h-3 w-3" /> {t("agentconfigform.general.debugging21")}</div>
          )}
          <div
            className={cn(
          ...
          >
            <ToggleField
              label={t("agentconfigform.general.capturerawprovidertraces")}
            ...
            {eff<unknown>("debug", "providerTrace", debug.providerTrace) === "raw" ? (
              <div className="mt-3 flex items-start gap-2 rounded-md border border-border bg-background/60 px-3 py-2 text-xs text-foreground">
                ...
                <span>
                  {t("agentconfigform.general.rawtracingisonforfutureruns")}</span>
              </div>
            ) : null}
          </div>
        </div>
  ...
  if (status === "authenticated") {
    return (
      <div className="flex items-center gap-2 text-(length:--text-micro) text-foreground">
        ...
        <span>{t("agentconfigform.general.authenticatedtheenvironmenthascredentialsnow")}</span>
      </div>
    );
  }
  const label =
    ...
    return (
      ...
      >
        {startError ? (
          ...
          <p role="alert" className="pl-2 text-xs text-destructive">
            {status === "timed_out"
              ? t("agentconfigform.general.thelogintimedoutstartitagain")
              : status === "cancelled"
                ? t("agentconfigform.general.theloginwascancelled")
                : t("agentconfigform.general.thelogindidnotfinishstartit")}
          </p>
        ) : (
      ...
      <div className="flex items-center justify-between gap-2">
        ...
        <div className="flex items-center gap-1.5">
          {isActive && (
            ...
            >
              {t("agentconfigform.general.cancel")}</Button>
          )}
          <Button
            type="button"
            variant="outline"
          ...
          >
            {t("agentconfigform.general.signin")}</Button>
        </div>
      </div>
      ...
      <div role="status" aria-live="polite" className="space-y-2 empty:hidden">
        {isActive && !prompt && (
          <div className="flex items-center gap-2 text-(length:--text-micro) text-muted-foreground">
            ...
            <span>{t("agentconfigform.general.preparing")}</span>
          </div>
        )}
        ...
        {isActive && prompt && (
          <div className="space-y-2">
            <div className="text-(length:--text-micro) text-muted-foreground">
              {t("agentconfigform.general.copythecodethenopentheauthentication")}</div>
          ...
          <div className="flex items-center justify-between gap-2">
            <div className="min-w-0">
              <div className="text-(length:--text-micro) uppercase tracking-wide text-muted-foreground">
                {t("agentconfigform.general.1code")}</div>
              <span className="font-mono text-xs text-foreground break-all">{prompt.code}</span>
            </div>
            <AdapterLoginCopyButton value={prompt.code} label={t("agentconfigform.general.copycode")} />
          </div>
          <div className="flex items-center justify-between gap-2">
            <div className="min-w-0">
              <div className="text-(length:--text-micro) uppercase tracking-wide text-muted-foreground">
                {t("agentconfigform.general.2authenticationurl")}</div>
              <span className="font-mono text-xs text-foreground break-all">{prompt.url}</span>
            </div>
            <div className="flex items-center">
              <AdapterLoginCopyButton value={prompt.url} label={t("agentconfigform.general.copyurl")} />
              <Button
                asChild
                type="button"
                variant="ghost"
                size="icon-xs"
                aria-label={t("agentconfigform.general.opentheauthenticationpage")}
                title={t("agentconfigform.general.opentheauthenticationpage22")}
                className="text-muted-foreground hover:text-foreground"
              >
        ...
        {status === "authenticated" && accountBindState === "saving" && (
          <div className="flex items-center gap-2 text-(length:--text-micro) text-muted-foreground">
            <Loader2 className="size-3 animate-spin shrink-0" />
            <span>{t("agentconfigform.general.bindingthisagenttothesignedin")}</span>
          </div>
        )}
        {status === "authenticated" && accountBindState === "bound" && (
          <div className="flex items-center gap-2 text-(length:--text-micro) text-foreground">
            <Check className="size-3 shrink-0" />
            <span>{t("agentconfigform.general.agentboundtothesignedinaccount")}</span>
          </div>
        )}
        {status === "authenticated" && accountBindState === "failed" && (
          <div className="flex items-center gap-2 text-(length:--text-micro)">
            ...
            <span className="text-destructive">
              {t("agentconfigform.general.couldnotbindthisagenttothe")}</span>
            <Button
              type="button"
              variant="outline"
              size="sm"
            ...
            >
              {t("agentconfigform.general.retry")}</Button>
          </div>
        )}
      </div>
    </div>
      ...
      >
        ...
        {transportInsecure && (
          <p className="flex items-start gap-2 pl-2 text-xs text-amber-700 dark:text-amber-200">
            ...
            {t("agentconfigform.general.thisconnectionisnotencryptedthelogin")}</p>
        )}
        {startError ? (
          <p role="alert" className="pl-2 text-xs text-destructive">
            {startError}
  ...
  return (
    <div className="rounded-md border border-border bg-muted/40 px-3 py-2 flex flex-col gap-2">
        ...
        <div className="flex items-center gap-1.5">
          {isActive && (
            <Button
              ...
              onClick={() => cancelLogin.mutate()}
            >
              {t("agentconfigform.general.cancel23")}</Button>
          )}
          ...
          {onApplyStored && storedToken && !isActive && !isStored && !appliedStored && (
            ...
            >
              {t("agentconfigform.general.usesavedlogin")}</Button>
          )}
          <Button
            type="button"
            variant="outline"
            ...
            onClick={() => startLogin.mutate()}
          >
            {storedToken && !isActive && !isStored ? t("agentconfigform.general.signintoreplace") : t("agentconfigform.general.signin24")}
          </Button>
        </div>
      </div>
      ...
      {storedToken && !isActive && !isStored && !appliedStored && (
        <div className="text-(length:--text-micro) text-muted-foreground">
          {t("agentconfigform.general.youhaveasavedclaudeloginuse")}</div>
      )}

      {appliedStored && (
        <div className="flex items-center gap-2 text-(length:--text-micro) text-foreground">
          <Check className="size-3 shrink-0" />
          <span>{t("agentconfigform.general.thesavedclaudeloginisboundto")}</span>
        </div>
      )}
      ...
      <div role="status" aria-live="polite" className="space-y-2 empty:hidden">
        {isActive && !authorizationUrl && !isCompleting && (
          <div className="flex items-center gap-2 text-(length:--text-micro) text-muted-foreground">
            <Loader2 className="size-3 animate-spin shrink-0" />
            <span>{t("agentconfigform.general.preparing25")}</span>
          </div>
        )}
          ...
          <div className="space-y-2">
            {transportInsecure && (
              ...
              >
                ...
                <span>
                  {t("agentconfigform.general.thisconnectionisnotencryptedthelogin26")}</span>
              </div>
            )}
            <div className="text-(length:--text-micro) text-muted-foreground">
              {t("agentconfigform.general.opentheauthorizationpagethenenterthe")}</div>
            <div className="flex items-center justify-between gap-2">
              <div className="min-w-0">
                <div className="text-(length:--text-micro) uppercase tracking-wide text-muted-foreground">
                  {t("agentconfigform.general.1authorizationurl")}</div>
                <span className="font-mono text-xs text-foreground break-all">{authorizationUrl}</span>
              </div>
              <div className="flex items-center">
                <AdapterLoginCopyButton value={authorizationUrl} label={t("agentconfigform.general.copyurl27")} />
                <Button
                  asChild
                  type="button"
                  variant="ghost"
                  size="icon-xs"
                  aria-label={t("agentconfigform.general.opentheauthorizationpage")}
                  title={t("agentconfigform.general.opentheauthorizationpage28")}
                  className="text-muted-foreground hover:text-foreground"
                >
            ...
            <div className="space-y-1">
              <div className="text-(length:--text-micro) uppercase tracking-wide text-muted-foreground">
                {t("agentconfigform.general.2browsercode")}</div>
              <div className="flex items-center gap-2">
                <input
                  aria-label={t("agentconfigform.general.browsercode")}
                  type="text"
                  autoComplete="off"
                  spellCheck={false}
                  value={browserCode}
                ...
                >
                  {t("agentconfigform.general.submit")}</Button>
              </div>
            </div>
          </div>
        )}
        ...
        {isCompleting && (
          <div className="flex items-center gap-2 text-(length:--text-micro) text-muted-foreground">
            <Loader2 className="size-3 animate-spin shrink-0" />
            <span>{t("agentconfigform.general.completingthelogin")}</span>
          </div>
        )}
        ...
        {isStored && (
          <div className="flex items-center gap-2 text-(length:--text-micro) text-foreground">
            <Check className="size-3 shrink-0" />
            <span>{t("agentconfigform.general.authenticatedtheenvironmenthascredentialsnow29")}</span>
          </div>
        )}
      ...
      <div className="mt-2 space-y-1.5">
        {result.checks.map((check, idx) => (
          <div key={`${check.code}-${idx}`} className="text-(length:--text-micro) leading-relaxed break-words">
            ...
            {check.hint && <span className="block opacity-90 break-words">{t("agentconfigform.general.hint")} {check.hint}</span>}
          </div>
        ))}
      </div>
    </div>
      ...
      <PopoverContent className="w-(--radix-popover-trigger-width) p-1" align="start">
        {adapterList.map((item) => (
          ...
          >
            ...
            {item.comingSoon && (
              <span className="text-(length:--text-nano) text-muted-foreground">{t("agentconfigform.general.comingsoon")}</span>
            )}
          </button>
        ))}
      </PopoverContent>
...
function ExperimentalBadge() {
  ...
  return (
    <span className="shrink-0 rounded border border-amber-500/30 bg-amber-500/10 px-1.5 py-0.5 text-(length:--text-nano) font-medium leading-none text-amber-700 dark:text-amber-200">
      {t("agentconfigform.general.experimental")}</span>
  );
}
...
export function ModelDropdown({
  ...
  return (
    <Field label={t("agentconfigform.general.model")} hint={help.model}>
      <Popover
        open={open}
        ...
        <PopoverContent className="w-(--radix-popover-trigger-width) p-1" align="start">
          ...
          {onDetectModel && !modelSearch.trim() && (
            ...
            >
              ...
              {detectingModel ? t("agentconfigform.general.detecting") : detectedModel ? (detectModelLabel?.replace(/^Detect\b/, "Re-detect") ?? t("agentconfigform.general.redetectfromconfig")) : (detectModelLabel ?? t("agentconfigform.general.detectfromconfig"))}
            </button>
          )}
          {onRefreshModels && !modelSearch.trim() && (
            ...
            >
              ...
              {refreshingModels ? t("agentconfigform.general.refreshing") : t("agentconfigform.general.refreshmodels")}
            </button>
          )}
          {value && (!models.some((m) => m.id === value) || promotedModelIds.has(value)) && (
            ...
            >
              ...
              <Badge variant="outline" className="ml-auto text-(length:--text-nano) px-1.5 bg-green-500/15 text-green-400 border-green-500/20">
                {t("agentconfigform.general.current")}</Badge>
            </button>
          )}
          {detectedModel && detectedModel !== value && (
            ...
            >
              ...
              <Badge variant="outline" className="ml-auto text-(length:--text-nano) px-1.5 bg-blue-500/15 text-blue-400 border-blue-500/20">
                {t("agentconfigform.general.detected")}</Badge>
            </button>
          )}
          {detectedModelCandidates
              ...
              return (
                ...
                >
                  ...
                  <Badge variant="outline" className="ml-auto text-(length:--text-nano) px-1.5 bg-sky-500/15 text-sky-400 border-sky-500/20">
                    {t("agentconfigform.general.config")}</Badge>
                </button>
              );
            })}
          <div className="max-h-(--sz-240px) overflow-y-auto">
            {allowDefault && (
              ...
              >
                {t("agentconfigform.general.default30")}</button>
            )}
            {canCreateManualModel && (
              ...
              >
                <span>{t("agentconfigform.general.usemanualmodel")}</span>
            ...
            {filteredModels.length === 0 && !canCreateManualModel && promotedModelIds.size === 0 && (
              <div className="px-2 py-2 space-y-2">
                <p className="text-xs text-muted-foreground">
                  {onDetectModel
                    ? (emptyDetectHint ?? t("agentconfigform.general.nomodeldetectedyetenteraprovider"))
                    : t("agentconfigform.general.nomodelsfound")}
                </p>
              </div>
            )}
          </div>
...
function ThinkingEffortDropdown({
  ...
  return (
    <Field label={t("agentconfigform.general.thinkingeffort")} hint={help.thinkingEffort}>
      <Popover open={open} onOpenChange={onOpenChange}>
        <PopoverTrigger asChild>
          <button className="inline-flex items-center gap-1.5 rounded-md border border-border px-2.5 py-1.5 text-sm hover:bg-accent/50 transition-colors w-full justify-between">
            <span className={cn(!value && "text-muted-foreground")}>{selected?.label ?? t("agentconfigform.general.auto")}</span>
            <ChevronDown className="h-3 w-3 text-muted-foreground" />
          </button>
        </PopoverTrigger>
        <PopoverContent className="w-(--radix-popover-trigger-width) p-1" align="start">
