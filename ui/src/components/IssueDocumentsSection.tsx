      ...
      {isEmpty && !draft?.isNew ? (
        <div className="flex flex-wrap items-center justify-end gap-2 min-w-0">
          ...
          <Button variant="outline" size="sm" onClick={beginNewDocument} className="shrink-0">
            ...
            <span className="hidden sm:inline">{t("issuedocumentssection.general.newdocument")}</span>
            <span className="sm:hidden">{t("issuedocumentssection.general.new")}</span>
          </Button>
        </div>
      ) : (
        <div className="flex flex-wrap items-center gap-2 min-w-0">
          <h3 className="w-full text-sm font-medium text-muted-foreground shrink-0 sm:w-auto">{t("issuedocumentssection.general.documents")}</h3>
          <div className="flex flex-wrap items-center gap-2 min-w-0 sm:ml-auto">
            {extraActions}
            <Button variant="outline" size="sm" onClick={beginNewDocument} className="shrink-0">
              <Plus className="mr-1.5 h-3.5 w-3.5" />
              <span className="hidden sm:inline">{t("issuedocumentssection.general.newdocument1")}</span>
              <span className="sm:hidden">{t("issuedocumentssection.general.new2")}</span>
            </Button>
          </div>
        </div>
      )}
      ...
      {draft?.isNew && (
        ...
        >
          <Input
            ...
            placeholder={t("issuedocumentssection.general.documentkey")}
          />
          {newDocumentKeyError && (
          ...
          {!isPlanKey(draft.key) && (
            <Input
              ...
              placeholder={t("issuedocumentssection.general.optionaltitle")}
            />
          )}
          <MarkdownEditor
            value={draft.body}
            ...
            placeholder={t("issuedocumentssection.general.markdownbody")}
            bordered={false}
          ...
          <div className="flex items-center justify-end gap-2">
            <Button variant="outline" size="sm" onClick={cancelDraft}>
              ...
              {t("issuedocumentssection.general.cancel")}</Button>
            <Button
              size="sm"
              onClick={() => void commitDraft(draft, { clearAfterSave: false, trackAutosave: false })}
              disabled={upsertDocument.isPending}
            >
              {upsertDocument.isPending ? t("issuedocumentssection.general.saving") : t("issuedocumentssection.general.createdocument")}
            </Button>
          </div>
        </div>
      )}
                ...
                actionsSlot={
                  <>
                    ...
                    >
                      ...
                      <span title={t("issuedocumentssection.general.lockeddocument")} aria-label={t("issuedocumentssection.general.lockeddocument3")} className="inline-flex h-6 w-6 items-center justify-center text-amber-700 dark:text-amber-300">
                    ...
                    <DropdownMenu>
                      <DropdownMenuTrigger asChild>
                        <Button
                          ...
                          title={t("issuedocumentssection.general.documentactions")}
                        >
                      ...
                      <DropdownMenuContent align="end">
                        {!isHistoricalPreview && !isLocked ? (
                          <DropdownMenuItem onClick={() => beginEdit(doc.key)}>
                            ...
                            {t("issuedocumentssection.general.editdocument")}</DropdownMenuItem>
                        ) : null}
                        ...
                        >
                          ...
                          {t("issuedocumentssection.general.downloaddocument")}</DropdownMenuItem>
                        {doc.latestRevisionNumber > 1 ? (
                          <DropdownMenuItem onClick={() => setDiffViewKey(doc.key)}>
                            <Diff className="h-3.5 w-3.5" />
                            {t("issuedocumentssection.general.viewdiff")}</DropdownMenuItem>
                        ) : null}
                        ...
                        {canDeleteDocuments && !isLocked ? (
                          ...
                          >
                            ...
                            {t("issuedocumentssection.general.deletedocument")}</DropdownMenuItem>
                        ) : null}
                      </DropdownMenuContent>
                    </DropdownMenu>
                  </>
                ...
                >
                      ...
                      <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
                        <div className="space-y-1">
                          <p className="text-sm font-medium text-amber-800 dark:text-amber-200">
                            {t("issuedocumentssection.general.viewingrevision")} {selectedHistoricalRevision.revisionNumber}
                          </p>
                          <p className="text-xs text-muted-foreground">
                            {t("issuedocumentssection.general.thisisahistoricalpreviewrestoringit")}</p>
                        </div>
                        <div className="flex flex-wrap items-center gap-2">
                          ...
                          >
                            {t("issuedocumentssection.general.returntolatest")}</Button>
                          {!isLocked ? (
                            <Button
                              size="sm"
                  ...
                  {activeConflict && !isHistoricalPreview && (
                    <div className="rounded-md border border-amber-500/30 bg-amber-500/5 px-3 py-3">
                      <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
                        <div className="space-y-1">
                          <p className="text-sm font-medium text-amber-800 dark:text-amber-200">{t("issuedocumentssection.general.outofdate")}</p>
                          <p className="text-xs text-muted-foreground">
                            {t("issuedocumentssection.general.thisdocumentchangedwhileyouwereediting")}</p>
                        </div>
                        <div className="flex flex-wrap items-center gap-2">
                          <Button
                            variant="outline"
                          ...
                          >
                            {t("issuedocumentssection.general.keepmydraft")}</Button>
                          <Button
                            variant="outline"
                            size="sm"
                            onClick={() => reloadDocumentFromServer(doc.key)}
                          >
                            {t("issuedocumentssection.general.reloadremote")}</Button>
                          <Button
                            size="sm"
                      ...
                      {activeConflict.showRemote && (
                        <div className="mt-3 rounded-md border border-border/70 bg-background/60 p-3">
                          <div className="mb-2 flex items-center gap-2 text-(length:--text-micro) text-muted-foreground">
                            <span>{t("issuedocumentssection.general.remoterevision")} {activeConflict.serverDocument.latestRevisionNumber}</span>
                            <span>•</span>
                            <span>{t("issuedocumentssection.general.updated")} {relativeTime(activeConflict.serverDocument.updatedAt)}</span>
                          </div>
                  ...
                  {activeDraft && !isPlanKey(doc.key) && !isHistoricalPreview && (
                    <Input
                      ...
                      placeholder={t("issuedocumentssection.general.optionaltitle4")}
                    />
                  )}
                  <div
                    ...
                    {(() => {
                      const renderedDocumentBody = isHistoricalPreview ? (
                        ...
                        <MarkdownEditor
                          ...
                          placeholder={t("issuedocumentssection.general.markdownbody5")}
                          bordered={false}
                          className="bg-transparent"
              ...
              {confirmDeleteKey === doc.key && (
                <div className="mt-3 flex items-center justify-between gap-3 rounded-md border border-destructive/20 bg-destructive/5 px-4 py-3">
                  <p className="text-sm text-destructive font-medium">
                    {t("issuedocumentssection.general.deletethisdocumentthiscannotbeundone")}</p>
                  <div className="flex items-center gap-2 shrink-0">
                    ...
                    >
                      {t("issuedocumentssection.general.cancel6")}</Button>
                    <Button
                      variant="destructive"
                      size="sm"
