...
export function SkillFolderRail({
  ...
  return (
    <div className="relative hidden h-full shrink-0 md:flex" style={{ width: `${width}px` }}>
      <nav
        aria-label={t("skillfoldertree.general.skillfolders")}
      ...
      <div className="mb-2 flex items-center justify-between gap-2 pt-0.5">
        <div className="text-(length:--text-micro) font-medium uppercase tracking-wide text-muted-foreground">
          {t("skillfoldertree.general.folders")}</div>
        <Button variant="ghost" size="icon-sm" title={t("skillfoldertree.general.newfolder")} onClick={() => onCreateFolder(null)}>
      ...
      {loading ? (
        ...
        <div className="space-y-0.5">
          <VirtualRow
            ...
            label={t("skillfoldertree.general.allskills")}
            count={allCount}
          ...
          {model.my ? (
            ...
            <VirtualRow
              active={false}
              label={t("skillfoldertree.general.myskills")}
              count={0}
          ...
          <RailHeading label={t("skillfoldertree.general.organization")} onCreate={() => onCreateFolder(null)} />
          {model.company.length > 0 ? (
            ...
            <div className="px-2 py-1 text-xs text-muted-foreground">{t("skillfoldertree.general.noorganizationfoldersyet")}</div>
          )}
          ...
          {model.projects ? (
            ...
            <VirtualRow
              active={false}
              label={t("skillfoldertree.general.projects")}
              count={0}
          ...
          <div className="px-2 pb-1 pt-3 text-(length:--text-micro) font-medium uppercase tracking-wide text-muted-foreground">
            {t("skillfoldertree.general.system")}</div>
          <VirtualRow
            active={selection === "unfiled"}
            label={t("skillfoldertree.general.unfiled")}
            count={unfiledCount}
      ...
      {tags.length > 0 ? (
        <div className="mt-4 border-t border-border pt-3">
          <div className="mb-1.5 flex items-center gap-1.5 px-2 text-(length:--text-micro) font-medium uppercase tracking-wide text-muted-foreground">
            ...
            {t("skillfoldertree.general.tags")}</div>
      ...
      <div
        role="separator"
        aria-label={t("skillfoldertree.general.resizeskillfolders")}
            ...
            <DropdownMenuContent align="end">
              {canNest ? (
                <DropdownMenuItem onSelect={() => onCreateFolder(folder.id)}>
                  ...
                  {t("skillfoldertree.general.newsubfolder")}</DropdownMenuItem>
              ) : null}
              {editable ? (
                <>
                  <DropdownMenuItem onSelect={() => onStartRename(folder)}>{t("skillfoldertree.general.rename")}</DropdownMenuItem>
                  <DropdownMenuItem onSelect={() => onEditFolder(folder)}>{t("skillfoldertree.general.editcolor")}</DropdownMenuItem>
                  <DropdownMenuItem onSelect={() => onMoveFolder(folder, isInMySkills ? "company" : "my")}>
                    <MoveRight className="h-3.5 w-3.5" />
                    {t("skillfoldertree.general.moveto")} {isInMySkills ? t("skillfoldertree.general.organization1") : t("skillfoldertree.general.myskills2")}
                  </DropdownMenuItem>
                  <DropdownMenuSeparator />
                  <DropdownMenuItem variant="destructive" onSelect={() => onDeleteFolder(folder)}>
                    <Trash2 className="h-3.5 w-3.5" />
                    {t("skillfoldertree.general.delete")}</DropdownMenuItem>
                </>
              ) : null}
            </DropdownMenuContent>
          </DropdownMenu>
...
export function FolderBreadcrumb({
  ...
  return (
    <nav aria-label={t("skillfoldertree.general.folderpath")} className="flex flex-wrap items-center gap-1 text-sm">
      <button
        type="button"
      ...
      >
        ...
        {t("skillfoldertree.general.allskills3")}</button>
      {selection === "unfiled" ? (
        <>
          <ChevronRight className="h-3.5 w-3.5 text-muted-foreground/60" />
          <span className="rounded px-1.5 py-0.5 font-medium text-foreground">{t("skillfoldertree.general.unfiled4")}</span>
        </>
      ) : null}
  ...
  return (
    <div className="mb-4">
      <div className="mb-2 text-(length:--text-micro) font-medium uppercase tracking-wide text-muted-foreground">
        {t("skillfoldertree.general.folders5")}</div>
      ...
      <div key={folder.id}>
        ...
        >
          ...
          {nestable ? (
            <button
              type="button"
              title={t("skillfoldertree.general.newfolderinside")}
      ...
      <DialogContent className="sm:max-w-lg">
        ...
        <div className="flex items-center gap-2 rounded-md border border-border px-2.5">
          ...
          <input
            ...
            placeholder={t("skillfoldertree.general.searchfolders")}
        ...
        <div className="max-h-72 overflow-y-auto rounded-md border border-border p-1">
          ...
          >
            ...
            <span className="min-w-0 flex-1 truncate">{t("skillfoldertree.general.unfiled6")}</span>
            {currentFolderId == null ? <span className="text-xs text-muted-foreground">{t("skillfoldertree.general.current7")}</span> : null}
          ...
          <div className="mt-1 border-t border-border pt-1">
            {creatingParent === null ? (
              ...
              >
                <Plus className="h-3.5 w-3.5" />
                {t("skillfoldertree.general.newtoplevelfolder")}</button>
            )}
          </div>
        </div>
        ...
        <div className="min-h-5 text-xs text-muted-foreground">
          {previewPath ? (
            <span>
              {t("skillfoldertree.general.movingto")}<span className="font-mono text-foreground">{previewPath}</span>
            </span>
          ) : (
            <span>{t("skillfoldertree.general.pickadestinationfolder")}</span>
          )}
        </div>

        <DialogFooter>
          <Button variant="ghost" onClick={() => onOpenChange(false)} disabled={pending}>
            {t("skillfoldertree.general.cancel")}</Button>
          <Button
            disabled={pending || chosen === undefined || chosen === currentFolderId}
            onClick={() => chosen !== undefined && onMove(chosen)}
          >
            {pending ? t("skillfoldertree.general.moving") : t("skillfoldertree.general.movehere")}
          </Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
  ...
  return (
    <div className="flex items-center gap-2 py-1" style={{ paddingLeft: `${0.5 + depth * 0.9}rem` }}>
      ...
      <Input
        ...
        placeholder={t("skillfoldertree.general.foldername")}
        autoFocus
      ...
      <Button size="sm" variant="ghost" onClick={onCancel} disabled={pending}>
        {t("skillfoldertree.general.cancel8")}</Button>
      <Button size="sm" onClick={onSubmit} disabled={pending || !value.trim()}>
        {t("skillfoldertree.general.add")}</Button>
    </div>
  );
}
