  ...
  return (
    <div className="space-y-4">
      <div className="space-y-1">
        <PropertyRow label={t("goalproperties.general.status")}>
          {onUpdate ? (
            <PickerButton
              current={goal.status}
        ...
        <PropertyRow label={t("goalproperties.general.level")}>
          {onUpdate ? (
            <PickerButton
              current={goal.level}
              options={GOAL_LEVELS}
        ...
        <PropertyRow label={t("goalproperties.general.owner")}>
          {ownerAgent ? (
            <Link
            ...
            <span className="text-sm text-muted-foreground">{t("goalproperties.general.none")}</span>
          )}
        </PropertyRow>

        {goal.parentId && (
          <PropertyRow label={t("goalproperties.general.parentgoal")}>
            <Link
      ...
      <div className="space-y-1">
        <PropertyRow label={t("goalproperties.general.created")}>
          <span className="text-sm">{formatDate(goal.createdAt)}</span>
        </PropertyRow>
        <PropertyRow label={t("goalproperties.general.updated")}>
