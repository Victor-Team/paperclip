...
export function ProfileSettings() {
  ...
  useEffect(() => {
    setBreadcrumbs([
      { label: t("profilesettings.general.settings"), href: "/company/settings" },
      { label: t("profilesettings.general.profile") },
    ]);
  }, [setBreadcrumbs, t]);
  ...
  const updateMutation = useMutation({
    ...
    onError: (error) => {
      setActionError(error instanceof Error ? error.message : t("profilesettings.general.failedToUpdateProfile"));
    },
  });
  ...
  const uploadAvatarMutation = useMutation({
    mutationFn: async (file: File) => {
      if (!selectedCompanyId) {
        throw new Error(t("profilesettings.general.selectAnOrganizationBefore"));
      }
    ...
    onError: (error) => {
      setActionError(error instanceof Error ? error.message : t("profilesettings.general.failedToUploadAvatar"));
    },
  });
  ...
  const removeAvatarMutation = useMutation({
    ...
    onError: (error) => {
      setActionError(error instanceof Error ? error.message : t("profilesettings.general.failedToRemoveAvatar"));
    },
  });

  if (sessionQuery.isLoading) {
    return <div className="text-sm text-muted-foreground">{t("profilesettings.general.loadingProfile")}</div>;
  }

  if (sessionQuery.error || !sessionQuery.data) {
    return (
      <div className="text-sm text-destructive">
        {sessionQuery.error instanceof Error ? sessionQuery.error.message : t("profilesettings.general.failedToLoadProfile")}
      </div>
    );
  }
  ...
  const uploadHint = selectedCompany
    ? t("profilesettings.general.storedInPaperclipFile", { name: selectedCompany.name })
    : t("profilesettings.general.selectAnOrganizationTo");
    ...
    <div className="max-w-6xl space-y-6">
      <div className="space-y-2">
        <div className="flex items-center gap-2">
          ...
          <h1 className="text-lg font-semibold">{t("profilesettings.general.profile")}</h1>
        </div>
        <p className="text-sm text-muted-foreground">
          {t("profilesettings.general.controlHowYourAccount")}
        </p>
      </div>
      ...
      <section className="space-y-8">
            ...
            <div className="flex flex-wrap items-end gap-5 rounded-(--rad-24) border border-border/70 bg-background/92 p-5 shadow-(--shadow-extract-18) backdrop-blur-sm">
              <div className="space-y-3">
                ...
                <div className="flex flex-wrap items-center gap-2">
                  ...
                  >
                    {uploadAvatarMutation.isPending ? <LoaderCircle className="size-4 animate-spin" /> : <Camera className="size-4" />}
                    {currentImage ? t("profilesettings.general.changePhoto") : t("profilesettings.general.uploadPhoto")}
                  </Button>
                  {currentImage ? (
                    <Button
                      type="button"
                    ...
                    >
                      ...
                      {t("profilesettings.general.remove")}
                    </Button>
                  ) : null}
                </div>
              </div>
              ...
              <div className="min-w-0 flex-1 space-y-2 pb-1">
                <div>
                  ...
                  <p className="truncate text-sm text-muted-foreground">{sessionQuery.data.user.email ?? t("profilesettings.general.noEmail")}</p>
                </div>
                <p className="max-w-2xl text-sm leading-6 text-muted-foreground">
                  {t("profilesettings.general.clickTheAvatarTo")} {uploadHint}
                </p>
              </div>
            </div>
          </div>
        ...
        >
          <div className="space-y-2">
            <Label htmlFor="profile-name">{t("profilesettings.general.displayName")}</Label>
            <Input
              id="profile-name"
              value={name}
          ...
          <div className="space-y-2">
            <Label htmlFor="profile-email">{t("profilesettings.general.email")}</Label>
            <Input
              id="profile-email"
            ...
            />
            <p className="text-xs text-muted-foreground">
              {t("profilesettings.general.emailIsManagedBy")}
            </p>
          </div>
          ...
          <div className="md:col-span-2 flex justify-end">
            <Button type="submit" disabled={isSavingProfile || !name.trim()}>
              ...
              {updateMutation.isPending ? t("profilesettings.general.saving") : t("profilesettings.general.saveProfile")}
            </Button>
          </div>
        </form>
