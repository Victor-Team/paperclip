...
export function Dashboard() {
  ...
  useEffect(() => {
    setBreadcrumbs([{ label: t("dashboard.general.dashboard") }]);
  }, [setBreadcrumbs, t]);
    ...
    if (companies.length === 0) {
      return (
        <EmptyState
          ...
          message={t("dashboard.general.welcomeMessage")}
          action={t("dashboard.general.getStarted")}
          onAction={openOnboarding}
        />
      );
    }
    return (
      <EmptyState icon={LayoutDashboard} message={t("dashboard.general.createOrSelectCompany")} />
    );
  }
        ...
        <InlineBanner
          ...
          title={t(
            pausedImportedCount === 1
              ? "dashboard.general.importedAgentsPausedSingular"
              : "dashboard.general.importedAgentsPausedPlural",
          ...
          actions={
            ...
            >
              {resumeImportedAgents.isPending ? t("dashboard.general.resuming") : t("dashboard.general.resumeAll")}
            </Button>
          }
        >
          {t("dashboard.general.agentsFromAnOrganization")}
        </InlineBanner>
      ) : pausedBanner?.kind === "all-paused" ? (
        <InlineBanner
          tone="warning"
          icon={PauseCircle}
          title={t("dashboard.general.titleAllagentsinthis")}
          actions={
            <Button variant="ghost" size="sm" asChild>
              <Link to="/agents">{t("dashboard.general.reviewAgents")}</Link>
            </Button>
          }
        >
          {t("dashboard.general.resumeAtLeastOne")}
        </InlineBanner>
      ) : null}
        ...
        <div className="flex items-center justify-between gap-3 rounded-md border border-amber-300 bg-amber-50 px-4 py-3 dark:border-amber-500/25 dark:bg-amber-950/60">
          <div className="flex items-center gap-2.5">
            ...
            <p className="text-sm text-amber-900 dark:text-amber-100">
              {t("dashboard.general.youHaveNoAgents")}
            </p>
          </div>
          <button
          ...
          >
            {t("dashboard.general.createOneHere")}
          </button>
        </div>
      )}
        ...
        <>
          {data.budgets.activeIncidents > 0 ? (
            <div className="flex items-start justify-between gap-3 rounded-xl border border-red-500/20 bg-(image:--gradient-extract-1) px-4 py-3">
                  ...
                  <p className="text-sm font-medium text-red-950 dark:text-red-50">
                    {t(
                      data.budgets.activeIncidents === 1
                        ? "dashboard.general.activeBudgetIncidentSingular"
                        : "dashboard.general.activeBudgetIncidentPlural",
                      { count: data.budgets.activeIncidents },
                    )}
                  </p>
                  <p className="text-xs text-red-900/70 dark:text-red-100/70">
                    {t("dashboard.general.budgetIncidentSummary", {
              ...
              <Link to="/costs" className="text-sm underline underline-offset-2 text-red-900 dark:text-red-100">
                {t("dashboard.general.openBudgets")}
              </Link>
            </div>
          ) : null}
          ...
          <div className="grid grid-cols-2 xl:grid-cols-4 gap-1 sm:gap-2">
            <MetricCard
              ...
              label={t("dashboard.general.labelAgentsenabled")}
              to="/agents"
              description={
                <span>
                  {t("dashboard.general.agentsBreakdown", {
            ...
            <MetricCard
              ...
              label={t("dashboard.general.labelTasksinprogress")}
              to="/issues"
              description={
                <span>
                  {t("dashboard.general.tasksBreakdown", { open: data.tasks.open, blocked: data.tasks.blocked })}
                </span>
              }
            />
            <MetricCard
              ...
              label={t("dashboard.general.labelMonthspend")}
              to="/costs"
              description={
                <span>
                  {data.costs.monthBudgetCents > 0
                    ? t("dashboard.general.budgetUtilization", {
                        percent: data.costs.monthUtilizationPercent,
                        budget: formatCents(data.costs.monthBudgetCents),
                      })
                    : t("dashboard.general.unlimitedBudget")}
                </span>
              }
            />
            <MetricCard
              ...
              label={t("dashboard.general.labelPendingapprovals")}
              to="/approvals"
              description={
                <span>
                  {data.budgets.pendingApprovals > 0
                    ? t("dashboard.general.budgetOverridesAwaiting", { count: data.budgets.pendingApprovals })
                    : t("dashboard.general.awaitingBoardReview")}
                </span>
              }
            />
          </div>
          ...
          <div className={cn("grid grid-cols-2 gap-4", SHOW_TASK_PRIORITY_UI ? "lg:grid-cols-4" : "lg:grid-cols-3")}>
            <ChartCard title={t("dashboard.general.titleRunactivity")} subtitle={t("dashboard.general.last14Days")}>
              <RunActivityChart activity={data.runActivity} />
            </ChartCard>
            {/* PAP-411: "Tasks by Priority" chart hidden behind SHOW_TASK_PRIORITY_UI. */}
            {SHOW_TASK_PRIORITY_UI && (
              <ChartCard title={t("dashboard.general.titleTasksbypriority")} subtitle={t("dashboard.general.last14Days")}>
                <PriorityChart issues={issues ?? []} />
              </ChartCard>
            )}
            <ChartCard title={t("dashboard.general.titleTasksbystatus")} subtitle={t("dashboard.general.last14Days")}>
              <IssueStatusChart issues={issues ?? []} />
            </ChartCard>
            <ChartCard title={t("dashboard.general.titleSuccessrate")} subtitle={t("dashboard.general.last14Days")}>
          ...
          <div className="grid md:grid-cols-2 gap-4">
            ...
            {recentActivity.length > 0 && (
              <div className="min-w-0">
                <h3 className="text-sm font-semibold text-muted-foreground uppercase tracking-wide mb-3">
                  {t("dashboard.general.recentActivity")}
                </h3>
            ...
            <div className="min-w-0">
              <h3 className="text-sm font-semibold text-muted-foreground uppercase tracking-wide mb-3">
                {t("dashboard.general.recentTasks")}
              </h3>
              {recentIssues.length === 0 ? (
                <Card className="block p-4">
                  <p className="text-sm text-muted-foreground">{t("dashboard.general.noTasksYet")}</p>
                </Card>
              ) : (
                <Card className="@container block py-0 divide-y divide-border overflow-hidden">
