        ...
        badge={
          showReleasePicker && pinnedRelease ? (
            <Badge variant="secondary" className="text-(length:--text-nano)">
              {t("agentskillstab.general.beta")} {releaseShortLabel(pinnedRelease)}
            </Badge>
          ) : undefined
        }
        accessory={
    ...
    <div className="max-w-4xl space-y-4">
      ...
      <div className="flex flex-col gap-3">
        <div className="flex flex-wrap items-center gap-2">
          <span className="text-sm font-medium text-foreground">
            {t("agentskillstab.general.enabledcount", { enabled: enabledRows.length, total: libraryRows.length })}</span>
          {applicationLabel ? (
            <Tooltip>
          ...
          <div className="ml-auto flex w-full flex-col items-stretch gap-2 sm:w-auto sm:flex-row sm:items-center">
            <div className="relative w-full sm:w-auto">
              ...
              <Input
                ...
                placeholder={t("agentskillstab.general.searchskills")}
                className="h-8 w-full pl-8 sm:w-56"
                aria-label={t("agentskillstab.general.searchskills1")}
              />
            </div>
            <Button asChild variant="outline" size="sm" className="shrink-0">
              <Link to="/skills" className="no-underline">
                <Store className="h-3.5 w-3.5" />
                {t("agentskillstab.general.browseskillsstore")}</Link>
            </Button>
          </div>
        </div>
        ...
        {syncSkills.isError ? (
          <p className="text-xs text-destructive">
            {syncSkills.error instanceof Error ? syncSkills.error.message : t("agentskillstab.general.failedtoupdateskills")}
          </p>
        ) : null}
      </div>
          ...
          {staleDesiredKeys.map((key) => (
            ...
            >
              <span className="min-w-0 truncate">
                <span className="font-medium">{key}</span> {t("agentskillstab.general.isenabledbutmissingfromtheorganization")}</span>
              <button
                type="button"
              ...
              >
                <X className="h-3 w-3" />
                {t("agentskillstab.general.remove")}</button>
            </div>
          ))}
        </div>
      ) : null}
      ...
      {isLoading ? (
        ...
        <div className="space-y-4">
          <SkillSection title={t("agentskillstab.general.enabledonthisagent")} count={filteredEnabled.length}>
            {filteredEnabled.length > 0 ? (
              filteredEnabled.map((row) => renderRow(row, "enabled"))
            ) : (
              <SectionEmpty>
                {search ? t("agentskillstab.general.noenabledskillsmatchyoursearch") : t("agentskillstab.general.noskillsenabledonthisagentyet")}
              </SectionEmpty>
            )}
          </SkillSection>

          <SkillSection title={t("agentskillstab.general.availablefromthelibrary")} count={filteredAvailable.length}>
            {filteredAvailable.length > 0 ? (
              filteredAvailable.map((row) => renderRow(row, "available"))
            ) : (
              <SectionEmpty>
                {search
                  ? t("agentskillstab.general.noavailableskillsmatchyoursearch")
                  : libraryEmpty
                    ? t("agentskillstab.general.importskillsintotheorganizationlibraryto")
                    : t("agentskillstab.general.everylibraryskillisenabledonthis")}
              </SectionEmpty>
            )}
          </SkillSection>
              ...
              <div className="overflow-hidden rounded-lg border border-border">
                <CollapsibleTrigger className="flex w-full items-center gap-2 bg-muted/50 px-3 py-2 text-left outline-none focus-visible:ring-2 focus-visible:ring-ring">
                  ...
                  <span className="text-xs font-medium text-muted-foreground">
                    {t("agentskillstab.general.automaticanddetectedskillsreadonly")}</span>
                ...
                <CollapsibleContent>
                  {filteredDetected.length > 0 ? (
                    ...
                    <SectionEmpty>{t("agentskillstab.general.nodetectedskillsmatchyoursearch")}</SectionEmpty>
                  )}
                </CollapsibleContent>
              </div>
            </Collapsible>
          ...
          <div className="text-xs text-muted-foreground">
            {t("agentskillstab.general.adapter")} {adapterLabels[agent.adapterType] ?? agent.adapterType}
          </div>
        </div>
      )}
    </div>
  ...
  if (pending) {
    return (
      <span className="inline-flex items-center gap-1.5 text-xs text-muted-foreground">
        ...
        {t("agentskillstab.general.saving")}</span>
    );
  }
  if (error) {
    return (
      <span className="inline-flex items-center gap-1.5 text-xs text-destructive">
        ...
        {t("agentskillstab.general.couldntsave")}</span>
    );
  }
  if (unsaved) {
    return (
  ...
  return (
    <div className="flex flex-col items-center gap-3 rounded-lg border border-dashed border-border px-6 py-10 text-center">
      ...
      <div className="space-y-1">
        <p className="text-sm font-medium text-foreground">{t("agentskillstab.general.noskillsintheorganizationlibrary")}</p>
        <p className="text-xs text-muted-foreground">
          {t("agentskillstab.general.installskillstotheorganizationthenenable")}</p>
      </div>
      <Button asChild variant="outline" size="sm">
        <Link to="/skills" className="no-underline">
          <Store className="h-3.5 w-3.5" />
          {t("agentskillstab.general.browseskillsstore2")}</Link>
      </Button>
    </div>
  );
}
