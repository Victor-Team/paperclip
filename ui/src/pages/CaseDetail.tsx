...
function CaseRelationshipsSection({
  ...
  return (
    <section className="space-y-3" aria-label={t("casedetail.general.caserelationships")}>
      {parent ? (
        <div className="space-y-1">
          <h2 className="text-xs font-medium text-muted-foreground">{t("casedetail.general.parent")}</h2>
          <CaseChildrenTree children={[parent]} />
        </div>
      ) : null}
      {children.length > 0 ? (
        <div className="space-y-1">
          <h2 className="text-xs font-medium text-muted-foreground">{t("casedetail.general.children")} {children.length}</h2>
    ...
    <Popover open={open} onOpenChange={setOpen}>
      <PopoverTrigger asChild>
        <button
          ...
          aria-label={t("casedetail.general.changecasestatus")}
        >
    ...
    <Popover open={open} onOpenChange={setOpen}>
      <PopoverTrigger asChild>
        <Button variant="ghost" size="sm" className="h-6 gap-1 px-2 text-xs text-muted-foreground">
          <Plus className="h-3.5 w-3.5" /> {t("casedetail.general.labels")}</Button>
      </PopoverTrigger>
      <PopoverContent align="start" className="w-64 p-2">
        <Input
          value={search}
          onChange={(e) => setSearch(e.target.value)}
          placeholder={t("casedetail.general.searchlabels")}
        ...
        <div className="max-h-52 space-y-0.5 overflow-y-auto">
          ...
          {filtered.length === 0 && !search.trim() && (
            <p className="px-2 py-1 text-xs text-muted-foreground">{t("casedetail.general.nolabelsyet")}</p>
          )}
        </div>
        {search.trim() && !all.some((l) => l.name.toLowerCase() === search.trim().toLowerCase()) && (
          <div className="mt-2 flex items-center gap-2 border-t border-border pt-2">
            <input
              ...
              aria-label={t("casedetail.general.newlabelcolor")}
            />
            <Button
              size="sm"
              variant="secondary"
            ...
            >
              {t("casedetail.general.create")}{search.trim()}”
            </Button>
          </div>
        )}
      </PopoverContent>
...
function CasePropertiesContent({
  ...
  return (
    <div className={cn("space-y-4", isFull && "space-y-6")}>
      <PropertySection title={t("casedetail.general.case")} first>
        <CasePropertyRow label={t("casedetail.general.type")} mode={mode}>
          <PropertyChip>{caseData.caseType}</PropertyChip>
        </CasePropertyRow>
        {caseData.key ? (
          <CasePropertyRow label={t("casedetail.general.key")} mode={mode}>
            <CaseCopyableToken
              value={caseData.key}
              label={t("casedetail.general.casekey")}
              className="font-mono text-xs text-muted-foreground"
              truncate={!isFull}
            />
          </CasePropertyRow>
        ) : null}
        <CasePropertyRow label={t("casedetail.general.labels1")} wrap mode={mode}>
          {caseData.labels.length > 0 ? (
            ...
            <span className="text-xs text-muted-foreground">{t("casedetail.general.none")}</span>
          )}
          {companyId ? (
            <CaseLabelsPicker
              companyId={companyId}
          ...
          {labelsPending ? <span className="text-xs text-muted-foreground">{t("casedetail.general.saving")}</span> : null}
        </CasePropertyRow>
      </PropertySection>

      {propertyRows.length > 0 ? (
        <PropertySection title={t("casedetail.general.fields")}>
      ...
      <PropertySection title={t("casedetail.general.linkedtasks")}>
        {caseData.issueLinks.length === 0 ? (
          <CasePropertyRow label={t("casedetail.general.tasks")} mode={mode}>
            <span className="text-xs text-muted-foreground">{t("casedetail.general.noneyet")}</span>
          </CasePropertyRow>
        ) : (
          <CasePropertyRow label={t("casedetail.general.tasks2")} wrap mode={mode}>
      ...
      {caseData.attachments.length > 0 ? (
        <PropertySection title={t("casedetail.general.attachments")}>
          <CasePropertyRow label={t("casedetail.general.files")} mode={mode}>
            <span className="text-xs text-muted-foreground">
              {caseData.attachments.length} {caseData.attachments.length === 1 ? t("casedetail.general.file") : t("casedetail.general.files3")}
            </span>
          </CasePropertyRow>
        </PropertySection>
      ) : null}
  ...
  if (caseQuery.isError || !caseData) {
    return (
      <div className="mx-auto max-w-md py-16 text-center">
        <p className="text-sm text-muted-foreground">{t("casedetail.general.casenotfound")}</p>
        <Link to={caseHref()} className="mt-2 inline-block text-sm text-primary hover:underline">
          {t("casedetail.general.backtocases")}</Link>
      </div>
    );
  }
    ...
    <div className="mx-auto max-w-3xl space-y-6">
      <header className="space-y-3">
        <div className="flex items-start justify-between gap-4">
          ...
          <div className="flex shrink-0 items-center gap-1">
            ...
            <Popover>
              <PopoverTrigger asChild>
                <Button variant="ghost" size="icon-xs" aria-label={t("casedetail.general.morecaseactions")} title={t("casedetail.general.morecaseactions4")}>
              ...
              <PopoverContent align="end" className="w-48 p-1">
                ...
                >
                  ...
                  {t("casedetail.general.copyasmarkdown")}</button>
                <button
                  type="button"
                  className="flex w-full items-center gap-2 rounded px-2 py-1.5 text-xs hover:bg-accent/50"
                  onClick={() => {
                ...
                >
                  ...
                  {t("casedetail.general.properties")}</button>
              </PopoverContent>
            </Popover>
          </div>
        </div>
      ...
      <Tabs defaultValue="overview" className="space-y-4">
        <TabsList variant="line" className="w-full justify-start gap-1">
          <TabsTrigger value="overview">{t("casedetail.general.overview")}</TabsTrigger>
          <TabsTrigger value="properties">{t("casedetail.general.properties5")}</TabsTrigger>
          <TabsTrigger value="activity">
            {t("casedetail.general.activity")}{events.length > 0 && <span className="ml-1 text-muted-foreground">{events.length}</span>}
          </TabsTrigger>
        </TabsList>
        ...
        <TabsContent value="overview" className="space-y-6">
          ...
          {description ? (
            <section className="space-y-2">
              <h2 className="text-sm font-semibold">{t("casedetail.general.description")}</h2>
          ...
          {caseData.attachments.length > 0 && (
            <section className="space-y-2">
              <h2 className="text-sm font-semibold">{t("casedetail.general.attachments6")}{caseData.attachments.length})</h2>
