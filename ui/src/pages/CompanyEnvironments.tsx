    ...
    <div className="mt-3 rounded-md border border-border/70 bg-background" data-testid={`custom-image-terminal-${sessionId}`}>
      <div className="flex flex-wrap items-center justify-between gap-2 border-b border-border/60 px-3 py-2">
        <div className="flex min-w-0 items-center gap-2 text-xs">
          ...
          <span className="font-medium">{t("companyenvironments.general.browserterminal")}</span>
        ...
        <div className="flex items-center gap-2">
          {terminalInteractive ? (
            <Button size="sm" variant="ghost" onClick={disconnectTerminal}>
              {t("companyenvironments.general.disconnect")}</Button>
          ) : (
            <Button
              size="sm"
              variant="outline"
            ...
            >
              ...
              {connectionState === "closed" || connectionState === "error" ? t("companyenvironments.general.reconnect") : t("companyenvironments.general.openterminal")}
            </Button>
          )}
        </div>
      </div>
      <div className="bg-neutral-950 p-2 focus-within:ring-2 focus-within:ring-ring">
        <div
          ...
          aria-label={t("companyenvironments.general.customimagebrowserterminal")}
          role="application"
          tabIndex={0}
  ...
  if (overviewQuery.isLoading) {
    return (
      <div className="mt-3 border-t border-border/60 pt-3 text-xs text-muted-foreground">
        {t("companyenvironments.general.loadingtemplatesetup")}</div>
    );
  }
  ...
  if (overviewQuery.isError) {
    return (
      <div className="mt-3 border-t border-border/60 pt-3 text-xs text-destructive">
        {overviewQuery.error instanceof Error ? overviewQuery.error.message : t("companyenvironments.general.templatesetupcouldnotbeloaded")}
      </div>
    );
  }
        ...
        <div className="flex flex-wrap items-start justify-between gap-3">
          ...
          <div className="flex flex-wrap items-center gap-2">
            ...
            >
              ...
              {t("companyenvironments.general.finished")}</Button>
            <Button
              size="sm"
              variant="ghost"
            ...
            >
              ...
              {t("companyenvironments.general.cancel")}</Button>
          </div>
        </div>
        {isCapturing ? (
          <div className="mt-2 text-xs text-muted-foreground">
            {t("companyenvironments.general.captureisinprogressifthisstate")}</div>
        ) : null}
        ...
        {session.status === "waiting_for_user" && connectionCommand ? (
          <details className="mt-2 rounded-md border border-border/60 bg-muted/20 px-3 py-2 text-xs text-muted-foreground">
            <summary className="cursor-pointer select-none font-medium text-foreground">
              {t("companyenvironments.general.sshcommandfallback")}</summary>
  ...
  if (activeTemplate) {
    ...
    return (
      <div className="mt-3 border-t border-border/60 pt-3" data-testid={`custom-image-template-state-${environment.id}`}>
        <div className="flex flex-wrap items-start justify-between gap-3">
          <div className="min-w-0 space-y-1">
            <div className="text-xs font-medium">{t("companyenvironments.general.activetemplate")}</div>
            <div className="text-xs text-muted-foreground">
            ...
            {templateOutOfSync ? (
              ...
              >
                {bootSourceDriftSummary
                  ...
                  : t("companyenvironments.general.notinusetheenvironmentconfigurationchanged")}
              </div>
            ) : null}
          </div>
          <div className="flex flex-wrap items-center gap-2">
            ...
            >
              ...
              {t("companyenvironments.general.refresh")}</Button>
            <Button
              size="sm"
              variant="outline"
            ...
            >
              ...
              {t("companyenvironments.general.relink")}</Button>
            <Button
              size="sm"
              variant="ghost"
            ...
            >
              ...
              {t("companyenvironments.general.rollback")}</Button>
            <Button
              size="sm"
              variant="ghost"
            ...
            >
              ...
              {t("companyenvironments.general.disable")}</Button>
          </div>
        </div>
      </div>
    );
    ...
    <div className="mt-3 border-t border-border/60 pt-3" data-testid={`custom-image-template-state-${environment.id}`}>
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div className="space-y-1">
          <div className="text-xs font-medium">{t("companyenvironments.general.notconfigured")}</div>
          <div className="text-xs text-muted-foreground">
            {latestSession
        ...
        >
          ...
          {t("companyenvironments.general.configureimage")}</Button>
      </div>
    </div>
  );
}
...
export function CompanyEnvironments({ mode = "list" }: CompanyEnvironmentsProps) {
  ...
  if (!selectedCompanyId) {
    return <div className="text-sm text-muted-foreground">{t("companyenvironments.general.selectanorganizationcontexttomanageenvironment")}</div>;
  }
    ...
    return (
      <div className="max-w-6xl space-y-4">
        <div className="text-sm text-muted-foreground">
          {t("companyenvironments.general.enableenvironmentsininstanceexperimentalsettingsto")}</div>
      </div>
    );
  }
  ...
  return (
    <div className="max-w-6xl space-y-6" data-testid="instance-settings-environments-section">
      ...
      <div className="space-y-4">
        <div className="flex flex-wrap items-center justify-between gap-3">
          <label className="flex flex-wrap items-center gap-3 text-sm font-medium">
            <span>{t("companyenvironments.general.default")}</span>
            <span>
              <select
                aria-label={t("companyenvironments.general.defaultenvironment")}
                ...
                {managedSandboxOnly ? (
                  ...
                  instanceDefaultEnvironmentId === "" ? (
                    <option value="" disabled>
                      {t("companyenvironments.general.selectenvironment")}</option>
                  ) : null
                ) : (
                  <option value="">{t("companyenvironments.general.local")}</option>
                )}
          ...
          <Button size="icon-sm" variant="ghost" asChild>
            <Link to={`${ENVIRONMENTS_PATH}/new`} aria-label={t("companyenvironments.general.addenvironment")} title={t("companyenvironments.general.addenvironment1")}>
                ...
                <div className="flex flex-wrap items-start justify-between gap-3">
                  <div className="space-y-1">
                    <div className="flex flex-wrap items-center gap-2 text-sm font-medium">
                      ...
                      {isPlatformManagedEnvironment(environment) ? (
                        <span className="inline-flex items-center gap-1 rounded-full bg-muted px-2 py-0.5 text-xs font-normal text-muted-foreground">
                          ...
                          {t("companyenvironments.general.managedbypaperclip")}</span>
                      ) : null}
                    </div>
                    ...
                    {environment.driver === "ssh" ? (
                      ...
                      <div className="text-xs text-muted-foreground">{t("companyenvironments.general.runsonthispapercliphost")}</div>
                    )}
                  </div>
                  <div className="flex flex-wrap items-center gap-2">
                    ...
                    <Button size="sm" variant="ghost" asChild>
                      <Link to={environmentEditPath(environment.id)}>{t("companyenvironments.general.edit")}</Link>
                    </Button>
                  </div>
                </div>
                {probe ? (
      ...
      {isEnvironmentFormPage && mode === "edit" && environments === undefined ? (
        <div className="text-sm text-muted-foreground">
          {t("companyenvironments.general.loadingenvironment")}</div>
      ) : null}

      {isEnvironmentFormPage && mode === "edit" && environments !== undefined && !editingEnvironment ? (
        <div className="space-y-3 text-sm">
          <div className="font-medium">{t("companyenvironments.general.environmentnotfound")}</div>
          <div className="text-muted-foreground">{t("companyenvironments.general.theenvironmentmayhavebeenremovedor")}</div>
          <Button size="sm" variant="outline" asChild>
            <Link to={ENVIRONMENTS_PATH}>{t("companyenvironments.general.backtoenvironments")}</Link>
          </Button>
        </div>
      ) : null}
      ...
      {isEnvironmentFormPage && mode === "edit" && editingEnvironment && isPlatformManagedEnvironment(editingEnvironment) ? (
        ...
        <div data-testid="managed-environment-form-page">
            ...
            <div className="mb-4">
              <Button size="sm" variant="ghost" asChild>
                <Link to={ENVIRONMENTS_PATH}>
                  ...
                  {t("companyenvironments.general.environments")}</Link>
              </Button>
            </div>
            <div className="flex flex-wrap items-center gap-2">
              ...
              <span className="inline-flex items-center gap-1 rounded-full bg-muted px-2 py-0.5 text-xs text-muted-foreground">
                <Lock className="h-3 w-3" aria-hidden />
                {t("companyenvironments.general.managedbypaperclip2")}</span>
            </div>
            <p className="mt-1 max-w-3xl text-sm text-muted-foreground">
              {editingEnvironment.description ?? t("companyenvironments.general.youragentrunsonacomputermanaged")}
            </p>
            <p className="mt-1 max-w-3xl text-xs text-muted-foreground">
              {t("companyenvironments.general.thisenvironmentisprovisionedandmaintainedfor")}</p>
          </div>
          <div className="py-4">
            <Field
            ...
            {managedEnvironmentEnvVarsMutation.isError ? (
              <div className="mt-3 text-xs text-destructive">
                {managedEnvironmentEnvVarsMutation.error instanceof Error
                  ...
                  : t("companyenvironments.general.failedtosaveenvironmentvariables")}
              </div>
            ) : null}
          </div>
          <div className="flex flex-wrap justify-end gap-2 py-4">
            ...
            >
              {t("companyenvironments.general.cancel3")}</Button>
            <Button
              onClick={() => managedEnvironmentEnvVarsMutation.mutate(flushEnvironmentForm().envVars)}
              disabled={managedEnvironmentEnvVarsMutation.isPending}
            >
              {managedEnvironmentEnvVarsMutation.isPending ? t("companyenvironments.general.saving") : t("companyenvironments.general.saveenvironmentvariables")}
            </Button>
          </div>
        </div>
      ...
      (mode === "create" || (editingEnvironment && !isPlatformManagedEnvironment(editingEnvironment))) ? (
        ...
        <div data-testid="environment-form-page">
          <div className="pb-4">
            <div className="mb-4 flex items-center justify-between gap-2">
              <Button size="sm" variant="ghost" asChild>
                <Link to={ENVIRONMENTS_PATH}>
                  <ArrowLeft className="mr-1.5 h-3.5 w-3.5" />
                  {t("companyenvironments.general.environments4")}</Link>
              </Button>
              {editingEnvironment ? (
                <Button
                  size="icon-sm"
                  ...
                  title={t("companyenvironments.general.deleteenvironment")}
            ...
            <h1 className="text-lg font-semibold">{editingEnvironmentId ? t("companyenvironments.general.editenvironment") : t("companyenvironments.general.addenvironment5")}</h1>
            <p className="mt-1 max-w-3xl text-sm text-muted-foreground">
              {t("companyenvironments.general.configureareusableexecutiontargetforyour")}</p>
          </div>
            ...
            <div className="space-y-4">
              ...
              <Field label={t("companyenvironments.general.driver")} hint="Sandbox stores plugin-backed provider config on the shared environment seam. SSH stores a remote machine target.">
                ...
                >
                  {sandboxCreationEnabled || environmentForm.driver === "sandbox" ? (
                    <option value="sandbox">{t("companyenvironments.general.sandbox")}</option>
                  ) : null}
                  <option value="ssh">SSH</option>
                  {environmentForm.driver === "local" ? (
                    <option value="local">{t("companyenvironments.general.local6")}</option>
                  ) : null}
                </select>
              </Field>
                  ...
                  */}
                  ...
                  <Field label={t("companyenvironments.general.privatekey")} hint="Optional PEM private key. Leave blank to rely on the server's SSH agent or default keychain.">
                    <div className="space-y-2">
                      ...
                      >
                        <option value="">{t("companyenvironments.general.nosavedsecret")}</option>
                        {(secrets ?? []).map((secret) => (
              ...
              {environmentForm.driver === "sandbox" ? (
                <div className="space-y-3">
                  ...
                  {selectedSandboxSchema ? (
                    ...
                    <div className="text-xs text-muted-foreground">
                      {t("companyenvironments.general.thisproviderdoesnotdeclareadditionalconfiguration")}</div>
                  )}
                  <ToggleField
              ...
              selectedCompanyId ? (
                <div className="space-y-2 py-3">
                  <div className="text-sm font-medium">{t("companyenvironments.general.customimage")}</div>
                  <div className="text-xs text-muted-foreground">
                    {t("companyenvironments.general.startasetupsandboxsshinto")}</div>
              ...
              {environmentMutation.isError ? (
                <div className="text-xs text-destructive">
                  {environmentMutation.error instanceof Error
                    ...
                    : t("companyenvironments.general.failedtosaveenvironment")}
                </div>
              ) : null}
          ...
          <div className="flex flex-wrap justify-end gap-2 py-4">
            ...
            >
              {t("companyenvironments.general.cancel8")}</Button>
            {environmentForm.driver !== "local" ? (
              <Button
                variant="outline"
              ...
              >
                {draftEnvironmentProbeMutation.isPending ? t("companyenvironments.general.testing") : t("companyenvironments.general.test")}
              </Button>
            ) : null}
            <Button
            ...
            >
              {environmentMutation.isPending
                ? editingEnvironmentId
                  ? t("companyenvironments.general.saving9")
                  : t("companyenvironments.general.creating")
                : editingEnvironmentId
                  ? t("companyenvironments.general.saveenvironment")
                  : t("companyenvironments.general.createenvironment")}
            </Button>
          </div>
          ...
          {editingEnvironment ? (
            ...
            >
              <AlertDialogContent data-testid="environment-delete-dialog">
                <AlertDialogHeader>
                  <AlertDialogTitle>{t("companyenvironments.general.delete")} {editingEnvironment.name}?</AlertDialogTitle>
                  <AlertDialogDescription>
                    {deleteUsageLoading
                      ? t("companyenvironments.general.checkingwhatusesthisenvironment")
                      : deleteUsageError
                        ? t("companyenvironments.general.couldnotcheckwhatusesthisenvironment")
                        : deleteBlockMessage
                          ?? ([
                ...
                {reusableLeaseHolderGroups.length > 0 ? (
                  <div className="space-y-1.5" data-testid="environment-delete-lease-holders">
                    <div className="text-xs font-medium text-muted-foreground">{t("companyenvironments.general.sandboxleasesheldby")}</div>
                    ...
                    <div className="text-xs text-muted-foreground">
                      {reusableLeaseOnlyBlock
                        ? t("companyenvironments.general.deletingdestroysthesesandboxestheworkspacesstay")
                        : t("companyenvironments.general.closetheseworkspacestoletpaperclipdestroy")}
                    </div>
                  </div>
                ) : null}
                  ...
                  <div className="space-y-3">
                    {agentsUsingEnvironment.length > 0 ? (
                      <label className="block space-y-1.5 text-sm">
                        ...
                        >
                          <option value="">
                            {t("companyenvironments.general.default10")} {instanceDefaultEnvironment
                              ? `${instanceDefaultEnvironment.name} · ${instanceDefaultEnvironment.driver}`
                              : t("companyenvironments.general.local11")}
                          </option>
                        ...
                        <span className="block text-xs text-muted-foreground">
                          {t("companyenvironments.general.affected")} {agentsUsingEnvironment.map((agent) => agent.name).join(", ")}
                        </span>
                      </label>
                    ) : null}
                ...
                <AlertDialogFooter>
                  <AlertDialogCancel disabled={deleteEnvironmentMutation.isPending}>{t("companyenvironments.general.cancel12")}</AlertDialogCancel>
                  <AlertDialogAction
                  ...
                  >
                    {deleteEnvironmentMutation.isPending
                      ? t("companyenvironments.general.deleting")
                      : reusableLeaseOnlyBlock && deleteBlastRadius
                        ? `Destroy ${deleteBlastRadius.reusableSandboxLeaseCount === 1 ? t("companyenvironments.general.1sandbox") : `${deleteBlastRadius.reusableSandboxLeaseCount} sandboxes`} and delete`
                        : t("companyenvironments.general.deleteenvironment13")}
                  </AlertDialogAction>
                </AlertDialogFooter>
              </AlertDialogContent>
            </AlertDialog>
