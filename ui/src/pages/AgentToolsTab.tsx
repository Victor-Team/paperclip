      ...
      <div className="flex flex-wrap items-start justify-between gap-3 px-3 py-3">
        <div className="flex min-w-0 items-start gap-3">
          ...
          <div className="min-w-0">
            <h3 className="text-sm font-semibold text-foreground">{t("agenttoolstab.general.githubidentity")}</h3>
            {loading ? (
              <p className="mt-0.5 text-xs text-muted-foreground">{t("agenttoolstab.general.checkinggithubidentity")}</p>
            ) : loadError ? (
              <>
                <p className="mt-0.5 text-sm font-medium text-foreground">{t("agenttoolstab.general.couldnotloadgithubidentity")}</p>
                <p className="mt-0.5 text-xs text-muted-foreground">
                  {t("agenttoolstab.general.paperclipcouldnotverifythisagentapos")}</p>
              </>
            ) : dedicatedIdentity ? (
              <>
                <p className="mt-0.5 text-sm font-medium text-foreground">
                  {dedicatedLogin ? `@${dedicatedLogin}` : t("agenttoolstab.general.dedicatedgithubaccount")}
                </p>
                <p className="mt-0.5 text-xs text-muted-foreground">
                  {t("agenttoolstab.general.usedonlyby")} {agentName}{t("agenttoolstab.general.thisdedicatedconnectiontakesprecedenceoverthe")}</p>
              </>
            ) : (
              <>
                <p className="mt-0.5 text-sm font-medium text-foreground">{t("agenttoolstab.general.useresponsiblepersonapossgithub")}</p>
                <p className="mt-0.5 text-xs text-muted-foreground">
                  {t("agenttoolstab.general.atrunstartpaperclipusesthepersonal")}</p>
                {personalIdentity ? (
                  <p className="mt-1 text-xs text-muted-foreground">
                    {t("agenttoolstab.general.youraccountisconnected")}{personalLogin ? ` as @${personalLogin}` : ""}.
                  </p>
                ) : null}
              </>
            )}
        ...
        {!loading ? (
          <div className="flex flex-wrap items-center gap-2">
            {loadError ? (
              <Button variant="outline" size="sm" onClick={onRetry}>{t("agenttoolstab.general.retry")}</Button>
            ) : dedicatedIdentity ? (
              <Button variant="outline" size="sm" asChild>
                <Link to={`/apps/${dedicatedIdentity.connection.id}/permissions`}>{t("agenttoolstab.general.managegithubidentity")}</Link>
              </Button>
            ) : (
              <>
                {personalIdentity ? (
                  <Button variant="outline" size="sm" asChild>
                    <Link to={`/apps/${personalIdentity.connection.id}/permissions`}>{t("agenttoolstab.general.managemygithub")}</Link>
                  </Button>
                ) : (
                  <Button size="sm" asChild>
                    <Link to="/apps/connect?source=github">{t("agenttoolstab.general.connectmygithub")}</Link>
                  </Button>
                )}
                <Button variant="outline" size="sm" asChild>
                  <Link to="/apps/connect?source=github">{t("agenttoolstab.general.useadedicatedaccount")}</Link>
                </Button>
              </>
            )}
          </div>
  ...
  return (
    <section className="rounded-lg border border-border bg-card">
      <div className="flex flex-wrap items-start justify-between gap-3 border-b border-border px-3 py-2.5">
        <div>
          <h3 className="text-sm font-semibold text-foreground">{t("agenttoolstab.general.installedapps")}</h3>
          <p className="mt-0.5 text-xs text-muted-foreground">
            {t("agenttoolstab.general.installedappsloadtoolsinto")} {agentName}{t("agenttoolstab.general.scontextoneveryrunpermittedonly")}</p>
        </div>
      ...
      <div className="space-y-3 p-3">
        <InlineBanner tone="info" compact>
          {t("agenttoolstab.general.hasaccessmeanstheappispermitted")}</InlineBanner>

        {connections.length === 0 ? (
          <p className="rounded-md border border-border bg-muted/30 px-3 py-4 text-sm text-muted-foreground">
            {t("agenttoolstab.general.nopermittedappsyetbindanaccess")}</p>
        ) : (
                  ...
                  <label className="flex items-start gap-3">
                    ...
                    <span className="min-w-0 flex-1">
                      <span className="flex flex-wrap items-center gap-2">
                        ...
                        {rowPending ? <span className="text-xs text-muted-foreground">{t("agenttoolstab.general.saving")}</span> : null}
                      </span>
                  ...
                  {permitted && !checked ? (
                    <InlineBanner
                      ...
                      actions={(
                        ...
                        >
                          {t("agenttoolstab.general.openpermissions")}</Link>
                      )}
                    >
                      {t("agenttoolstab.general.permittedbutnotinstalledtoolswillnot")}</InlineBanner>
                  ) : null}
                </div>
              );
            })}
...
function InstallSaveStatusChip({
  ...
  const { t } = useTranslation();
  if (pending) return <span className="text-xs text-muted-foreground">{t("agenttoolstab.general.saving1")}</span>;
  if (error) return <span className="text-xs text-destructive">{t("agenttoolstab.general.couldnotsave")}</span>;
  if (unsaved) return <span className="text-xs text-muted-foreground">{t("agenttoolstab.general.unsavedchanges")}</span>;
  return <span className="text-xs text-muted-foreground">{t("agenttoolstab.general.saved")}</span>;
}
...
export function AgentToolsTab({ agent, companyId }: { agent: AgentDetailRecord; companyId: string }) {
  ...
  if (effective.isLoading) return <ToolsLoadingState label={t("agenttoolstab.general.resolvingeffectiveaccess")} />;
  if (effective.error) {
  ...
  return (
    <div className="space-y-4">
      <EnforcementBanner
        tone="info"
        title={t("agenttoolstab.general.effectiveaccess")}
        body={
          <>
            {t("agenttoolstab.general.thisisexactlythetoolsetpaperclip")}{" "}
            <span className="font-medium">{agent.name}</span>{t("agenttoolstab.general.profileandpolicyeditsarereflectedwithin")}{" "}
            <span className="font-medium">{t("agenttoolstab.general.cannotexpandit")}</span> {t("agenttoolstab.general.everythingelseisblockedbydefault")}</>
        }
      />
      ...
      <div className="grid grid-cols-1 gap-4 lg:grid-cols-3">
        ...
        <div className="lg:col-span-2">
          <div className="rounded-lg border border-border">
            <div className="flex items-center justify-between gap-2 border-b border-border px-3 py-2.5">
              <h3 className="text-sm font-semibold text-foreground">{t("agenttoolstab.general.allowedtools")}</h3>
              <span className="text-xs text-muted-foreground tabular-nums">
                {allowedTools.length} {allowedTools.length === 1 ? t("agenttoolstab.general.tool") : t("agenttoolstab.general.tools")}
              </span>
            </div>
            {allowedTools.length === 0 ? (
              <p className="px-3 py-6 text-sm text-muted-foreground">
                {t("agenttoolstab.general.notoolsareallowedforthisagent")}</p>
            ) : (
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b border-border text-left text-xs text-muted-foreground">
                    <th className="px-3 py-2 font-medium">{t("agenttoolstab.general.tool2")}</th>
                    <th className="px-3 py-2 font-medium">{t("agenttoolstab.general.capability")}</th>
                    <th className="px-3 py-2 font-medium">{t("agenttoolstab.general.risk")}</th>
                    <th className="px-3 py-2 font-medium">{t("agenttoolstab.general.source")}</th>
                  </tr>
                </thead>
        ...
        <aside className="space-y-4">
          <div className="rounded-lg border border-border bg-background/60 p-3">
                      ...
                      {profile.summary.isCompanyDefault ? (
                        <div className="mt-1">
                          <span className="rounded border border-border px-1.5 py-0.5 text-(length:--text-nano) uppercase text-muted-foreground">
                            {t("agenttoolstab.general.organizationdefault")}</span>
                        </div>
                      ) : null}
                    </div>
                  );
            ...
            <div className="mt-3 space-y-1.5">
              <div className="text-(length:--text-micro) font-medium uppercase tracking-wide text-muted-foreground">
                {t("agenttoolstab.general.activepolicies")}</div>
              {policiesQuery.isLoading ? (
                <p className="text-xs text-muted-foreground">{t("agenttoolstab.general.loadingpolicies")}</p>
              ) : governingPolicies.length === 0 ? (
                <p className="text-xs text-muted-foreground">
                  {t("agenttoolstab.general.noenabledpolicycurrentlymutatesthisagent")}</p>
              ) : (
            ...
            <div className="mt-3 space-y-1.5">
              <div className="text-(length:--text-micro) font-medium uppercase tracking-wide text-muted-foreground">
                {t("agenttoolstab.general.unavailabletools")}</div>
              {catalogLoading ? (
                <p className="text-xs text-muted-foreground">{t("agenttoolstab.general.checkingtools")}</p>
              ) : deniedTools.length === 0 ? (
                <p className="text-xs text-muted-foreground">
                  {t("agenttoolstab.general.everyknowntoolthisagentcouldname")}</p>
              ) : (
                <>
                  <p className="text-(length:--text-micro) text-muted-foreground">
                    {t("agenttoolstab.general.toolstheagentcouldnamebutpaperclip")}</p>
                  ...
                  {deniedTools.length > DENIED_TOOLS_DISPLAY_LIMIT ? (
                    <p className="text-(length:--text-micro) text-muted-foreground">
                      +{deniedTools.length - DENIED_TOOLS_DISPLAY_LIMIT} {t("agenttoolstab.general.more")}</p>
                  ) : null}
                </>
              )}
            </div>
