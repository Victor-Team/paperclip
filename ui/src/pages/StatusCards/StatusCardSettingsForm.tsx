...
const TRIGGER_ROWS: { key: TriggerKey; labelKey: string; noisy?: boolean }[] = [
  { key: "statusTransitions", labelKey: "statuscardsettingsform.general.triggerstatus" },
  { key: "membershipChanges", labelKey: "statuscardsettingsform.general.triggermembership" },
  { key: "humanComments", labelKey: "statuscardsettingsform.general.triggercomments" },
  { key: "assigneeChanges", labelKey: "statuscardsettingsform.general.triggerassignee" },
  { key: "anyUpdate", labelKey: "statuscardsettingsform.general.triggeranyupdate", noisy: true },
];
...
export function StatusCardSettingsForm({
  ...
  const costEstimate = estimateStatusCardCost(policy, t, i18n.language);
  ...
  return (
    <div className="space-y-6">
      <section className="space-y-2">
        <h3 className="text-sm font-semibold">{t("statuscardsettingsform.general.autoupdatepolicy")}</h3>
        <div className="space-y-2">
          <RadioRow
            selected={policy.mode === "manual"}
            title={t("statuscardsettingsform.general.manualonlyupdateswhenipressrefresh")}
            badge={
              <span className="ml-auto rounded-full bg-muted px-2 py-0.5 text-(length:--text-nano) font-medium uppercase tracking-wide text-muted-foreground">
                {t("statuscardsettingsform.general.default")}</span>
            }
            onSelect={() => setMode("manual")}
          />
          <RadioRow
            selected={policy.mode === "interval"}
            title={t("statuscardsettingsform.general.onascheduleonlyifsomethingchanged")}
            onSelect={() => setMode("interval")}
          >
            <div className="flex items-center gap-2 text-sm text-muted-foreground">
              <span>{t("statuscardsettingsform.general.checkevery")}</span>
              <Select
                value={String(policy.intervalMinutes ?? 15)}
                onValueChange={(next) => setPolicy({ intervalMinutes: Number(next) })}
              >
                <SelectTrigger size="sm" className="w-28" aria-label={t("statuscardsettingsform.general.checkinterval")}>
                  <SelectValue />
                </SelectTrigger>
                <SelectContent>
                  {INTERVAL_OPTIONS.map((minutes) => (
                    <SelectItem key={minutes} value={String(minutes)}>
                      {minutes} {t("statuscardsettingsform.general.min")}</SelectItem>
                  ))}
                </SelectContent>
              </Select>
            </div>
          ...
          <RadioRow
            ...
            title={t("statuscardsettingsform.general.assoonassomethingchangesdebounced")}
            onSelect={() => setMode("reactive")}
          >
            <div className="flex flex-wrap items-center gap-2 text-sm text-muted-foreground">
              <span>{t("statuscardsettingsform.general.wait")}</span>
              <Select
                value={String(policy.debounceSeconds ?? 60)}
                onValueChange={(next) => setPolicy({ debounceSeconds: Number(next) })}
              >
                <SelectTrigger size="sm" className="w-24" aria-label={t("statuscardsettingsform.general.debounce")}>
                  <SelectValue />
                </SelectTrigger>
                <SelectContent>
                  {DEBOUNCE_OPTIONS.map((seconds) => (
                    <SelectItem key={seconds} value={String(seconds)}>
                      {seconds}{t("statuscardsettingsform.general.s")}</SelectItem>
                  ))}
                </SelectContent>
              </Select>
              <span className="text-xs">{t("statuscardsettingsform.general.afterthelastchangemax")}</span>
              <Input
                type="number"
                min={1}
                max={60}
                ...
                aria-label={t("statuscardsettingsform.general.maxupdatesperhour")}
              />
              <span className="text-xs">{t("statuscardsettingsform.general.updateshour")}</span>
            </div>
          </RadioRow>
        </div>
      </section>
      ...
      */}
      {autoUpdating ? (
        <Collapsible className="rounded-md border border-border">
          <CollapsibleTrigger className="group flex w-full items-center justify-between gap-2 px-3 py-2.5 text-sm font-semibold">
            {t("statuscardsettingsform.general.advanced")}<ChevronDown className="h-4 w-4 text-muted-foreground transition-transform group-data-[state=open]:rotate-180" />
          </CollapsibleTrigger>
          <CollapsibleContent className="space-y-6 border-t border-border px-3 py-3">
            <section className="space-y-2">
              <h3 className="text-sm font-semibold">{t("statuscardsettingsform.general.countasachange")}</h3>
              <div className="space-y-2">
            ...
            <section className="space-y-3">
              <h3 className="text-sm font-semibold">{t("statuscardsettingsform.general.guardrails")}</h3>
              <label className="flex items-start gap-2.5 text-sm">
                <Checkbox checked={Boolean(activeHours)} onCheckedChange={(checked) => setActiveHoursEnabled(Boolean(checked))} className="mt-0.5" aria-label={t("statuscardsettingsform.general.limittoactivehours")} />
                <span>{t("statuscardsettingsform.general.onlyautoupdateduringactivehours")}</span>
              </label>
              {activeHours ? (
                <div className="flex flex-wrap items-center gap-2 pl-6 text-sm">
                  <Input
                    ...
                    className="h-8 w-32"
                    aria-label={t("statuscardsettingsform.general.activehoursstart")}
                  />
                  ...
                  <Input
                    ...
                    className="h-8 w-32"
                    aria-label={t("statuscardsettingsform.general.activehoursend")}
                  />
                  <Input
                    value={activeHours.timezone}
                    onChange={(event) => setPolicy({ activeHours: { ...activeHours, timezone: event.target.value } })}
                    className="h-8 w-40"
                    placeholder={t("statuscardsettingsform.general.timezone")}
                    aria-label={t("statuscardsettingsform.general.activehourstimezone")}
                  />
                </div>
              ) : null}
              <div className="flex flex-wrap items-center gap-2 text-sm">
                <span className="w-32 shrink-0">{t("statuscardsettingsform.general.dailytokencap")}</span>
                <Input
                  type="number"
                  min={0}
                  step={1000}
                  ...
                  className="h-8 w-36"
                  placeholder={t("statuscardsettingsform.general.nocap")}
                  aria-label={t("statuscardsettingsform.general.dailytokencap1")}
                />
              </div>
            </section>
          </CollapsibleContent>
      ...
      <div className="flex items-center gap-2 text-sm">
        <span className="font-semibold">{t("statuscardsettingsform.general.estimatedcost")}</span>
        <span className="text-muted-foreground">=</span>
        <Tooltip>
          ...
          <TooltipContent className="max-w-(--sz-18rem) text-left">
            ...
            <p className="mt-1 opacity-80">{t("statuscardsettingsform.general.roughestimatefromtypicalupdatesizesactual")}</p>
          </TooltipContent>
        </Tooltip>
      </div>
    </div>
