  ...
  if (issueRef) {
    return (
      <Link to={`/issues/${issueRef}`} className="font-medium text-primary hover:underline">
        {issue?.identifier ? `${issue.identifier}${issue.title ? ` · ${issue.title}` : ""}` : t("auditfeedproduction.general.thetask")}
      </Link>
    );
  }
  if (document) {
        ...
        <div className="min-w-0 space-y-1">
          ...
          {documentKey ? (
            <p className="text-xs text-muted-foreground">
              {t("auditfeedproduction.general.document")}<span className="font-mono text-(length:--text-micro)">{documentKey}</span>
            </p>
          ) : null}
          <div className="flex flex-wrap items-center gap-2 text-xs text-muted-foreground">
            ...
            {record.runId && record.agentId ? (
              ...
              >
                {t("auditfeedproduction.general.viewrun")}</Link>
            ) : null}
  ...
  return (
    <Card>
      <CardContent className="flex flex-col items-center gap-3 py-14 text-center">
        ...
        <div>
          <p className="text-sm font-medium text-foreground">{t("auditfeedproduction.general.agentauditisapaperclipenterpriseview")}</p>
          <p className="mx-auto mt-1 max-w-md text-sm text-muted-foreground">
            {t("auditfeedproduction.general.theagentauditloggivesyoua")}{" "}
            <span className="font-mono text-(length:--text-micro)">{t("auditfeedproduction.general.auditviewagentactions")}</span>{" "}
            {t("auditfeedproduction.general.permissiontoviewit")}</p>
        </div>
      </CardContent>
    </Card>
  );
    ...
    <div className="space-y-1 md:space-y-4">
      {!hideHeader ? (
        <div className="flex flex-wrap items-start justify-between gap-3">
          <div>
            <h1 className="text-lg font-semibold text-foreground">{t("auditfeedproduction.general.activity")}</h1>
            <p className="mt-1 max-w-2xl text-sm text-muted-foreground">
              {resolvedMode === "agents"
                ? t("auditfeedproduction.general.everyrecordedagentactionnewestfirstwith")
                : t("auditfeedproduction.general.everythinghappeninginyourcompanynewestfirst")}
            </p>
          </div>
        </div>
      ) : null}
      ...
      {showModeToggle ? (
        <Tabs value={resolvedMode} onValueChange={(value) => onModeChange?.(value as AuditFeedMode)}>
          <TabsList aria-label={t("auditfeedproduction.general.activityscope")}>
            <TabsTrigger value="all">{t("auditfeedproduction.general.allactivity")}</TabsTrigger>
            <TabsTrigger value="agents">{t("auditfeedproduction.general.agentactions")}</TabsTrigger>
          </TabsList>
        </Tabs>
      ) : null}
        ...
        <div className="flex flex-wrap items-center gap-2">
          {!lockedAgentId ? (
            <Select value={agent} onValueChange={setAgent}>
              <SelectTrigger className="w-40">
                <SelectValue placeholder={t("auditfeedproduction.general.agent")} />
              </SelectTrigger>
              <SelectContent>
                <SelectItem value={ALL}>{t("auditfeedproduction.general.allagents")}</SelectItem>
                {(agents.data ?? []).map((a) => (
          ...
          <Select value={responsibleUser} onValueChange={setResponsibleUser}>
            ...
            <SelectTrigger className="w-52">
              <SelectValue placeholder={t("auditfeedproduction.general.responsibleuser")} />
            </SelectTrigger>
            <SelectContent>
              <SelectItem value={ALL}>{t("auditfeedproduction.general.allresponsibleusers")}</SelectItem>
          ...
          <Input
            type="date"
            aria-label={t("auditfeedproduction.general.fromdate")}
            value={dateFrom}
          ...
          <Input
            type="date"
            aria-label={t("auditfeedproduction.general.todate")}
            value={dateTo}
          ...
          {hasActiveFilters ? (
            <Button variant="ghost" size="sm" onClick={clearFilters}>
              {t("auditfeedproduction.general.clearfilters")}</Button>
          ) : null}
          <Button
            variant="outline"
            size="sm"
          ...
          >
            ...
            {exporting ? t("auditfeedproduction.general.exporting") : t("auditfeedproduction.general.exportcsv")}
          </Button>
        </div>
      ) : null}
      ...
      {recoveringFromAccessDowngrade || fallingBackToAllActivity ? (
        <Card>
          <CardContent className="py-14 text-center text-sm text-muted-foreground">
            {t("auditfeedproduction.general.refreshingauditaccess")}</CardContent>
        </Card>
      ) : feed.isLoading ? (
        <Card>
          <CardContent className="py-14 text-center text-sm text-muted-foreground">{t("auditfeedproduction.general.loading")}</CardContent>
        </Card>
      ) : feed.error ? (
        <Card>
          <CardContent className="flex flex-col items-center gap-3 py-6 text-center md:py-14">
            <p className="text-sm text-muted-foreground">
              {feed.error instanceof Error ? feed.error.message : t("auditfeedproduction.general.failedtoloadtheauditlog")}
            </p>
            <Button variant="outline" size="sm" onClick={() => feed.refetch()}>
              {t("auditfeedproduction.general.tryagain")}</Button>
          </CardContent>
        </Card>
      ) : items.length === 0 ? (
          ...
          <CardContent className="flex flex-col items-center gap-2 py-0 text-center md:gap-3 md:py-14">
            ...
            <div>
              <p className="text-sm font-medium text-foreground">
                {hasActiveFilters ? t("auditfeedproduction.general.noactionsmatchthesefilters") : t("auditfeedproduction.general.nothinghereyet")}
              </p>
              <p className="mt-1 max-w-md text-sm text-muted-foreground">
                {hasActiveFilters
                  ? t("auditfeedproduction.general.tryawiderdaterangeordifferent")
                  : resolvedMode === "agents"
                    ? t("auditfeedproduction.general.assoonasyouragentsstartdoing")
                    : t("auditfeedproduction.general.assoonasanyoneinyourcompany")}
              </p>
            </div>
            {hasActiveFilters ? (
              <Button variant="outline" size="sm" onClick={clearFilters}>
                {t("auditfeedproduction.general.clearfilters1")}</Button>
            ) : null}
          </CardContent>
        </Card>
      ) : (
      ...
      {feed.hasNextPage ? (
        <div className="flex justify-center">
          ...
          >
            {feed.isFetchingNextPage ? t("auditfeedproduction.general.loading2") : t("auditfeedproduction.general.loadmore")}
          </Button>
        </div>
      ) : null}

      <p className="text-xs text-muted-foreground">
        {t("auditfeedproduction.general.recordedbypaperclipentriescantbe")}</p>
    </div>
  );
}
