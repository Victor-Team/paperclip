  ...
  return (
    <Card>
      <CardHeader className="px-5 pt-5 pb-2">
        <CardTitle className="text-base">{t("costsproduction.general.financeledger")}</CardTitle>
        <CardDescription>
          {t("costsproduction.general.accountlevelchargesthatdonotmap")}</CardDescription>
      </CardHeader>
      <CardContent className="grid gap-3 px-5 pb-5 pt-2 sm:grid-cols-2 xl:grid-cols-4">
        <MetricTile
          label={t("costsproduction.general.debits")}
          value={formatCents(debitCents)}
          subtitle={t("costsproduction.general.eventcountinrange", { count: eventCount })}
          icon={ArrowUpRight}
        />
        <MetricTile
          label={t("costsproduction.general.credits")}
          value={formatCents(creditCents)}
          subtitle={t("costsproduction.general.refundsoffsetscreditreturns")}
          icon={ArrowDownLeft}
        />
        <MetricTile
          label={t("costsproduction.general.net")}
          value={formatCents(netCents)}
          subtitle={t("costsproduction.general.debitminuscredit")}
          icon={ReceiptText}
        />
        <MetricTile
          label={t("costsproduction.general.estimated")}
          value={formatCents(estimatedDebitCents)}
          subtitle={t("costsproduction.general.estimateddebitsnotinvoiced")}
          icon={Coins}
        />
      </CardContent>
    </Card>
...
export function Costs() {
  ...
  useEffect(() => {
    setBreadcrumbs([{ label: t("costsproduction.general.costsbreadcrumb") }]);
  }, [setBreadcrumbs, t]);
    ...
    return [
      {
        ...
        label: (
          <span className="flex items-center gap-1.5">
            <span>{t("costsproduction.general.allproviders")}</span>
    ...
    return [
      {
        ...
        label: (
          <span className="flex items-center gap-1.5">
            <span>{t("costsproduction.general.allbillers")}</span>
            {billerKeys.length > 0 ? (
              <>
                <span className="font-mono text-xs text-muted-foreground">{formatTokens(allTokens)}</span>
                <span className="text-xs text-muted-foreground">{formatCents(allCents)}</span>
  ...
  if (!selectedCompanyId) {
    return <EmptyState icon={DollarSign} message={t("costsproduction.general.selectcompanyforcosts")} />;
  }
    ...
    <div className="space-y-6">
      <div className="space-y-5">
          <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
            <div>
                <h1 className="text-3xl font-semibold tracking-tight">{t("costsproduction.general.costs")}</h1>
                <p className="mt-2 max-w-2xl text-sm leading-6 text-muted-foreground">
                  {t("costsproduction.general.inferencespendplatformfeescreditsandlive")}</p>
            </div>
            ...
            <div className="flex flex-wrap items-center gap-2">
              {PRESET_KEYS.map((key) => (
                ...
                >
                  {t(`costsproduction.general.preset${key}`, { defaultValue: PRESET_LABELS[key] })}
                </Button>
              ))}
            </div>
          </div>
          ...
          {preset === "custom" ? (
            <div className="flex flex-wrap items-center gap-2 border border-border p-3">
              <input
                type="date"
                aria-label={t("costsproduction.general.startdate")}
                value={customFrom}
                onChange={(event) => setCustomFrom(event.target.value)}
                className="h-9 rounded-md border border-input bg-background px-3 text-sm text-foreground"
              />
              <span className="text-sm text-muted-foreground">{t("costsproduction.general.to")}</span>
              <input
                type="date"
                aria-label={t("costsproduction.general.enddate")}
                value={customTo}
          ...
          <div className="grid gap-3 lg:grid-cols-4">
            <MetricTile
              label={t("costsproduction.general.inferencespend")}
              value={formatCents(spendData?.summary.spendCents ?? 0)}
              subtitle={t("costsproduction.general.requestscopedtokens", { count: formatTokens(inferenceTokenTotal) })}
              icon={DollarSign}
            />
            <MetricTile
              label={t("costsproduction.general.budget")}
              value={activeBudgetIncidents.length > 0 ? String(activeBudgetIncidents.length) : (
                spendData?.summary.budgetCents && spendData.summary.budgetCents > 0
                  ? `${spendData.summary.utilizationPercent}%`
                  : t("costsproduction.general.openbudget")
              )}
              subtitle={
                activeBudgetIncidents.length > 0
                  ? t("costsproduction.general.pausedagentsprojects", { agents: budgetData?.pausedAgentCount ?? 0, projects: budgetData?.pausedProjectCount ?? 0 })
                  : spendData?.summary.budgetCents && spendData.summary.budgetCents > 0
                    ? t("costsproduction.general.spentofbudget", { spent: formatCents(spendData.summary.spendCents), budget: formatCents(spendData.summary.budgetCents) })
                    : t("costsproduction.general.nomonthlycap")
              }
              icon={Coins}
            />
            <MetricTile
              label={t("costsproduction.general.financenet")}
              value={formatCents(financeData?.summary.netCents ?? 0)}
              subtitle={t("costsproduction.general.debitscredits", { debits: formatCents(financeData?.summary.debitCents ?? 0), credits: formatCents(financeData?.summary.creditCents ?? 0) })}
              icon={ReceiptText}
            />
            <MetricTile
              label={t("costsproduction.general.financeevents")}
              value={String(financeData?.summary.eventCount ?? 0)}
              subtitle={t("costsproduction.general.estimatedinrange", { amount: formatCents(financeData?.summary.estimatedDebitCents ?? 0) })}
              icon={ArrowUpRight}
            />
          </div>
      </div>
      ...
      <Tabs value={mainTab} onValueChange={(value) => setMainTab(value as typeof mainTab)}>
        <TabsList variant="line" className="justify-start">
          <TabsTrigger value="overview">{t("costsproduction.general.overview")}</TabsTrigger>
          <TabsTrigger value="budgets">{t("costsproduction.general.budgets")}</TabsTrigger>
          <TabsTrigger value="providers">{t("costsproduction.general.providers")}</TabsTrigger>
          <TabsTrigger value="billers">{t("costsproduction.general.billers")}</TabsTrigger>
          <TabsTrigger value="finance">{t("costsproduction.general.finance")}</TabsTrigger>
        </TabsList>

        <TabsContent value="overview" className="mt-4 space-y-4">
          {showCustomPrompt ? (
            <p className="text-sm text-muted-foreground">{t("costsproduction.general.selectastartandenddateto")}</p>
          ) : showOverviewLoading ? (
            ...
            <>
              ...
              <div className="grid gap-4 xl:grid-cols-(--gtc-31)">
                <Card>
                  <CardHeader className="px-5 pt-5 pb-2">
                    <CardTitle className="text-base">{t("costsproduction.general.inferenceledger")}</CardTitle>
                    <CardDescription>
                      {t("costsproduction.general.requestscopedinferencespendfortheselected")}</CardDescription>
                  </CardHeader>
                    ...
                    <div className="flex flex-wrap items-end justify-between gap-3">
                      <div>
                        ...
                        <div className="mt-1 text-sm text-muted-foreground">
                          {spendData?.summary.budgetCents && spendData.summary.budgetCents > 0
                            ? t("costsproduction.general.budgetamount", { amount: formatCents(spendData.summary.budgetCents) })
                            : t("costsproduction.general.unlimitedbudget")}
                        </div>
                      </div>
                      <div className="border border-border px-4 py-3 text-right">
                        <div className="text-(length:--text-micro) uppercase tracking-(--tracking-eyebrow) text-muted-foreground">{t("costsproduction.general.usage")}</div>
                    ...
                    {spendData?.summary.budgetCents && spendData.summary.budgetCents > 0 ? (
                      <div className="space-y-2">
                        ...
                        <div className="text-xs text-muted-foreground">
                          {t("costsproduction.general.monthlybudgetused", { percent: spendData.summary.utilizationPercent })}</div>
                      </div>
                    ) : null}
                  </CardContent>
                </Card>
              ...
              <div className="grid gap-4 xl:grid-cols-(--gtc-32)">
                <Card>
                  <CardHeader className="px-5 pt-5 pb-2">
                    <CardTitle className="text-base">{t("costsproduction.general.byagent")}</CardTitle>
                    <CardDescription>{t("costsproduction.general.whateachagentconsumedintheselected")}</CardDescription>
                  </CardHeader>
                  <CardContent className="space-y-2 px-5 pb-5 pt-2">
                    {(spendData?.byAgent.length ?? 0) === 0 ? (
                      <p className="text-sm text-muted-foreground">{t("costsproduction.general.nocosteventsyet")}</p>
                    ) : (
                          ...
                          <div key={row.agentId} className="border border-border px-4 py-3">
                            ...
                            >
                              ...
                              <div className="text-right text-sm tabular-nums">
                                ...
                                <div className="text-xs text-muted-foreground">
                                  {t("costsproduction.general.inputoutputtokens", { input: formatTokens(row.inputTokens + row.cachedInputTokens), output: formatTokens(row.outputTokens) })}
                                </div>
                                {(row.apiRunCount > 0 || row.subscriptionRunCount > 0) ? (
                                  <div className="text-xs text-muted-foreground">
                                    {t("costsproduction.general.apiruncount", { count: row.apiRunCount })}
                                    {" · "}
                                    {t("costsproduction.general.subscriptionruncount", { count: row.subscriptionRunCount })}
                                  </div>
                                ) : null}
                              </div>
                            </div>
                                    ...
                                    >
                                      <div className="min-w-0">
                                        ...
                                        <div className="truncate text-muted-foreground">
                                          {providerDisplayName(modelRow.biller)} · {t(`costsproduction.general.billingtype${modelRow.billingType.replaceAll("_", "")}`, { defaultValue: billingTypeDisplayName(modelRow.billingType) })}
                                        </div>
                                      </div>
                                      <div className="text-right tabular-nums">
                                        ...
                                        <div className="text-muted-foreground">
                                          {formatTokens(modelRow.inputTokens + modelRow.cachedInputTokens + modelRow.outputTokens)} {t("costsproduction.general.tok")}</div>
                                      </div>
                                    </div>
                                  );
                                })}
                ...
                <div className="space-y-4">
                  <Card>
                    <CardHeader className="px-5 pt-5 pb-2">
                      <CardTitle className="text-base">{t("costsproduction.general.byproject")}</CardTitle>
                      <CardDescription>{t("costsproduction.general.runcostsattributedthroughprojectlinkedtasks")}</CardDescription>
                    </CardHeader>
                    <CardContent className="space-y-2 px-5 pb-5 pt-2">
                      {(spendData?.byProject.length ?? 0) === 0 ? (
                        <p className="text-sm text-muted-foreground">{t("costsproduction.general.noprojectattributedruncostsyet")}</p>
                      ) : (
                        spendData?.byProject.map((row, index) => (
                          ...
                          >
                            <span className="truncate">{row.projectName ?? row.projectId ?? t("costsproduction.general.unattributed")}</span>
                  ...
                  <FinanceTimelineCard rows={topFinanceEvents.slice(0, 6)} emptyMessage={t("costsproduction.general.financeeventshelp")} />
                </div>
              </div>
            </>
          )}
            ...
            <>
              <Card className="border-border/70 bg-(image:--gradient-extract-2)">
                <CardHeader className="px-5 pt-5 pb-3">
                  <CardTitle className="text-base">{t("costsproduction.general.budgetcontrolplane")}</CardTitle>
                  <CardDescription>
                    {t("costsproduction.general.hardstopspendlimitsforagentsand")}</CardDescription>
                </CardHeader>
                <CardContent className="grid gap-3 px-5 pb-5 pt-0 md:grid-cols-4">
                  <MetricTile
                    label={t("costsproduction.general.activeincidents")}
                    value={String(activeBudgetIncidents.length)}
                    subtitle={t("costsproduction.general.thresholdcrossings")}
                    icon={ReceiptText}
                  />
                  <MetricTile
                    label={t("costsproduction.general.pendingapprovals")}
                    value={String(budgetData?.pendingApprovalCount ?? 0)}
                    subtitle={t("costsproduction.general.budgetapprovals")}
                    icon={ArrowUpRight}
                  />
                  <MetricTile
                    label={t("costsproduction.general.pausedagents")}
                    value={String(budgetData?.pausedAgentCount ?? 0)}
                    subtitle={t("costsproduction.general.agentheartbeatsblocked")}
                    icon={Coins}
                  />
                  <MetricTile
                    label={t("costsproduction.general.pausedprojects")}
                    value={String(budgetData?.pausedProjectCount ?? 0)}
                    subtitle={t("costsproduction.general.projectexecutionblocked")}
                    icon={DollarSign}
                  />
                </CardContent>
              </Card>
              ...
              {activeBudgetIncidents.length > 0 ? (
                <div className="space-y-3">
                  <div>
                    <h2 className="text-lg font-semibold">{t("costsproduction.general.activeincidents1")}</h2>
                    <p className="text-sm text-muted-foreground">
                      {t("costsproduction.general.resolvehardstopsherebyraisingthe")}</p>
                  </div>
                  <div className="grid gap-4 xl:grid-cols-2">
                ...
                {(["company", "agent", "project"] as const).map((scopeType) => {
                  ...
                  return (
                    <section key={scopeType} className="space-y-3">
                      <div>
                        <h2 className="text-lg font-semibold capitalize">{t(`costsproduction.general.${scopeType}budgets`)}</h2>
                        <p className="text-sm text-muted-foreground">
                          {scopeType === "company"
                            ? t("costsproduction.general.companymonthlypolicy")
                            : scopeType === "agent"
                              ? t("costsproduction.general.agentmonthlypolicies")
                              : t("costsproduction.general.projectlifetimepolicies")}
                        </p>
                      </div>
                      <div className="grid gap-4 xl:grid-cols-2">
                        {rows.map((summary) => (
                ...
                {budgetPolicies.length === 0 ? (
                  <Card>
                    <CardContent className="px-5 py-8 text-sm text-muted-foreground">
                      {t("costsproduction.general.nobudgetpoliciesyetsetagentand")}</CardContent>
                  </Card>
                ) : null}
              </div>
            </>
        ...
        <TabsContent value="providers" className="mt-4 space-y-4">
          {showCustomPrompt ? (
            <p className="text-sm text-muted-foreground">{t("costsproduction.general.selectastartandenddateto3")}</p>
          ) : (
            <>
              <Tabs value={effectiveProvider} onValueChange={setActiveProvider}>
                ...
                <TabsContent value="all" className="mt-4">
                  {providers.length === 0 ? (
                    <p className="text-sm text-muted-foreground">{t("costsproduction.general.nocosteventsinthisperiod")}</p>
                  ) : (
        ...
        <TabsContent value="billers" className="mt-4 space-y-4">
          {showCustomPrompt ? (
            <p className="text-sm text-muted-foreground">{t("costsproduction.general.selectastartandenddateto4")}</p>
          ) : (
            <>
              <Tabs value={effectiveBiller} onValueChange={setActiveBiller}>
                ...
                <TabsContent value="all" className="mt-4">
                  {billers.length === 0 ? (
                    <p className="text-sm text-muted-foreground">{t("costsproduction.general.nobillableeventsinthisperiod")}</p>
                  ) : (
                    <div className="grid gap-4 md:grid-cols-2">
                      {billers.map((biller) => {
        ...
        <TabsContent value="finance" className="mt-4 space-y-4">
          {showCustomPrompt ? (
            <p className="text-sm text-muted-foreground">{t("costsproduction.general.selectastartandenddateto5")}</p>
          ) : financeLoading ? (
            <PageSkeleton variant="costs" />
          ) : financeError ? (
            ...
            <>
              ...
              <div className="grid gap-4 xl:grid-cols-(--gtc-33)">
                <div className="space-y-4">
                  <Card>
                    <CardHeader className="px-5 pt-5 pb-2">
                      <CardTitle className="text-base">{t("costsproduction.general.bybiller")}</CardTitle>
                      <CardDescription>{t("costsproduction.general.accountlevelfinancialeventsgroupedbywho")}</CardDescription>
                    </CardHeader>
                    <CardContent className="grid gap-4 px-5 pb-5 pt-2 md:grid-cols-2">
                      {(financeData?.byBiller.length ?? 0) === 0 ? (
                        <p className="text-sm text-muted-foreground">{t("costsproduction.general.nofinanceeventsyet")}</p>
                      ) : (
