...
const SECTION_TITLES: Record<RoutineSectionKey, string> = {
  ...
  history: "routinedetail.general.sectionVersionHistory",
};
...
export function RoutineDetail() {
  ...
  const createSecret = useMutation({
    mutationFn: (input: { name: string; value: string }) => {
      if (!selectedCompanyId) throw new Error(t("routinedetail.general.selectCompanyForSecrets"));
  ...
  useEffect(() => {
    if (!routine) return;
    if (!triggerSetup) setBreadcrumbs([{ label: t("routinedetail.general.routinesBreadcrumb"), href: "/routines" }, { label: routine.title }]);
    if (!routineDefaults) return;
  ...
  if (!selectedCompanyId) {
    return <EmptyState icon={Repeat} message={t("routinedetail.general.selectCompanyToViewRoutines")} />;
  }
