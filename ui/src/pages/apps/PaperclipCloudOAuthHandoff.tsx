      ...
      <div className="flex max-w-lg items-start gap-3">
        ...
        <div className="min-w-0">
          <h1 className="text-xl font-bold tracking-tight">
            {failed
              ? t("paperclipcloudoauthhandoff.general.signincouldntcontinue")
              : t("paperclipcloudoauthhandoff.general.preparingsecuresignin")}
          </h1>
          <p className="mt-1 text-sm text-muted-foreground">
            {failed
              ? error ?? t("paperclipcloudoauthhandoff.general.couldntprepareprovider")
              : phase === "reauthenticating"
                ? t("paperclipcloudoauthhandoff.general.signinisbeingrefreshed")
                : t("paperclipcloudoauthhandoff.general.openingprovidersecurely")}
          </p>
          {failed ? (
            <div className="mt-6 flex items-center gap-2">
              <Button type="button" onClick={onRetry}>{t("paperclipcloudoauthhandoff.general.tryagain")}</Button>
              <Button type="button" variant="ghost" onClick={onCancel}>
                {t("paperclipcloudoauthhandoff.general.returntopaperclip")}
              </Button>
            </div>
          ) : null}
        </div>
...
export function PaperclipCloudOAuthHandoffPage() {
  ...
  const resume = useCallback(async () => {
    ...
    if (!handoff) {
      setPhase("error");
      setError(t("paperclipcloudoauthhandoff.general.signinexpired"));
      return;
    }
    setPhase("loading");
    setError(null);
    try {
      ...
      if (target.kind === "reauthentication") {
        setPhase("error");
        setError(t("paperclipcloudoauthhandoff.general.couldntrefreshsignin"));
        return;
      }
      clearPendingCloudHandoff();
      ...
      setPhase("error");
      setError(caught instanceof Error ? caught.message : t("paperclipcloudoauthhandoff.general.couldntpreparesecure"));
    }
  }, [t]);
