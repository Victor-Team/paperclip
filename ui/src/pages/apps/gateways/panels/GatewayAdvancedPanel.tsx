...
export function GatewayAdvancedPanel({
  ...
  const archiveMutation = useMutation({
    ...
    onSuccess: async () => {
      pushToast({
        title: t("gatewayadvancedpanel.general.gatewayarchived"),
        body: t("gatewayadvancedpanel.general.gatewayarchivedbody", { name: gateway.name }),
        tone: "success",
      });
    ...
    onError: (error) =>
      pushToast({
        title: t("gatewayadvancedpanel.general.couldntarchivethegateway"),
  ...
  async function copy(value: string, label: string) {
    try {
      ...
      pushToast({ title: t("gatewayadvancedpanel.general.copied"), body: label, tone: "success" });
    } catch {
      pushToast({
        title: t("gatewayadvancedpanel.general.copyfailed"),
        body: t("gatewayadvancedpanel.general.clipboardaccessisunavailable"),
        tone: "error",
      });
    }
  }
  ...
  return (
    <div className="space-y-5">
      <section className="space-y-2">
        <h3 className="text-sm font-semibold text-foreground">{t("gatewayadvancedpanel.general.transport")}</h3>
        <dl className="grid gap-x-6 gap-y-2 text-sm sm:grid-cols-2">
          <Row label={t("gatewayadvancedpanel.general.transport")} value="streamable_http" />
          <Row label={t("gatewayadvancedpanel.general.authentication")} value="bearer" />
          <Row label={t("gatewayadvancedpanel.general.protocolversion")} value="2025-03-26" />
          <Row label={t("gatewayadvancedpanel.general.publicid")} value={gateway.gatewayPublicId} mono />
        </dl>
        <div className="flex items-center gap-2">
          <code className="min-w-0 flex-1 truncate rounded-md bg-muted px-3 py-2 font-mono text-xs text-muted-foreground">
            {endpoint}
          </code>
          <Button variant="outline" size="sm" onClick={() => void copy(endpoint, t("gatewayadvancedpanel.general.endpointurl"))}>
            <Copy className="mr-1 h-3.5 w-3.5" />
            {t("gatewayadvancedpanel.general.copy")}
          </Button>
        </div>
      </section>
      ...
      <section className="space-y-2">
        <div className="flex items-center justify-between">
          <h3 className="text-sm font-semibold text-foreground">{t("gatewayadvancedpanel.general.rawconfiguration")}</h3>
          <Button variant="outline" size="sm" onClick={() => void copy(rawConfig, t("gatewayadvancedpanel.general.gatewayconfigjson"))}>
            <Copy className="mr-1 h-3.5 w-3.5" />
            {t("gatewayadvancedpanel.general.copyjson")}
          </Button>
        </div>
      ...
      <section className="space-y-2 rounded-lg border border-destructive/40 p-4">
        <h3 className="text-sm font-semibold text-destructive">{t("gatewayadvancedpanel.general.dangerzone")}</h3>
        <p className="text-sm text-muted-foreground">
          {t("gatewayadvancedpanel.general.archivingtakesthegatewayoffline")}
        </p>
        {confirming ? (
          <div className="space-y-2">
            <Input
              ...
              aria-label={t("gatewayadvancedpanel.general.typethegatewaynametoconfirmarchive")}
            />
            <div className="flex gap-2">
              ...
              >
                {archiveMutation.isPending
                  ? t("gatewayadvancedpanel.general.archiving")
                  : t("gatewayadvancedpanel.general.archivegateway")}
              </Button>
              <Button variant="ghost" size="sm" onClick={() => { setConfirming(false); setConfirmName(""); }}>
                {t("gatewayadvancedpanel.general.cancel")}
              </Button>
            </div>
          </div>
        ) : (
