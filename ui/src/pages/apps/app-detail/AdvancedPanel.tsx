    ...
    <section>
      <div className="flex items-center justify-between">
        <div className="flex items-start gap-3">
          ...
          <div>
            <h2 className="text-sm font-medium text-foreground">{t("advancedpanel.general.reconnect")}</h2>
            <p className="mt-0.5 text-xs text-muted-foreground">
              {canReplace ? t("advancedpanel.general.replacestoredcredential") : unavailableMessage}
            </p>
          </div>
        </div>
        {canReplace && !open && (
          <Button size="sm" variant="outline" onClick={() => setOpen(true)}>
            {t("advancedpanel.general.reconnect")}
          </Button>
        )}
      </div>
      {open && (
...
export function ReconnectCard({
  ...
  const reconnectOAuth = useMutation({
    ...
    onSuccess: async (start) => {
      try {
        ...
        pushToast({
          title: t("advancedpanel.general.couldntstartsignin"),
          body: error instanceof Error ? error.message : t("advancedpanel.general.pleasetryagain"),
          tone: "error",
        });
      }
    },
  ...
  const verifyVercel = useMutation({
    ...
    onSuccess: () => {
      pushToast({
        title: t("advancedpanel.general.vercelcredentialverified"),
        body: t("advancedpanel.general.connectionbackonline", { connection: humanizeConnectionDisplayName(connection) }),
        tone: "success",
      });
      onReconnected();
    },
    onError: (error) => pushToast({
      title: t("advancedpanel.general.credentialstillneedsattention"),
      body: error instanceof Error ? error.message : t("advancedpanel.general.reviewconnectorinvercelconnect"),
      tone: "error",
    }),
  });
    ...
    <div className="flex flex-col gap-4 rounded-lg border border-amber-500/50 bg-amber-500/10 px-4 py-3 sm:flex-row sm:items-center sm:justify-between">
      <div className="min-w-0">
        <h2 className="text-sm font-semibold text-amber-900 dark:text-amber-100">
          {methodUnavailable
            ? t("advancedpanel.general.connectionnolongerupported")
            : oauth
              ? t("advancedpanel.general.reconnectrequired")
              : t("advancedpanel.general.thisappneedsreconnecting")}
        </h2>
        <p className="mt-0.5 text-sm text-amber-800 dark:text-amber-200">
          {methodUnavailable
            ? t("advancedpanel.general.addsupportedconnection")
            : connection.healthMessage?.trim() || (oauth
            ? t("advancedpanel.general.authorizationexpired")
            : t("advancedpanel.general.keystoppedworking"))}
        </p>
      </div>
      <div className="shrink-0">
        {!canReconnect ? (
          <p className="text-sm text-amber-800 dark:text-amber-200">
            {reconnectUnavailableMessage ?? t("advancedpanel.general.nopermissiontoreconnectidentity")}
          </p>
        ) : methodUnavailable ? (
          <Button size="sm" variant="outline" asChild>
            <Link to={`/apps/connect?source=${encodeURIComponent(galleryEntry!.slug)}`}>
              {t("advancedpanel.general.addsupportedconnectionbutton")}
            </Link>
          </Button>
        ) : onReconnect ? (
          <Button size="sm" variant="outline" onClick={onReconnect}>{t("advancedpanel.general.reconnect")}</Button>
        ) : managedByVercel && !oauth ? (
          <div className="flex items-center gap-2">
            <Button type="button" size="sm" variant="outline" asChild>
              <a href="https://vercel.com/connect" target="_blank" rel="noreferrer">
                {t("advancedpanel.general.manageinvercel")} <ArrowUpRight className="ml-1.5 h-3.5 w-3.5" />
              </a>
            </Button>
            <Button
              type="button"
            ...
            >
              ...
              {t("advancedpanel.general.checkagain")}
            </Button>
          </div>
        ) : oauth ? (
          <Button
          ...
          >
            ...
            {reconnectOAuth.isPending ? t("advancedpanel.general.openingsignin") : t("advancedpanel.general.reconnect")}
          </Button>
        ) : (
...
function ReconnectForm({
  ...
  const reconnect = useMutation({
    ...
    onSuccess: (result) => {
      ...
      if (healthy) {
        pushToast({
          title: t("advancedpanel.general.reconnected"),
          body: t("advancedpanel.general.connectionbackonline", { connection: humanizeConnectionDisplayName(connection) }),
          tone: "success",
        });
        onReconnected();
        ...
        pushToast({
          title: t("advancedpanel.general.stillnotworking"),
          body: result.connection.healthMessage?.trim() || t("advancedpanel.general.keydidntcheckout"),
          tone: "error",
        });
      }
    },
    onError: (error) =>
      pushToast({
        title: t("advancedpanel.general.keydidntwork"),
        body: error instanceof Error ? error.message : t("advancedpanel.general.checkkeyandtryagain"),
        tone: "error",
      }),
  });
  ...
  if (connection.credentialSource === "vercel_connect") {
    return (
      <p className="text-sm text-muted-foreground">
        {t("advancedpanel.general.credentialsmanagedinvercelconnect")}
      </p>
    );
  }
    ...
    <div className="space-y-3">
      {usesGallery ? (
        fields.map((field) => (
          <div key={field.configPath}>
            ...
            {field.helpUrl && (
              ...
              >
                {t("advancedpanel.general.wheredofindthis")} <ArrowUpRight className="h-3 w-3" />
              </a>
            )}
          </div>
        ))
        ...
        <Input
          ...
          placeholder={t("advancedpanel.general.pasteyournewkey")}
      ...
      <div className="flex items-center gap-2">
        <Button size="sm" disabled={!filled || reconnect.isPending} onClick={() => reconnect.mutate()}>
          ...
          {reconnect.isPending ? t("advancedpanel.general.checking") : t("advancedpanel.general.checkandreconnect")}
        </Button>
        {onCancel && (
      ...
      <section>
        <CollapsibleTrigger asChild>
          <button type="button" className="flex w-full items-center gap-3 py-1 text-left">
            <span className="min-w-0 flex-1 text-sm font-medium text-foreground">{t("advancedpanel.general.connectiondetails")}</span>
            <ChevronRight
        ...
        <CollapsibleContent>
          <dl className="mt-4 grid gap-2 pb-2 text-xs sm:grid-cols-(--gtc-59)">
            <dt className="text-muted-foreground">{t("advancedpanel.general.address")}</dt>
            <dd className="break-all font-mono text-foreground">{connectionAddress(connection, t)}</dd>
            <dt className="text-muted-foreground">{t("advancedpanel.general.type")}</dt>
...
export function DangerZone({
  ...
  const unavailableMessage = credentialUnavailableMessage ?? t("advancedpanel.general.nopermissiontoreplaceidentity");
    ...
    >
      <section>
        <CollapsibleTrigger asChild>
          ...
          >
            <span className="min-w-0 flex-1 text-sm font-medium text-destructive">{t("advancedpanel.general.dangerzone")}</span>
            <ChevronRight
              className={cn("h-4 w-4 shrink-0 text-muted-foreground transition-transform", open && "rotate-90")}
            />
          </button>
          ...
          <div className="mt-3 divide-y divide-border border-t border-border">
            {connection && onToggleConnection ? (
              <div className="flex items-center justify-between gap-4 py-4">
                <h2 className="text-sm font-medium text-foreground">{t("advancedpanel.general.pauseconnection")}</h2>
                <ToggleSwitch
                  aria-label={t("advancedpanel.general.pauseconnection")}
                  checked={paused}
            ...
            {connection?.authKind === "oauth" && !methodUnavailable && (onReconnectIdentity || !canReplaceCredential) ? (
              <div className="flex flex-wrap items-center justify-between gap-3 py-4">
                <div>
                  <p className="text-sm font-medium text-foreground">{t("advancedpanel.general.reconnect")}</p>
                  <p className="text-xs text-muted-foreground">
                    {canReplaceCredential
                      ? t("advancedpanel.general.signintoagain", { identityProviderName })
                      : unavailableMessage}
                  </p>
                </div>
            ...
            {identityGrant?.capabilities?.canRevoke
              ...
              && onRevokeIdentity ? (
                <div className="flex flex-wrap items-center justify-between gap-3 py-4">
                  <div>
                    <p className="text-sm font-medium text-foreground">{t("advancedpanel.general.revokeidentity")}</p>
                    <p className="text-xs text-muted-foreground">
                      {t("advancedpanel.general.disconnectidentity")}
                    </p>
                  </div>
                  <Button
                    variant="destructive"
                  ...
                  >
                    {t("advancedpanel.general.revoke")}
                  </Button>
                </div>
            ) : null}
              ...
              {confirming ? (
                <div className="flex items-center gap-2">
                  ...
                  <Button variant="destructive" size="sm" onClick={onRemove} disabled={removing}>
                    ...
                    {t("advancedpanel.general.yesremoveit")}
                  </Button>
                </div>
              ) : (
                <Button variant="destructive" size="sm" onClick={() => setConfirming(true)}>
                  {t("advancedpanel.general.removeapp")}
                </Button>
              )}
            </div>
          </div>
...
export function connectionAddress(connection: ToolConnection, translate?: Translate): string {
  ...
  if (connection.transport === "local_stdio") return translate?.("advancedpanel.general.localcommand") ?? "Local command";
  return translate?.("advancedpanel.general.notset") ?? "Not set";
}

export function connectionTransportLabel(transport: ToolConnection["transport"], translate?: Translate): string {
  if (transport === "mcp_remote") return translate?.("advancedpanel.general.remotehttp") ?? "Remote HTTP";
  if (transport === "local_stdio") return translate?.("advancedpanel.general.localcommand") ?? "Local command";
  return translate?.("advancedpanel.general.unknown") ?? "Unknown";
}
