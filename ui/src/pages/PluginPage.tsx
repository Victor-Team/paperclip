  ...
  if (!resolvedCompanyId) {
    ...
    return (
      <div className="space-y-4">
        <p className="text-sm text-muted-foreground">{t("pluginpage.general.selectanorganizationtoviewthispage")}</p>
      </div>
    );
  }

  if (!contributions) {
    return <div className="text-sm text-muted-foreground">{t("pluginpage.general.loading")}</div>;
  }
    ...
    if (duplicateMatches.length > 1) {
      return (
        <div className="rounded-md border border-destructive/30 bg-destructive/5 px-3 py-2 text-sm text-destructive">
          {t("pluginpage.general.multiplepluginsdeclaretheroute")}<code>{pluginRoutePath}</code>{t("pluginpage.general.usethepluginidrouteuntilthe")}</div>
      );
    }
  }
        ...
        <div className="flex items-center gap-2">
          <Button variant="ghost" size="sm" asChild>
            <Link to={companyPrefix ? `/${companyPrefix}/dashboard` : "/dashboard"}>
              ...
              {t("pluginpage.general.back")}</Link>
          </Button>
        </div>
      )}
      <PluginSlotMount
