  ...
  return (
    <Card>
      <CardHeader className="px-5 pt-5 pb-2">
        <CardTitle className="text-base">{t("costs.general.financeledger")}</CardTitle>
        <CardDescription>
          {t("costs.general.accountlevelchargesthatdonotmap")}</CardDescription>
      </CardHeader>
    ...
    return [
      {
        ...
        label: (
          <span className="flex items-center gap-1.5">
            <span>{t("costs.general.allproviders")}</span>
    ...
    return [
      {
        ...
        label: (
          <span className="flex items-center gap-1.5">
            <span>{t("costs.general.allbillers")}</span>
            {billerKeys.length > 0 ? (
              <>
                <span className="font-mono text-xs text-muted-foreground">{formatTokens(allTokens)}</span>
                <span className="text-xs text-muted-foreground">{formatCents(allCents)}</span>
      ...
      {showSummaryChrome ? (
        <div className="space-y-5">
          <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
            <div>
              {embedded ? (
                <h2 className="text-lg font-semibold text-foreground">{t("costs.general.costs")}</h2>
              ) : (
                <h1 className="text-3xl font-semibold tracking-tight">{t("costs.general.costs1")}</h1>
              )}
              <p className="mt-2 max-w-2xl text-sm leading-6 text-muted-foreground">
                {t("costs.general.inferencespendplatformfeescreditsandlive")}</p>
            </div>
      ...
      <Tabs value={mainTab} onValueChange={(value) => setMainTab(value as typeof mainTab)}>
        {!lockTab ? (
          <TabsList variant="line" className="justify-start">
            <TabsTrigger value="overview">{t("costs.general.overview")}</TabsTrigger>
            {!hideBudgetsTab ? <TabsTrigger value="budgets">{t("costs.general.budgets")}</TabsTrigger> : null}
            <TabsTrigger value="providers">{t("costs.general.providers")}</TabsTrigger>
            <TabsTrigger value="billers">{t("costs.general.billers")}</TabsTrigger>
            <TabsTrigger value="finance">{t("costs.general.finance")}</TabsTrigger>
          </TabsList>
        ) : null}

        <TabsContent value="overview" className="mt-4 space-y-4">
          {showCustomPrompt ? (
            <p className="text-sm text-muted-foreground">{t("costs.general.selectastartandenddateto")}</p>
          ) : showOverviewLoading ? (
            ...
            <>
              ...
              <div className="grid gap-4 xl:grid-cols-(--gtc-31)">
                <Card>
                  <CardHeader className="px-5 pt-5 pb-2">
                    <CardTitle className="text-base">{t("costs.general.inferenceledger")}</CardTitle>
                    <CardDescription>
                      {t("costs.general.requestscopedinferencespendfortheselected")}</CardDescription>
                  </CardHeader>
                  <CardContent className="space-y-4 px-5 pb-5 pt-2">
                    <div className="flex flex-wrap items-end justify-between gap-3">
                      ...
                      <div className="border border-border px-4 py-3 text-right">
                        <div className="text-(length:--text-micro) uppercase tracking-(--tracking-eyebrow) text-muted-foreground">{t("costs.general.usage")}</div>
              ...
              <div className="grid gap-4 xl:grid-cols-(--gtc-32)">
                <Card>
                  <CardHeader className="px-5 pt-5 pb-2">
                    <CardTitle className="text-base">{t("costs.general.byagent")}</CardTitle>
                    <CardDescription>{t("costs.general.whateachagentconsumedintheselected")}</CardDescription>
                  </CardHeader>
                  <CardContent className="space-y-2 px-5 pb-5 pt-2">
                    {(spendData?.byAgent.length ?? 0) === 0 ? (
                      <p className="text-sm text-muted-foreground">{t("costs.general.nocosteventsyet")}</p>
                    ) : (
                                    ...
                                    >
                                      ...
                                      <div className="text-right tabular-nums">
                                        ...
                                        <div className="text-muted-foreground">
                                          {formatTokens(modelRow.inputTokens + modelRow.cachedInputTokens + modelRow.outputTokens)} {t("costs.general.tok")}</div>
                                      </div>
                                    </div>
                                  );
                                })}
                ...
                <div className="space-y-4">
                  <Card>
                    <CardHeader className="px-5 pt-5 pb-2">
                      <CardTitle className="text-base">{t("costs.general.byproject")}</CardTitle>
                      <CardDescription>{t("costs.general.runcostsattributedthroughprojectlinkedtasks")}</CardDescription>
                    </CardHeader>
                    <CardContent className="space-y-2 px-5 pb-5 pt-2">
                      {(spendData?.byProject.length ?? 0) === 0 ? (
                        <p className="text-sm text-muted-foreground">{t("costs.general.noprojectattributedruncostsyet")}</p>
                      ) : (
            ...
            <>
              <Card className="border-border/70 bg-(image:--gradient-extract-2)">
                <CardHeader className="px-5 pt-5 pb-3">
                  <CardTitle className="text-base">{t("costs.general.budgetcontrolplane")}</CardTitle>
                  <CardDescription>
                    {t("costs.general.hardstopspendlimitsforagentsand")}</CardDescription>
                </CardHeader>
              ...
              {activeBudgetIncidents.length > 0 ? (
                <div className="space-y-3">
                  <div>
                    <h2 className="text-lg font-semibold">{t("costs.general.activeincidents2")}</h2>
                    <p className="text-sm text-muted-foreground">
                      {t("costs.general.resolvehardstopsherebyraisingthe")}</p>
                  </div>
                  <div className="grid gap-4 xl:grid-cols-2">
                ...
                {budgetPolicies.length === 0 ? (
                  <Card>
                    <CardContent className="px-5 py-8 text-sm text-muted-foreground">
                      {t("costs.general.nobudgetpoliciesyetsetagentand")}</CardContent>
                  </Card>
                ) : null}
              </div>
            </>
        ...
        <TabsContent value="providers" className="mt-4 space-y-4">
          {showCustomPrompt ? (
            <p className="text-sm text-muted-foreground">{t("costs.general.selectastartandenddateto4")}</p>
          ) : (
            <>
              <Tabs value={effectiveProvider} onValueChange={setActiveProvider}>
                ...
                <TabsContent value="all" className="mt-4">
                  {providers.length === 0 ? (
                    <p className="text-sm text-muted-foreground">{t("costs.general.nocosteventsinthisperiod")}</p>
                  ) : (
        ...
        <TabsContent value="billers" className="mt-4 space-y-4">
          {showCustomPrompt ? (
            <p className="text-sm text-muted-foreground">{t("costs.general.selectastartandenddateto5")}</p>
          ) : (
            <>
              <Tabs value={effectiveBiller} onValueChange={setActiveBiller}>
                ...
                <TabsContent value="all" className="mt-4">
                  {billers.length === 0 ? (
                    <p className="text-sm text-muted-foreground">{t("costs.general.nobillableeventsinthisperiod")}</p>
                  ) : (
                    <div className="grid gap-4 md:grid-cols-2">
                      {billers.map((biller) => {
        ...
        <TabsContent value="finance" className="mt-4 space-y-4">
          {showCustomPrompt ? (
            <p className="text-sm text-muted-foreground">{t("costs.general.selectastartandenddateto6")}</p>
          ) : financeLoading ? (
            <PageSkeleton variant="costs" />
          ) : financeError ? (
            ...
            <>
              ...
              <div className="grid gap-4 xl:grid-cols-(--gtc-33)">
                <div className="space-y-4">
                  <Card>
                    <CardHeader className="px-5 pt-5 pb-2">
                      <CardTitle className="text-base">{t("costs.general.bybiller")}</CardTitle>
                      <CardDescription>{t("costs.general.accountlevelfinancialeventsgroupedbywho")}</CardDescription>
                    </CardHeader>
                    <CardContent className="grid gap-4 px-5 pb-5 pt-2 md:grid-cols-2">
                      {(financeData?.byBiller.length ?? 0) === 0 ? (
                        <p className="text-sm text-muted-foreground">{t("costs.general.nofinanceeventsyet")}</p>
                      ) : (
