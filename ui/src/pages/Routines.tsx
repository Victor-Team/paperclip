...
export function Routines() {
  ...
  useEffect(() => {
    setBreadcrumbs([{ label: t("routines.general.routineslabel") }]);
  }, [setBreadcrumbs, t]);
  ...
  const createRoutine = useMutation({
    ...
    onSuccess: async (routine) => {
      ...
      pushToast({
        title: t("routines.general.routinecreatedtoast"),
        body: routine.assigneeAgentId
          ? t("routines.general.addfirsttrigger")
          : t("routines.general.draftsavedaddagent"),
        tone: "success",
      });
    ...
    onSuccess: async (folder) => {
      ...
      if (moveAfterCreateIds.length > 0) {
        ...
        try {
          ...
          pushToast({
            title: t("routines.general.foldercreatedmovefailed"),
            body: moveError instanceof Error ? moveError.message : t("routines.general.couldnotmoveroutines"),
            tone: "error",
          });
          return;
        }
      ...
      pushToast({ title: t("routines.general.foldercreated"), body: folder.name, tone: "success" });
    },
    onError: (mutationError) => {
      pushToast({
        title: t("routines.general.failedsavefolder"),
        body: mutationError instanceof Error ? mutationError.message : t("routines.general.couldnotsavefolder"),
        tone: "error",
      });
    },
  });
  const updateFolder = useMutation({
    ...
    onError: (mutationError) => {
      pushToast({
        title: t("routines.general.foldersavefailed"),
        body: mutationError instanceof Error ? mutationError.message : t("routines.general.couldnotupdatefolder"),
        tone: "error",
      });
    },
  });
  const deleteFolder = useMutation({
    ...
    onSuccess: async (_, folderId) => {
      ...
      pushToast({ title: t("routines.general.folderdeleted"), body: t("routines.general.itemsmovedtounfiled"), tone: "success" });
    },
    onError: (mutationError) => {
      pushToast({
        title: t("routines.general.folderdeletefailed"),
        body: mutationError instanceof Error ? mutationError.message : t("routines.general.couldnotdeletefolder"),
        tone: "error",
      });
    },
  });
  const moveRoutineToFolder = useMutation({
    ...
    onError: (mutationError) => {
      pushToast({
        title: t("routines.general.movefailed"),
        body: mutationError instanceof Error ? mutationError.message : t("routines.general.couldnotmoveroutine"),
        tone: "error",
      });
    },
  });
  const updateRoutineStatus = useMutation({
    ...
    onError: (mutationError) => {
      pushToast({
        title: t("routines.general.failedupdateroutine"),
        body: mutationError instanceof Error ? mutationError.message : t("routines.general.couldnotupdateroutine"),
        tone: "error",
      });
    },
  });
  ...
  const runRoutine = useMutation({
    ...
    onError: (mutationError) => {
      pushToast({
        title: t("routines.general.routinerunfailed"),
        body: mutationError instanceof Error ? mutationError.message : t("routines.general.couldnotstartroutinerun"),
        tone: "error",
      });
    },
  });
  ...
  const recentRunsIssueLinkState = useMemo(
    () => createIssueDetailLocationState(t("routines.general.recentruns"), "/routines?tab=runs", "issues"),
    [t],
  );
  ...
  const routineSections = useMemo(
    () => buildRoutineSections(sortedRoutines, routineViewState.groupBy, projectById, agentById, folderById, (key, fallback) => t(`routines.general.${key}`, { defaultValue: fallback })),
  ...
  async function moveSelectedRoutines(folderId: string | null) {
    ...
    try {
      ...
      pushToast({ title: t("routines.general.routinesmoved"), body: t("routines.general.routinesfiled", { count: ids.length }), tone: "success" });
    } catch (moveError) {
      pushToast({
        title: t("routines.general.failedmoveroutines"),
        body: moveError instanceof Error ? moveError.message : t("routines.general.couldnotmoveroutines"),
        tone: "error",
      });
    }
  ...
  function handleToggleEnabled(routine: RoutineListItem, enabled: boolean) {
    if (!enabled && !routine.assigneeAgentId) {
      pushToast({
        title: t("routines.general.defaultagentrequired"),
        body: t("routines.general.setdefaultagent"),
        tone: "warn",
      });
      return;
    }
  ...
  if (!selectedCompanyId) {
    return <EmptyState icon={Repeat} message={t("routines.general.selectcompanytoroutines")} />;
  }
      ...
      <div className="space-y-6">
        <div className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
          <div className="space-y-1">
            <h1 className="text-2xl font-semibold tracking-tight">{t("routines.general.routines")}</h1>
            <p className="text-sm text-muted-foreground">
              {t("routines.general.recurringworkdefinitionsthatmaterializeintoauditable")}</p>
          </div>
          <Button onClick={openCreateRoutine}>
            <Plus className="mr-2 h-4 w-4" />
            {t("routines.general.createroutine")}</Button>
        </div>
        <Tabs value={activeTab} onValueChange={handleLegacyTabChange}>
          <PageTabBar
            ...
            items={[
              { value: "routines", label: t("routines.general.routineslabel") },
              { value: "runs", label: t("routines.general.recentruns") },
            ]}
          />
        </Tabs>
        <IssuesList
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div className="space-y-1">
          <h1 className="text-xl font-bold">{t("routines.general.routines1")}</h1>
          <p className="text-sm text-muted-foreground">
            {t("routines.general.recurringworkdefinitionsthatmaterializeintoauditable2")}</p>
        </div>
        <div className="flex items-center gap-2">
          {streamlinedUiEnabled ? (
            <Button variant="outline" asChild>
              <Link to={auditSectionHref("runs", {})}>{t("routines.general.viewallruns")}</Link>
            </Button>
          ) : null}
          <Button onClick={openCreateRoutine}>
            <Plus className="mr-2 h-4 w-4" />
            {t("routines.general.createroutine3")}</Button>
        </div>
      </div>
      ...
      <div className="flex flex-col gap-4">
        <div className="flex items-center justify-between gap-3">
          <p className="text-sm text-muted-foreground">
            {t("routines.general.routinecount", { count: visibleRoutines.length })}
          </p>
          <div className="flex items-center gap-1">
              <Popover>
                <PopoverTrigger asChild>
                  <Button variant="ghost" size="sm" className="text-xs" title={t("routines.general.sort")}>
                    <ArrowUpDown className="h-3.5 w-3.5 sm:h-3 sm:w-3 sm:mr-1" />
                    <span className="hidden sm:inline">{t("routines.general.sort4")}</span>
                  </Button>
                </PopoverTrigger>
                <PopoverContent align="end" className="w-44 p-0">
                  <div className="p-2 space-y-0.5">
                    {([
                      ["updated", t("routines.general.updatedlabel")],
                      ["created", t("routines.general.createdlabel")],
                      ["lastRun", t("routines.general.lastrunlabel")],
                      ["title", t("routines.general.titlelabel")],
                      ...
                      >
                        ...
                        {routineViewState.sortField === field ? (
                          <span className="text-xs text-muted-foreground">
                            {routineViewState.sortDir === "asc" ? t("routines.general.sortasc") : t("routines.general.sortdesc")}
                          </span>
                        ) : null}
                      </button>
                    ))}
              ...
              <Popover>
                <PopoverTrigger asChild>
                  <Button variant="ghost" size="sm" className="text-xs" title={t("routines.general.group")}>
                    <Layers className="h-3.5 w-3.5 sm:h-3 sm:w-3 sm:mr-1" />
                    <span className="hidden sm:inline">{t("routines.general.group5")}</span>
                  </Button>
                </PopoverTrigger>
                <PopoverContent align="end" className="w-44 p-0">
                  <div className="p-2 space-y-0.5">
                    {([
                      ["folder", t("routines.general.folderlabel")],
                      ["project", t("routines.general.projectlabel")],
                      ["assignee", t("routines.general.agentlabel")],
                      ["none", "None"],
              ...
              {routineViewState.groupBy === "folder" && !hasRoutineFolders ? (
                <Button variant="outline" size="sm" onClick={() => openCreateFolder()}>
                  ...
                  {t("routines.general.newfolder")}</Button>
              ) : null}
              {showFolderRail ? (
                <Button variant="ghost" size="sm" className="text-xs" onClick={() => setSelectMode((current) => !current)}>
                  {selectMode ? t("routines.general.done") : t("routines.general.select")}
                </Button>
              ) : null}
          </div>
        </div>
        {routineViewState.groupBy === "folder" ? (
          <div className="md:hidden">
            <FolderChip
              ...
              allLabel={t("routines.general.allroutineslabel")}
      ...
      >
        ...
        >
          <div className="shrink-0 flex flex-wrap items-center justify-between gap-3 border-b border-border/60 px-5 py-3">
            <div>
              <p className="text-xs font-medium uppercase tracking-(--tracking-caps) text-muted-foreground">{t("routines.general.newroutine")}</p>
              <p className="text-sm text-muted-foreground">
                {t("routines.general.definetherecurringworkfirstdefaultproject")}</p>
            </div>
            <Button
              variant="ghost"
              size="sm"
            ...
            >
              {t("routines.general.cancel")}</Button>
          </div>
          ...
          <div className="min-h-0 flex-1 overflow-y-auto">
            <div className="px-5 pt-5 pb-3">
              <textarea
                ...
                placeholder={t("routines.general.routinetitle")}
                rows={1}
                value={draft.title}
                onChange={(event) => {
            ...
            <div className="px-5 pb-3">
              <div className="overflow-x-auto overscroll-x-contain">
                <div className="inline-flex min-w-full flex-wrap items-center gap-2 text-sm text-muted-foreground sm:min-w-max sm:flex-nowrap">
                  <span>{t("routines.general.for")}</span>
                  <InlineEntitySelector
                    ref={assigneeSelectorRef}
                    value={draft.assigneeAgentId}
                    options={assigneeOptions}
                    recentOptionIds={recentAssigneeIds}
                    placeholder={t("routines.general.responsible")}
                    noneLabel={t("routines.general.noresponsible")}
                    searchPlaceholder={t("routines.general.searchresponsible")}
                    emptyMessage={t("routines.general.noresponsiblefound")}
                    ...
                    renderTriggerValue={(option) =>
                      option ? (
                        ...
                        <span className="text-muted-foreground">{t("routines.general.responsible6")}</span>
                      )
                    }
                    renderOption={(option) => {
                  ...
                  />
                  <span>{t("routines.general.in")}</span>
                  <InlineEntitySelector
                    ref={projectSelectorRef}
                    value={draft.projectId}
                    options={projectOptions}
                    recentOptionIds={recentProjectIds}
                    placeholder={t("routines.general.project")}
                    noneLabel={t("routines.general.noprojectlabel")}
                    searchPlaceholder={t("routines.general.searchprojects")}
                    emptyMessage={t("routines.general.noprojectsfound")}
                    onChange={(projectId) => {
                    ...
                    renderTriggerValue={(option) =>
                      option && currentProject ? (
                        ...
                        <span className="text-muted-foreground">{t("routines.general.project7")}</span>
                      )
                    }
                    renderOption={(option) => {
                      if (!option.id) return <span className="truncate">{option.label}</span>;
                  ...
                  />
                  <span>{t("routines.general.filedin")}</span>
                  <Select
                  ...
                  >
                    ...
                    <SelectContent>
                      <SelectItem value="__unfiled">{t("routines.general.unfiled")}</SelectItem>
            ...
            <div className="border-t border-border/60 px-5 py-4">
              <MarkdownEditor
                ...
                placeholder={t("routines.general.addinstructions")}
                bordered={false}
            ...
            <div className="border-t border-border/60 px-5 py-3">
              <Collapsible open={advancedOpen} onOpenChange={setAdvancedOpen}>
                <CollapsibleTrigger className="flex w-full items-center justify-between text-left">
                  <div>
                    <p className="text-sm font-medium">{t("routines.general.advanceddeliverysettings")}</p>
                    <p className="text-sm text-muted-foreground">{t("routines.general.keeppolicycontrolssecondarytothework")}</p>
                  </div>
                ...
                <CollapsibleContent className="pt-3">
                  <div className="grid gap-4 md:grid-cols-2">
                    <div className="space-y-2">
                      <p className="text-xs font-medium uppercase tracking-(--tracking-caps) text-muted-foreground">{t("routines.general.concurrency")}</p>
                      <Select
                      ...
                      >
                        ...
                        <SelectContent>
                          {concurrencyPolicies.map((value) => (
                            <SelectItem key={value} value={value}>{t(`routines.general.policy${value.replaceAll("_", "")}`, { defaultValue: value.replaceAll("_", " ") })}</SelectItem>
                          ))}
                        </SelectContent>
                      </Select>
                      <p className="text-xs text-muted-foreground">{t(`routines.general.policydescription${draft.concurrencyPolicy.replaceAll("_", "")}`, { defaultValue: concurrencyPolicyDescriptions[draft.concurrencyPolicy] })}</p>
                    </div>
                    <div className="space-y-2">
                      <p className="text-xs font-medium uppercase tracking-(--tracking-caps) text-muted-foreground">{t("routines.general.catchup")}</p>
                      <Select
                      ...
                      <p className="text-xs text-muted-foreground">{t(`routines.general.policydescription${draft.catchUpPolicy.replaceAll("_", "")}`, { defaultValue: catchUpPolicyDescriptions[draft.catchUpPolicy] })}</p>
                    </div>
                  </div>
                </CollapsibleContent>
              </Collapsible>
          ...
          <div className="shrink-0 flex flex-col gap-3 border-t border-border/60 px-5 py-4 sm:flex-row sm:items-center sm:justify-between">
            <div className="text-sm text-muted-foreground">
              {t("routines.general.aftercreationpapercliptakesyoustraightto")}</div>
            <div className="flex flex-col gap-2 sm:items-end">
              ...
              >
                <Plus className="mr-2 h-4 w-4" />
                {createRoutine.isPending ? t("routines.general.creating") : t("routines.general.createroutine8")}
              </Button>
              {createRoutine.isError ? (
                <p className="text-sm text-destructive">
                  {createRoutine.error instanceof Error ? createRoutine.error.message : t("routines.general.failedtocreateroutine")}
                </p>
              ) : null}
            </div>
          </div>
      ...
      {error ? (
        <Card>
          <CardContent className="pt-6 text-sm text-destructive">
            {error instanceof Error ? error.message : t("routines.general.failedtoloadroutines")}
          </CardContent>
        </Card>
      ) : null}
      ...
      <div className={cn(showFolderRail && "flex gap-4")}>
          {showFolderRail ? (
            <FolderRail
              ...
              selection={folderSelection}
              allLabel={t("routines.general.allroutineslabel")}
              itemLabelPlural={t("routines.general.routineslabel")}
          ...
          {routineViewState.groupBy === "folder" && hasRoutineFolders ? (
            <div className="mb-3 flex flex-wrap items-center gap-2">
              {folderSelection === "all" ? <FolderIconHeader label={t("routines.general.allroutines")} count={sortedRoutines.length} /> : (
                <div className="flex min-w-0 items-center gap-2 text-sm">
                  <FolderSwatch color={activeFolder?.color} />
                  <span className="truncate font-medium">{folderSelection === "unfiled" ? t("routines.general.unfiled9") : activeFolder?.name ?? t("routines.general.folder")}</span>
                  <span className="text-muted-foreground">{t("routines.general.routinecount", { count: sortedRoutines.length })}</span>
                </div>
              )}
            </div>
          ) : null}
          ...
          {visibleRoutines.length === 0 ? (
            <div className="py-12">
              <EmptyState
                icon={Repeat}
                message={t("routines.general.noactiveroutines")}
              />
            </div>
            ...
            <div className="py-12">
              <EmptyState
                icon={Repeat}
                message={folderSelection === "all" ? t("routines.general.noroutinesmatch") : t("routines.general.folderempty")}
              />
              {folderSelection !== "all" ? (
                <div className="mt-3 flex justify-center">
                  <Button size="sm" onClick={openCreateRoutine}>
                    <Plus className="mr-2 h-3.5 w-3.5" />
                    {t("routines.general.newroutineinthisfolder")}</Button>
                </div>
              ) : null}
            </div>
          ) : (
                          ...
                          extraMenuItems={
                            <MoveToMenu
                              ...
                              onMove={(folderId) => {
                                ...
                                pushToast({
                                  title: t("routines.general.routinemoved"),
                                  body: folderId
                                    ? t("routines.general.routinemovedtofolder", { title: routine.title, folder: routineFolders?.folders.find((folder) => folder.id === folderId)?.name ?? t("routines.general.folderfallback") })
                                    : t("routines.general.routinemovedtounfiled", { title: routine.title }),
                                  tone: "success",
                                  action: {
                                    label: t("routines.general.undo"),
...
function FolderIconHeader({ label, count }: { label: string; count: number }) {
  ...
  return (
    <div className="flex min-w-0 items-center gap-2 text-sm">
      ...
      <span className="text-muted-foreground">{t("routines.general.routinecount", { count })}</span>
    </div>
  );
}
