    ...
    <Dialog open={open} onOpenChange={setOpen}>
      <DialogTrigger asChild>
        <Button
          ...
          aria-label={t("mcpconfighelpdialog.general.gethelpcreatinganmcpconfig")}
        >
      ...
      <DialogContent className="max-h-(--sz-85vh) overflow-y-auto sm:max-w-2xl">
        <DialogHeader>
          <DialogTitle>{t("mcpconfighelpdialog.general.askanagentforanmcpconfig")}</DialogTitle>
          <DialogDescription>
            {t("mcpconfighelpdialog.general.dontknowtheurlorheaders")}</DialogDescription>
        </DialogHeader>
        ...
        <div className="space-y-2">
          <label htmlFor="mcp-config-help-prompt" className="text-sm font-medium text-foreground">
            {t("mcpconfighelpdialog.general.prompttosend")}</label>
          <Textarea
        ...
        <div className="flex flex-wrap items-center gap-3">
          <Button type="button" onClick={() => void copyPrompt()}>
            ...
            {t("mcpconfighelpdialog.general.copyprompt")}</Button>
          {/* aria-live so a screen reader hears the outcome without moving focus
              off the button the operator just pressed. */}
          <span aria-live="polite" className="text-xs text-muted-foreground">
            {copyState === "copied"
              ? t("mcpconfighelpdialog.general.copiedtoclipboard")
              : copyState === "failed"
                ? t("mcpconfighelpdialog.general.couldntcopyautomaticallyselectthetext")
                : null}
          </span>
        </div>
      </DialogContent>
