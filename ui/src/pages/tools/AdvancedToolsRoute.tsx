...
export function AdvancedToolsRoute() {
  ...
  if (boardAccess.isLoading) {
    return <div className="mx-auto max-w-xl py-10 text-sm text-muted-foreground">{t("advancedtoolsroute.general.loading")}</div>;
  }
      ...
      <div className="mx-auto max-w-xl py-10">
        <div className="flex flex-col gap-3 rounded-lg border border-border bg-card p-6">
          <div className="flex items-center gap-2 text-foreground">
            ...
            <h1 className="text-lg font-semibold">{t("advancedtoolsroute.general.advancedsetupisforadministrators")}</h1>
          </div>
          <p className="text-sm text-muted-foreground">
            {t("advancedtoolsroute.general.thisarealetsadministratorswireuptools")}{" "}
            <Link to="/apps" className="font-medium text-primary hover:underline">
              {t("advancedtoolsroute.general.yourapps")}</Link>
            .
          </p>
        </div>
      </div>
