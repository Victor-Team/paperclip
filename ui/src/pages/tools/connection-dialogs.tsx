  ...
  return (
    <Dialog open onOpenChange={(o) => !o && onClose()}>
      <DialogContent className="max-w-2xl">
        <DialogHeader>
          <DialogTitle>{t("connectiondialogs.general.toolcatalog")} {connection.name}</DialogTitle>
        </DialogHeader>
        {catalog.isLoading ? (
          <LoadingState />
        ) : catalog.error ? (
          ...
          <p className="py-6 text-sm text-muted-foreground">
            {t("connectiondialogs.general.notoolsdiscoveredyetuserefreshcatalog")}</p>
        ) : (
  ...
  return (
    <Dialog open onOpenChange={(o) => !o && onClose()}>
      <DialogContent className="max-w-xl">
        <DialogHeader>
          <DialogTitle>{t("connectiondialogs.general.addapplication")}</DialogTitle>
          <DialogDescription>
            {t("connectiondialogs.general.chooseanexistingapplicationorcreateone")}</DialogDescription>
        </DialogHeader>

        <div className="space-y-3">
          <div className="flex items-center gap-2 text-xs text-muted-foreground">
            <span className={step === 1 ? "font-medium text-foreground" : ""}>{t("connectiondialogs.general.1application")}</span>
            <span>/</span>
            <span className={step === 2 ? "font-medium text-foreground" : ""}>{t("connectiondialogs.general.2connection")}</span>
          </div>

          {step === 1 && !locked ? (
            <>
              <div className="space-y-1.5">
                <Label>{t("connectiondialogs.general.application")}</Label>
                <Select value={applicationMode} onValueChange={(v) => setApplicationMode(v as "existing" | "new")}>
                  <SelectTrigger>
                    <SelectValue />
                  </SelectTrigger>
                  <SelectContent>
                    <SelectItem value="existing">{t("connectiondialogs.general.useexistingapplication")}</SelectItem>
                    <SelectItem value="new">{t("connectiondialogs.general.createnewapplication")}</SelectItem>
                  </SelectContent>
                </Select>
              </div>
              ...
              {applicationMode === "existing" ? (
                <div className="space-y-1.5">
                  <Label>{t("connectiondialogs.general.existingapplication")}</Label>
                  <Select value={applicationId} onValueChange={setApplicationId}>
                    <SelectTrigger>
                      <SelectValue placeholder={t("connectiondialogs.general.selectanapplication")} />
                    </SelectTrigger>
                    <SelectContent>
                ...
                <div className="space-y-1.5">
                  <Label htmlFor="app-name">{t("connectiondialogs.general.newapplicationname")}</Label>
                  <Input
                    id="app-name"
                    value={applicationName}
                    onChange={(e) => setApplicationName(e.target.value)}
                    placeholder={t("connectiondialogs.general.eggithubtriage")}
                  />
                  <p className="text-xs text-muted-foreground">
                    {t("connectiondialogs.general.applicationtypeisinferredfromthetransport")}</p>
                </div>
              )}
            </>
          ) : null}
            ...
            <>
              {applicationMode === "new" ? (
                <div className="rounded-md border border-border bg-muted/30 px-3 py-2 text-xs text-muted-foreground">
                  <span className="font-medium text-foreground">{applicationName.trim()}</span> {t("connectiondialogs.general.willbecreatedas")}{" "}
                  {inferredType}.
                </div>
              ) : null}

              <div className="space-y-1.5">
                <Label htmlFor="conn-name">{t("connectiondialogs.general.connectionname")}</Label>
                <Input
                  id="conn-name"
                  value={name}
                  onChange={(e) => setName(e.target.value)}
                  placeholder={t("connectiondialogs.general.egproductiongithub")}
                  disabled={locked}
                />
              </div>

              <div className="space-y-1.5">
                <Label>{t("connectiondialogs.general.transport")}</Label>
                <Select
                  value={transport}
                ...
                >
                  ...
                  <SelectContent>
                    <SelectItem value="mcp_remote">{t("connectiondialogs.general.remotehttpnolocalprocess")}</SelectItem>
                    <SelectItem value="local_stdio">{t("connectiondialogs.general.localstdioapprovedtemplate")}</SelectItem>
                  </SelectContent>
                </Select>
              </div>
              ...
              {transport === "mcp_remote" ? (
                <div className="space-y-1.5">
                  <Label htmlFor="conn-url">{t("connectiondialogs.general.endpointurl")}</Label>
                  <Input
                    id="conn-url"
                    value={endpointUrl}
                ...
                <div className="space-y-1.5">
                  <Label>{t("connectiondialogs.general.commandtemplate")}</Label>
                  <Select value={templateId} onValueChange={setTemplateId} disabled={locked}>
                    <SelectTrigger>
                      <SelectValue placeholder={t("connectiondialogs.general.selectanapprovedtemplate")} />
                    </SelectTrigger>
                    <SelectContent>
                  ...
                  <p className="text-xs text-muted-foreground">
                    {t("connectiondialogs.general.onlyboardapprovedcommandtemplatescanrun")}</p>
                </div>
              )}

              {/* Vault-reference credential picker — no free-text token field. */}
              <div className="space-y-1.5">
                <Label>{t("connectiondialogs.general.credentialreferences")}</Label>
                {creds.length > 0 ? (
                ...
                {!locked ? (
                  <>
                    <div className="flex items-end gap-2">
                      <div className="flex-1 space-y-1">
                        <Select value={pendingSecretId} onValueChange={setPendingSecretId}>
                          <SelectTrigger>
                            <SelectValue placeholder={t("connectiondialogs.general.selectavaultsecret")} />
                          </SelectTrigger>
                          <SelectContent>
                            {(secrets.data ?? []).map((s) => (
                      ...
                      <Input
                        ...
                        placeholder={t("connectiondialogs.general.header")}
                        className="w-32"
                        aria-label={t("connectiondialogs.general.headername")}
                      />
                      <Button type="button" size="sm" variant="outline" onClick={addCred} disabled={!pendingSecretId}>
                        {t("connectiondialogs.general.add")}</Button>
                    </div>
                    {pendingSecretId ? (
                    ...
                    <p className="text-xs text-muted-foreground">
                      {t("connectiondialogs.general.freetextsecretsarenotacceptedpick")}<span className="font-mono"> {t("connectiondialogs.general.vault")}</span> {t("connectiondialogs.general.referenceandresolvesitatgatewayuse")}</p>
                  </>
                ) : null}
              </div>
            </>
          ...
          {locked ? (
            probe.isPending ? (
              <div className="rounded-md border border-border bg-muted/40 p-3">
                <LoadingState label={t("connectiondialogs.general.probingconnection")} />
              </div>
            ) : probe.isError ? (
              ...
              <div className="rounded-md border border-border bg-muted/40 p-3">
                <div className="flex items-center gap-2 text-sm">
                  <span className="font-medium text-foreground">{t("connectiondialogs.general.proberesult")}</span>
                ...
                <div className="mt-2 grid grid-cols-3 gap-2">
                  <div>
                    ...
                    <p className="text-xs text-muted-foreground">{t("connectiondialogs.general.toolsdiscovered")}</p>
                  </div>
                  <div>
                    <p className="text-lg font-semibold tabular-nums text-foreground">
                      {probeResult.latencyMs != null ? `${probeResult.latencyMs}ms` : "—"}
                    </p>
                    <p className="text-xs text-muted-foreground">{t("connectiondialogs.general.probelatency")}</p>
                  </div>
                  <div>
                    <p className="text-lg font-semibold tabular-nums text-foreground">
                      {probeResult.quarantinedCount}
                    </p>
                    <p className="text-xs text-muted-foreground">{t("connectiondialogs.general.quarantined")}</p>
                  </div>
                </div>
                ...
                <p className="mt-2 text-(length:--text-micro) text-muted-foreground">
                  {t("connectiondialogs.general.probelatencyisasingleroundtrip")}</p>
              </div>
            ) : null
          ) : null}
        </div>
        ...
        <DialogFooter>
          <Button variant="outline" onClick={onClose}>
            {t("connectiondialogs.general.cancel")}</Button>
          {step === 1 && !locked ? (
            <Button disabled={!appChoiceValid} onClick={() => setStep(2)}>
              {t("connectiondialogs.general.continue")}</Button>
          ) : !locked ? (
            <>
              <Button variant="outline" onClick={() => setStep(1)}>
                {t("connectiondialogs.general.back")}</Button>
              <Button disabled={!canCreate} onClick={() => create.mutate()}>
                {create.isPending ? t("connectiondialogs.general.creatingdraft") : t("connectiondialogs.general.createprobe")}
              </Button>
            </>
          ) : (
            <>
              <Button variant="outline" disabled={probe.isPending} onClick={() => draft && probe.mutate(draft.id)}>
                ...
                {probe.isPending ? t("connectiondialogs.general.probing") : t("connectiondialogs.general.reprobe")}
              </Button>
              <Button disabled={activate.isPending || probe.isPending} onClick={() => draft && activate.mutate(draft.id)}>
                {activate.isPending ? t("connectiondialogs.general.activating") : t("connectiondialogs.general.activate")}
              </Button>
            </>
          )}
        </DialogFooter>
