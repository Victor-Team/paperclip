    ...
    <div className="space-y-6">
        ...
        actions={
          <div className="flex flex-wrap gap-2">
            <Button variant="outline" disabled={archived} onClick={() => setDialog("edit")}>
              ...
              {t("profiledetail.general.edit")}</Button>
            <Button variant="outline" disabled={archived} onClick={() => setDialog("duplicate")}>
              <Copy className="mr-1.5 h-4 w-4" />
              {t("profiledetail.general.duplicate")}</Button>
            {archived ? (
              <Button variant="outline" onClick={() => setDialog("restore")}>
                <ArchiveRestore className="mr-1.5 h-4 w-4" />
                {t("profiledetail.general.restore")}</Button>
            ) : (
              <Button variant="outline" onClick={() => setDialog("archive")}>{t("profiledetail.general.archive")}</Button>
            )}
            <Button variant="outline" className="text-destructive hover:text-destructive" onClick={() => setDialog("delete")}>
              <Trash2 className="mr-1.5 h-4 w-4" />
              {t("profiledetail.general.delete")}</Button>
          </div>
        }
      />
      ...
      {created ? (
        <div className="flex items-center justify-between gap-3 rounded-lg border border-primary/30 bg-primary/5 px-4 py-3">
          <div>
            <p className="text-sm font-medium text-foreground">{t("profiledetail.general.profilesaved")}</p>
            <p className="text-sm text-muted-foreground">
              {unassigned ? t("profiledetail.general.assignittoagentsbeforeitchanges") : t("profiledetail.general.assignmentsareactivenow")}
            </p>
          </div>
          <div className="flex gap-2">
            <Button size="sm" onClick={() => navigate(`/apps/advanced/profiles/${profile.id}/edit?step=3`)}>
              {t("profiledetail.general.assign")}</Button>
            <Button size="sm" variant="ghost" onClick={() => setSearchParams({})}>
              {t("profiledetail.general.dismiss")}</Button>
          </div>
        </div>
      ) : null}
      ...
      {archived ? (
        <div className="rounded-lg border border-amber-300 bg-amber-50 px-4 py-3 text-sm text-amber-950">
          {t("profiledetail.general.thisprofileisarchiveditdoesnot")}</div>
      ) : null}
      ...
      <section className="space-y-3">
        <div className="flex items-center justify-between gap-3">
          <h2 className="text-base font-semibold text-foreground">{t("profiledetail.general.whatitallows")}</h2>
          <Button variant="outline" size="sm" disabled={archived} onClick={() => navigate(`/apps/advanced/profiles/${profile.id}/edit?step=2`)}>
            {t("profiledetail.general.edittools")}</Button>
        </div>
      ...
      <section className="space-y-3">
        <div className="flex items-center justify-between gap-3">
          <h2 className="text-base font-semibold text-foreground">{t("profiledetail.general.whohasit")}</h2>
          <Button variant="outline" size="sm" disabled={archived} onClick={() => navigate(`/apps/advanced/profiles/${profile.id}/edit?step=3`)}>
            {t("profiledetail.general.assign1")}</Button>
        </div>
        <Assignments
          profile={profile}
          companyId={companyId}
      ...
      <section className="space-y-3">
        <h2 className="text-base font-semibold text-foreground">{t("profiledetail.general.newtoolsthatappearlater")}</h2>
        <NewToolsSetting
      ...
      <Button variant="link" className="h-auto px-0" onClick={() => navigate("/apps/advanced/profiles?check=1")}>
        ...
        {t("profiledetail.general.checkwhatanagentcanactuallydo")}</Button>
...
function NewToolsReviewBanner({
  ...
  return (
    <div className="flex flex-wrap items-center justify-between gap-3 rounded-lg border border-amber-300 bg-amber-50 px-4 py-3 text-sm text-amber-950">
      ...
      <Button size="sm" onClick={onReview}>{t("profiledetail.general.review")}</Button>
    </div>
  );
}
  ...
  return (
    <Dialog open={open} onOpenChange={(next) => !next && onClose()}>
      <DialogContent className="max-w-2xl">
        <DialogHeader>
          <DialogTitle>{t("profiledetail.general.reviewnewtools")}</DialogTitle>
          <DialogDescription>
            {t("profiledetail.general.allowthetoolsthisprofileshoulduse")}</DialogDescription>
        </DialogHeader>
        {loading ? (
          <LoadingState label={t("profiledetail.general.loadingnewtools")} />
        ) : error ? (
          <ErrorState error={error} onRetry={onRetry} />
        ) : tools.length === 0 ? (
          <div className="rounded-lg border border-dashed border-border px-4 py-6 text-sm text-muted-foreground">
            {t("profiledetail.general.therearenonewtoolswaitingfor")}</div>
        ) : (
              ...
              <div key={tool.catalogEntryId} className="grid gap-3 px-3 py-3 sm:grid-cols-[1fr_auto]">
                ...
                <div className="flex items-center gap-2 sm:justify-end">
                  <label className="inline-flex items-center gap-1.5 text-sm">
                    ...
                    />
                    {t("profiledetail.general.allow")}</label>
                  <label className="inline-flex items-center gap-1.5 text-sm">
                    <input
                      type="radio"
                      name={`review-${tool.catalogEntryId}`}
                    ...
                    />
                    {t("profiledetail.general.keepblocked")}</label>
                </div>
              </div>
            ))}
          </div>
        ...
        <DialogFooter>
          <Button variant="ghost" onClick={onClose}>{t("profiledetail.general.cancel")}</Button>
          <Button disabled={pending || loading || tools.length === 0} onClick={onSubmit}>
            {t("profiledetail.general.submitreview")}</Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
  );
...
function AllowList({ rows, total }: { rows: AllowRow[]; total: number }) {
  ...
  if (rows.length === 0) {
    return (
      <div className="rounded-lg border border-amber-300 bg-amber-50 px-4 py-3 text-sm text-amber-950">
        {t("profiledetail.general.thisprofileallows0toolsagentswith")}</div>
    );
  }
  return (
      ...
      <table className="w-full text-sm">
        <thead>
          <tr className="border-b border-border bg-muted/40 text-left text-xs font-medium text-muted-foreground">
            <th className="px-3 py-2 font-medium">{t("profiledetail.general.tool2")}</th>
            <th className="px-3 py-2 font-medium">{t("profiledetail.general.app")}</th>
            <th className="px-3 py-2 font-medium">{t("profiledetail.general.capabilities")}</th>
            <th className="px-3 py-2 font-medium">{t("profiledetail.general.source")}</th>
          </tr>
        </thead>
        <tbody>
                ...
                <div className="flex flex-col">
                  ...
                  {row.degraded ? (
                    <a className="mt-0.5 inline-flex items-center gap-1 text-xs font-medium text-primary hover:underline" href={`/apps/${row.connectionId}`}>
                      ...
                      {t("profiledetail.general.reconnect")}</a>
                  ) : null}
                </div>
              </td>
              <td className="px-3 py-2">
  ...
  if (profile.bindings.length === 0) {
    return (
      <div className="rounded-lg border border-dashed border-border px-4 py-5">
        <p className="text-sm font-medium text-foreground">{t("profiledetail.general.notassignedyet")}</p>
        <p className="text-sm text-muted-foreground">{t("profiledetail.general.assignthisprofilebeforeitchangesaccess")}</p>
      </div>
    );
  }
  return (
      ...
      {profile.bindings.map((binding) => (
        <div key={binding.id} className="flex items-center justify-between gap-3 px-3 py-2.5">
          ...
          <Button variant="ghost" size="sm" disabled={archived} onClick={() => onRemove(binding)}>
            ...
            {t("profiledetail.general.remove")}</Button>
        </div>
      ))}
    </div>
  );
  ...
  if (kind === "edit") {
    return (
      <Dialog open={open} onOpenChange={(next) => !next && onClose()}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>{t("profiledetail.general.editprofile")}</DialogTitle>
            <DialogDescription>{t("profiledetail.general.updatetheprofilenameanddescription")}</DialogDescription>
          </DialogHeader>
          <div className="space-y-3">
            <div className="space-y-1.5">
              <Label htmlFor="edit-profile-name">{t("profiledetail.general.name")}</Label>
              <Input id="edit-profile-name" value={name} onChange={(e) => setName(e.target.value)} />
              {duplicateName ? <p className="text-xs text-destructive">{t("profiledetail.general.anotherprofilealreadyusesthisname")}</p> : null}
            </div>
            <div className="space-y-1.5">
              <Label htmlFor="edit-profile-description">{t("profiledetail.general.description")}</Label>
            ...
            <button type="button" className="text-sm font-medium text-muted-foreground hover:text-foreground" onClick={() => setAdvancedOpen((v) => !v)}>
              {t("profiledetail.general.advanced")}</button>
            {advancedOpen ? (
              <div className="space-y-1.5">
                <Label htmlFor="edit-profile-key">{t("profiledetail.general.identifier")}</Label>
                <Input id="edit-profile-key" value={profileKey} onChange={(e) => setProfileKey(e.target.value)} className="font-mono text-xs" />
              </div>
            ) : null}
          </div>
          <DialogFooter>
            <Button variant="ghost" onClick={onClose}>{t("profiledetail.general.cancel3")}</Button>
            <Button disabled={!name.trim() || duplicateName || pending} onClick={() => onUpdate({ name: name.trim(), description: description.trim() || null, profileKey: profileKey.trim() })}>
              {t("profiledetail.general.save")}</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
    );
  ...
  if (kind === "duplicate") {
    return (
      <Dialog open={open} onOpenChange={(next) => !next && onClose()}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>{t("profiledetail.general.duplicateprofile")}</DialogTitle>
            <DialogDescription>{t("profiledetail.general.thecopystartsunassignedunlessyouchoose")}</DialogDescription>
          </DialogHeader>
          <div className="space-y-3">
            <div className="space-y-1.5">
              <Label htmlFor="copy-profile-name">{t("profiledetail.general.name4")}</Label>
              <Input id="copy-profile-name" value={copyName} onChange={(e) => setCopyName(e.target.value)} />
              {duplicateCopyName ? <p className="text-xs text-destructive">{t("profiledetail.general.anotherprofilealreadyusesthisname5")}</p> : null}
            </div>
            <label className="flex items-center gap-2 text-sm">
              <input type="checkbox" checked={copyAssignments} onChange={(e) => setCopyAssignments(e.target.checked)} />
              {t("profiledetail.general.alsocopyassignments")}</label>
          </div>
          <DialogFooter>
            <Button variant="ghost" onClick={onClose}>{t("profiledetail.general.cancel6")}</Button>
            <Button disabled={!copyName.trim() || duplicateCopyName || pending} onClick={() => onDuplicate({ name: copyName.trim(), includeAssignments: copyAssignments })}>
              {t("profiledetail.general.duplicate7")}</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
    );
...
function RemoveAssignmentDialog({
  ...
  return (
    <Dialog open={Boolean(binding)} onOpenChange={(next) => !next && onClose()}>
      <DialogContent>
        <DialogHeader>
          <DialogTitle>{t("profiledetail.general.removeassignment")}</DialogTitle>
          <DialogDescription>
        ...
        <DialogFooter>
          <Button variant="ghost" onClick={onClose}>{t("profiledetail.general.cancel8")}</Button>
          <Button disabled={pending} onClick={onConfirm}>{t("profiledetail.general.remove9")}</Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
  );
