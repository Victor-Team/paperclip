...
const SECTION_TITLES: Record<RoutineSectionKey, string> = {
  overview: "routinedetail.general.sectionOverview",
  triggers: "routinedetail.general.sectionTriggers",
  variables: "routinedetail.general.sectionVariables",
  secrets: "routinedetail.general.sectionSecrets",
  delivery: "routinedetail.general.sectionDelivery",
  runs: "routinedetail.general.sectionRuns",
  activity: "routinedetail.general.sectionActivity",
  history: "routinedetail.general.sectionHistory",
};
...
export function RoutineDetail() {
  ...
  const createSecret = useMutation({
    mutationFn: (input: { name: string; value: string }) => {
      if (!selectedCompanyId) throw new Error(t("routinedetail.general.selectCompanyForSecretsProduction"));
  ...
  const dirtyFields = useMemo<RoutineHistoryDirtyFieldDescriptor[]>(() => {
    ...
    if (editDraft.title !== routineDefaults.title) result.push({ key: "title", label: t("routinedetail.general.fieldTitle") });
    if (editDraft.description !== routineDefaults.description) {
      result.push({ key: "description", label: t("routinedetail.general.fieldDescription") });
    }
    if (editDraft.projectId !== routineDefaults.projectId) {
      result.push({ key: "projectId", label: t("routinedetail.general.fieldProject") });
    }
    if (editDraft.assigneeAgentId !== routineDefaults.assigneeAgentId) {
      result.push({ key: "assigneeAgentId", label: t("routinedetail.general.fieldDefaultAgent") });
    }
    if (editDraft.priority !== routineDefaults.priority) {
      result.push({ key: "priority", label: t("routinedetail.general.fieldPriority") });
    }
    if (editDraft.concurrencyPolicy !== routineDefaults.concurrencyPolicy) {
      result.push({ key: "concurrencyPolicy", label: t("routinedetail.general.fieldConcurrencyPolicy") });
    }
    if (editDraft.catchUpPolicy !== routineDefaults.catchUpPolicy) {
      result.push({ key: "catchUpPolicy", label: t("routinedetail.general.fieldCatchUpPolicy") });
    }
    if (editDraft.activityGatePolicy !== routineDefaults.activityGatePolicy) {
      result.push({ key: "activityGatePolicy", label: t("routinedetail.general.fieldAdvancedRunPolicy") });
    }
    if (editDraft.activityGateScope !== routineDefaults.activityGateScope) {
      result.push({ key: "activityGateScope", label: t("routinedetail.general.fieldActivityGateScope") });
    }
    if (JSON.stringify(editDraft.variables) !== JSON.stringify(routineDefaults.variables)) {
      result.push({ key: "variables", label: t("routinedetail.general.fieldVariables") });
    }
    if (JSON.stringify(editDraft.env ?? null) !== JSON.stringify(routineDefaults.env ?? null)) {
      result.push({ key: "env", label: t("routinedetail.general.fieldSecrets") });
    }
    return result;
  ...
  useEffect(() => {
    if (!routine) return;
    setBreadcrumbs([{ label: t("routinedetail.general.routinesBreadcrumb"), href: "/routines" }, { label: routine.title }]);
    if (!routineDefaults) return;
  ...
  const copySecretValue = useCallback(
    async (label: string, value: string) => {
      try {
        ...
        pushToast({ title: t("routinedetail.general.copied", { label }), tone: "success" });
      } catch (copyError) {
        pushToast({
          title: t("routinedetail.general.failedToCopy", { label: label.toLowerCase() }),
          body: copyError instanceof Error ? copyError.message : t("routinedetail.general.clipboardDenied"),
          tone: "error",
        });
      }
    },
  ...
  const saveRoutine = useMutation({
    ...
    onError: (mutationError) => {
      if (mutationError instanceof ApiError && mutationError.status === 409) {
        ...
        pushToast({
          title: t("routinedetail.general.routineChanged"),
          body: t("routinedetail.general.routineChangedBody"),
          tone: "warn",
        });
        return;
      }
      pushToast({
        title: t("routinedetail.general.failedToSaveRoutine"),
        body: mutationError instanceof Error ? mutationError.message : t("routinedetail.general.couldNotSaveRoutine"),
        tone: "error",
      });
    },
  });
  ...
  const runRoutine = useMutation({
    ...
    onSuccess: async () => {
      pushToast({ title: t("routinedetail.general.routineRunStarted"), tone: "success" });
    ...
    onError: (runError) => {
      pushToast({
        title: t("routinedetail.general.routineRunFailed"),
        body: runError instanceof Error ? runError.message : t("routinedetail.general.couldNotStartRoutineRun"),
        tone: "error",
      });
    },
  });
  ...
  const updateRoutineStatus = useMutation({
    ...
    onSuccess: async (_data, status) => {
      pushToast({
        title: t("routinedetail.general.routineSaved"),
        body: status === "paused" ? t("routinedetail.general.automationPaused") : t("routinedetail.general.automationEnabled"),
        tone: "success",
      });
      await Promise.all([
        queryClient.invalidateQueries({ queryKey: queryKeys.routines.detail(routineId!) }),
    ...
    onError: (statusError) => {
      pushToast({
        title: t("routinedetail.general.failedToUpdateRoutine"),
        body: statusError instanceof Error ? statusError.message : t("routinedetail.general.couldNotUpdateRoutine"),
        tone: "error",
      });
    },
  });
  ...
  const createTrigger = useMutation({
    ...
    onSuccess: async (result) => {
      if (result.secretMaterial) {
        setSecretMessage({
          title: t("routinedetail.general.webhookTriggerCreated"),
          entries: [{ webhookUrl: result.secretMaterial.webhookUrl, webhookSecret: result.secretMaterial.webhookSecret }],
        });
      } else {
        pushToast({ title: t("routinedetail.general.triggerAdded"), body: t("routinedetail.general.routineScheduleSaved"), tone: "success" });
      }
      await Promise.all([
        queryClient.invalidateQueries({ queryKey: queryKeys.routines.detail(routineId!) }),
        queryClient.invalidateQueries({ queryKey: queryKeys.routines.list(selectedCompanyId!) }),
    ...
    onError: (triggerError) => {
      pushToast({
        title: t("routinedetail.general.failedToAddTrigger"),
        body: triggerError instanceof Error ? triggerError.message : t("routinedetail.general.couldNotCreateTrigger"),
        tone: "error",
      });
    },
  });
  ...
  const updateTrigger = useMutation({
    ...
    onSuccess: async () => {
      pushToast({ title: t("routinedetail.general.triggerSaved"), body: t("routinedetail.general.routineCadenceSaved"), tone: "success" });
      await Promise.all([
        queryClient.invalidateQueries({ queryKey: queryKeys.routines.detail(routineId!) }),
        queryClient.invalidateQueries({ queryKey: queryKeys.routines.list(selectedCompanyId!) }),
        queryClient.invalidateQueries({ queryKey: queryKeys.routines.activity(selectedCompanyId!, routineId!) }),
    ...
    onError: (triggerError) => {
      pushToast({
        title: t("routinedetail.general.failedToUpdateTrigger"),
        body: triggerError instanceof Error ? triggerError.message : t("routinedetail.general.couldNotUpdateTrigger"),
        tone: "error",
      });
    },
  });
  ...
  const deleteTrigger = useMutation({
    ...
    onSuccess: async () => {
      pushToast({ title: t("routinedetail.general.triggerDeleted"), tone: "success" });
      await Promise.all([
        queryClient.invalidateQueries({ queryKey: queryKeys.routines.detail(routineId!) }),
        queryClient.invalidateQueries({ queryKey: queryKeys.routines.list(selectedCompanyId!) }),
        queryClient.invalidateQueries({ queryKey: queryKeys.routines.activity(selectedCompanyId!, routineId!) }),
    ...
    onError: (triggerError) => {
      pushToast({
        title: t("routinedetail.general.failedToDeleteTrigger"),
        body: triggerError instanceof Error ? triggerError.message : t("routinedetail.general.couldNotDeleteTrigger"),
        tone: "error",
      });
    },
  });
  ...
  const rotateTrigger = useMutation({
    ...
    onSuccess: async (result) => {
      setSecretMessage({
        title: t("routinedetail.general.webhookSecretRotated"),
        entries: [{ webhookUrl: result.secretMaterial.webhookUrl, webhookSecret: result.secretMaterial.webhookSecret }],
      });
      await Promise.all([
        queryClient.invalidateQueries({ queryKey: queryKeys.routines.detail(routineId!) }),
    ...
    onError: (triggerError) => {
      pushToast({
        title: t("routinedetail.general.failedToRotateWebhookSecret"),
        body: triggerError instanceof Error ? triggerError.message : t("routinedetail.general.couldNotRotateWebhookSecret"),
        tone: "error",
      });
    },
  });
      ...
      setSecretMessage({
        title:
          response.secretMaterials.length === 1
            ? t("routinedetail.general.webhookTriggerRestored")
            : t("routinedetail.general.webhookTriggersRestored", { count: response.secretMaterials.length }),
  ...
  if (!selectedCompanyId) {
    return <EmptyState icon={Repeat} message={t("routinedetail.general.selectCompanyToViewRoutinesProduction")} />;
  }
  ...
  if (error || !routine || !routineDefaults) {
    return (
      <EmptyState
        icon={AlertCircle}
        message={error instanceof Error ? error.message : t("routinedetail.general.couldNotLoadRoutine")}
      />
    );
  }
  ...
  const automationLabel =
    routine.status === "archived"
      ? t("routinedetail.general.statusArchived")
      : !routine.assigneeAgentId
        ? t("routinedetail.general.statusDraft")
        : automationEnabled
          ? t("routinedetail.general.statusActive")
          : t("routinedetail.general.statusPaused");
  ...
  const contextValue: RoutineDetailContextValue = {
    ...
    onToggleAutomation: () => {
      if (!automationEnabled && !routine.assigneeAgentId) {
        pushToast({
          title: t("routinedetail.general.defaultAgentRequired"),
          body: t("routinedetail.general.setDefaultAgent"),
          tone: "warn",
        });
        return;
      }
  ...
  return (
    <RoutineDetailContext.Provider value={contextValue}>
      ...
      >
        {t("routinedetailproduction.general.skiptosection")}</a>
        ...
        <header className="flex h-14 shrink-0 items-center gap-3 border-b border-border bg-background px-6">
          <div className="flex min-w-0 flex-1 items-center gap-3">
            <textarea
              ...
              placeholder={t("routinedetailproduction.general.routinetitle")}
              rows={1}
          ...
          <div className="ml-auto flex shrink-0 items-center gap-3">
            ...
            <div className="flex items-center gap-2">
              <ToggleSwitch
                ...
                aria-label={automationEnabled ? t("routinedetail.general.pauseAutomaticTriggers") : t("routinedetail.general.enableAutomaticTriggers")}
              />
